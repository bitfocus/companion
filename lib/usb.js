...
function usb(system, type, devicepath, cb) {
	...
	var child = (self.child = cp.fork(path.join(__dirname, 'usb/handler.js'), [], {
		...
		env: {
			...
			COMPANION_USE_HIDRAW: process.env.COMPANION_USE_HIDRAW,
		},
	}))

	const graphics_set_bank_bg = (page, bank, bgcolor) => {
		child.send({ id: self.id, cmd: 'system', args: ['graphics_set_bank_bg', page, bank, bgcolor] })
	}

	const doCleanup = () => {
		system.off('graphics_set_bank_bg', graphics_set_bank_bg)
	}

	child.on('exit', doCleanup)

	system.on('graphics_set_bank_bg', graphics_set_bank_bg)
...
usb.prototype.setConfig = function (config, cb) {
	var self = this

	self._execute('setConfig', [config], cb)
}
