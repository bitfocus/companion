const TriggerPluginBase = require('./Base')
const moment = require('moment')

class TriggerPluginTimeOfDay extends TriggerPluginBase {
	setup() {
		this.day_format = 'ddd'

		this.interval_watch = setInterval(() => {
			const now = new Date()
			const hms = moment(now).format('HH:mm:ss')
			this.matches(now.getDay(), hms)
		}, 1000)

		this.options = [
			{
				key: 'time',
				name: 'Time',
				type: 'textinput',
				placeholder: 'Time in format HH:MM:SS',
				pattern: '(0[0-9]|1[0-9]|2[0-3])(:[0-5][0-9]){2}',
			},
			{
				key: 'days',
				name: 'Days',
				type: 'select',
				multi: true,
				choices: this._day_list(),
			},
		]
	}

	/**
	 * @return {Array.<{id: number, label: string}>} List of days
	 */
	_day_list() {
		return [...Array(7).keys()].map((i) => {
			return {
				id: i,
				label: moment().weekday(i).format(this.day_format),
			}
		})
	}

	/**
	 * Checks if event should work
	 * @param {number} day
	 * @param {string} hms
	 */
	matches(day, hms) {
		for (const x of this.watch) {
			if ((x.config.days.includes(day) || x.config.days.includes(day.toString())) && x.config.time === hms) {
				this.scheduler.action(x.id)
			}
		}
	}

	get type() {
		return 'tod'
	}

	get name() {
		return 'Time of day'
	}

	config_desc(config) {
		const days = config.days
		let day_str = days.toString()
		if (days.length === 7) {
			day_str = 'Daily'
		} else if (day_str === '1,2,3,4,5') {
			day_str = 'Weekdays'
		} else if (day_str === '0,6') {
			day_str = 'Weekends'
		} else {
			try {
				day_str = days.map((d) => moment().weekday(d).format('ddd')).join(', ')
			} catch (e) {
				day_str = 'Error'
			}
		}

		return `<strong>${day_str}</strong>, ${config.time}`
	}
}

module.exports = TriggerPluginTimeOfDay
