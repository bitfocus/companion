...
class PageController extends CoreBase {
	...
	setControlIdAt(pageNumber, coordinate, controlId) {
		...
		if (page) {
			// Update the cache
			const oldControlId = page.controlId[coordinate]
			if (oldControlId) {
	 ...
	 * @access public
	 */
	setPage(pageNumber, value, redraw = true) {
		// TODO - review all usages to make them populate things correctly
		if (!value) value = { name: 'PAGE', controls: {} }

		// Update cache for old controls
		...
		// Update cache for old controls
		if (this.pages[pageNumber]) {
			for (const [coordinate, controlId] of Object.entries(this.pages[pageNumber])) {
				this.#locationCache.set(controlId, { pageNumber, coordinate })
			}
		}
	...
	#commitChanges(page, newValue, redraw = true) {
		this.io.emitToRoom(PagesRoom, 'pages:update', page, newValue)

		this.db.setKey('page', this.pages)

		this.emit('name', page, newValue?.name)
