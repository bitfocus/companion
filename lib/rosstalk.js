...
class rosstalk {
	constructor(_system) {
		...
		this.system.emit('get_userconfig', obj => {
			...
			if(this.config['rosstalk_enabled'] === undefined) {
				...
				system.emit('set_userconfig_key', 'rosstalk_enabled', this.config['rosstalk_enabled'])
			}
		...
		this.system.on('set_userconfig_key', (key, val) => {
			if(key !== 'rosstalk_enabled') return;

			try {
				if(val === true) this.listen();
					else this.close();
			} catch(e) {
	...
	close() {
		if(this.active && !this.config.rosstalk_enabled) {
	...
	process_incomming(data) {
		...
		if(match[1] === 'CC') this.press_button(match[2], match[3]);
	}
