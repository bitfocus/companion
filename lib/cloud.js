...
const REGIONS = {
	'no-oslo-cloud1': { host: 'no-oslo-cloud1.bitfocus.io', name: 'Europe' },
...
class Cloud {
	...
	async handleCloudLogin(client, email, password) {
		...
		try {
			...
			this.setState({ authenticated: false, authenticating: false, error: 'Cannot reach authentication/cloud-api server' })
			this.destroy()
			return
		}
	...
	async handleCloudRefresh(token) {
		...
		try {
			...
			if (result.token) {
				...
				this.system.emit('db_set', 'cloud', this.data)
				this.setState({ authenticated: true, authenticatedAs: result.customer?.email, authenticating: false, error: null })
			} else {
				this.setState({ authenticated: false, authenticating: false, error: 'Cannot refresh login token, please login again.' })
			}
		} catch (e) {
	...
	readConnections(connections) {
		...
		if (connections) {
			let update = {}
			console.log("HAS REGIONS? ", this.regions);
			for (let region in connections) {
				console.log("Has region: ", region, region in this.regions);
				if (this.regions[region]) {
					console.log("UPDATING INITIAL CONNECTION STATE FOR", region, connections[region])
	...
	saveConnection(region, enabled) {
		...
		console.log("SET CONNECTIONS", this.data.connections);
		this.system.emit('db_set', 'cloud', this.data)
	}
