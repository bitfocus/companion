...
class Cloud {
	...
	constructor(system) {
		...
		this.system.emit('io_get', (io) => {
			this.debug('constructor:io_get->response')
				...
				for await (let event of currentSocket.listener('error')) {
					if (event.error.code === 4401) {
						...
						this.setState({
							...
							connected: { ...this.state.connected, [region]: false },
							pingResults: { ...this.state.pingResults, [region]: -1 },
						})
					} else {
	...
	handleCloudStateSet(client, newState) {
		this.debug('handleCloudStateRequest', newState)
	...
	setState(draftState, sourceClient = null) {
		...
		if (this.token) {
			if (
				this.state['oal-clusterEnabled'] !== newState['oal-clusterEnabled'] ||
				this.state.authenticated !== newState.authenticated ||
				(this.sockets['oal-cluster'] === undefined && newState['oal-clusterEnabled'])
			) {
			...
			if (
				this.state.virginiaEnabled !== newState.virginiaEnabled ||
				this.state.authenticated !== newState.authenticated ||
				(this.sockets.virginia === undefined && newState.virginiaEnabled)
			) {
			...
			if (
				this.state.stockholmEnabled !== newState.stockholmEnabled ||
				this.state.authenticated !== newState.authenticated ||
				(this.sockets.stockholm === undefined && newState.stockholmEnabled)
			) {
