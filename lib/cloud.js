...
class Cloud {
	...
	async cloudConnect() {
		...
		(async () => {
			while (this.socket) {
				for await (let _event of this.socket.listener("connect")) {  // eslint-disable-line
					...
					try {
						const login = await this.socket.invoke('cloudLogin', { token: this.token, uuid: this.uuid });
			...
			while (this.socket) {
				for await (let event of this.socket.listener('authenticate')) {
					...
					this.system.emit('db_get', 'bank', (banks) => {
						...
						this.socket.transmitPublish('companion-banks:' + this.uuid, {
							type: 'full',
							data: banks
						});
					});	
		...
		this.registerCompanionProcs('getBanks', (...args) => this.clientGetBanks(...args));
		this.registerCompanionProcs('push', (...args) => this.clientPushBank(...args));
		this.registerCompanionProcs('release', (...args) => this.clientReleaseBank(...args));
	...
	registerCompanionProcs(name, callback) {
		if (typeof callback === 'function') {
			(async () => {
				for await (let data of this.socket.subscribe(`companionProc:${this.uuid}:${name}`, { waitForAuth: true })) {
					debug('Received RPC for %o', name);
					try {
						const result = await callback(...data.args);
						this.socket.invokePublish('companionProcResult:' + data.callerId, { result: result });
						debug('rpc result: %o : %o', 'companionProcResult:' + data.callerId, result);
					} catch (e) {
						this.socket.invokePublish('companionProcResult:' + data.callerId, { error: e.message });
	...
	async updateBank(page, bank) {
		this.system.emit('db_get', 'bank', (banks) => {
			// In case someone overwrites the reference elsewhere
			this.banks = banks;
