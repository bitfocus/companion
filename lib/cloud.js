...
const CLOUD_LOGIN_URL = process.env.NODE_ENV === 'production' ? 'https://cloud.bitfocus.io/api/auth/login' : 'http://127.0.0.1:3333/v1/auth/login'
			...
			while (this.socket) {
				for await (let event of this.socket.listener('authenticate')) {
					...
					try {
						...
						const result = await this.socket.invoke('companion-connect', this.clientId);
						debug('Register result: %o', result);
					} catch (e) {
						return this.system.emit('log', 'cloud', 'error', 'Error registering companion in cloud: ' + e.message);
					}
