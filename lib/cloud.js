...
const regions = {
	'oal-cluster': 'oal-cluster.staging.bitfocus.io',
	'stockholm': '127.0.0.1:8001',
	'virginia': '127.0.0.1:8001'
}

const CLOUD_URL = process.env.NODE_ENV === 'production' ? 'https://api.staging.bitfocus.io/v1' : 'http://127.0.0.1:8002/v1'
...
class Cloud {
	constructor(system) {
		...
		this.system.emit('db_get', 'cloud_user', (user) => {
			this.setState({ authenticatedAs: user })
		});
	...
	readConnections() {
		this.system.emit('db_get', 'cloud_connections', (connections) => {
	...
	saveConnections() {
		...
		for (let key of Object.keys(this.state)) {
			if (key.match(/Enabled$/) && this.state[key]) {
				update.push(key.substr(0, key.length - 7));
			}
		}
		this.system.emit('db_set', 'cloud_connections', update);
	}
	...
	async handleCloudLogin(client, email, password) {
		...
		try {
			...
			if (responseObject.token !== undefined) {
				...
				this.system.emit('db_set', 'cloud_user', email)
	...
	destroy(region) {
		if ((!region || region === 'oal-cluster') && this.sockets['oal-cluster']) {
			this.sockets['oal-cluster'].disconnect()
			delete this.sockets['oal-cluster']
			this.setState({ connected: { ...this.state.connected, 'oal-cluster': false } })
		}
		if ((!region || region === 'stockholm') && this.sockets.stockholm) {
			this.sockets.stockholm.disconnect()
			delete this.sockets.stockholm
			this.setState({ connected: { ...this.state.connected, stockholm: false } })
		}
		if ((!region || region === 'virginia') && this.sockets.virginia) {
			this.sockets.virginia.disconnect()
			delete this.sockets.virginia
			this.setState({ connected: { ...this.state.connected, virginia: false } })
		}
	...
	handleCloudLogout(client) {
		...
		this.system.emit('db_set', 'cloud_user', '')
		this.system.emit('db_set', 'cloud_connections', [])
		this.setState({ authenticated: false, 'oal-clusterEnabled': false, stockholmEnabled: false, virginiaEnabled: false, authenticatedAs: '' })
		this.destroy()
	}
	...
	setState(draftState, sourceClient = null) {
		...
		if (this.token) {
			if ((this.state['oal-clusterEnabled'] !== newState['oal-clusterEnabled'] || this.state.authenticated !== newState.authenticated) || (this.sockets['oal-cluster'] === undefined && newState['oal-clusterEnabled'])) {
				if (newState['oal-clusterEnabled'] && newState.authenticated) {
					this.cloudConnect('oal-cluster');
					this.saveConnections();
				} else {
					this.state = newState
					this.destroy('oal-cluster');
					this.saveConnections();
					abortState = true;
				}
			}

			if ((this.state.virginiaEnabled !== newState.virginiaEnabled || this.state.authenticated !== newState.authenticated) || (this.sockets.virginia === undefined && newState.virginiaEnabled)) {
			...
			if ((this.state.stockholmEnabled !== newState.stockholmEnabled || this.state.authenticated !== newState.authenticated) || (this.sockets.stockholm === undefined && newState.stockholmEnabled)) {
	...
	async cloudConnect(region) {
		...
		this.sockets[region] = SCClient.create({
			...
			secure: !regions[region].match(/^127\./),
			...
			while (this.sockets[region]) {
				...
				for await (let _event of currentSocket.listener("connect")) {  // eslint-disable-line
					...
					try {
						...
						this.setState({ error: e.message, connected: { ...this.state.connected, [region]: false } });
		...
		(async () => {
			while (this.sockets[region]) {
				...
				for await (let event of currentSocket.listener('authenticate')) {
					...
					this.setState({ connected: { ...this.state.connected, [region]: true } });
			...
			while (this.sockets[region]) {
				...
				for await (let event of currentSocket.listener('error')) {
					if (event.error.code === 4401) {
						...
						this.setState({ [region + 'Enabled']: false, connected: { ...this.state.connected, [region]: false }, pingResults: { ...this.state.pingResults, [region]: -1 } });
					} else {
						console.log(`DISCONNECT::::::::`, event);
						this.setState({ connected: { ...this.state.connected, [region]: false }, pingResults: { ...this.state.pingResults, [region]: -1 } });
					}
				}
			}
		})();
