 ...
 * Type is set to either 'buffer' or 'base64' according to your input.
 * Width and height is information about your buffer, not scaling.
 *
 * The buffer data is expected to be RGB data, 1 byte per color, horizontally.
 * Top left is first three bytes.
 */
image.prototype.drawPixelBuffer = function (x, y, width, height, buffer, type) {
	...
	if (buffer.length < x*y*3) {
		throw new Error('Pixelbuffer of ' + buffer.length + ' bytes is less than expected ' + (x*y*3) + ' bytes');
		return;
	}
