...
function loadsave(_system) {
	...
	system.emit('io_get', function (io) {
		io.on('connect', function (socket) {
			...
			socket.on('loadsave_reset_page_all', function (page) {
				...
				system.emit('bank_style', page, 6, 'pagenum');
				system.emit('bank_style', page, 11, 'pagedown');
			...
			socket.on('loadsave_reset_page_nav', function (page) {
				...
				system.emit('bank_reset', page, 6);
				system.emit('bank_reset', page, 11);
			...
			socket.on('loadsave_import_full', function (data) {
				...
				for (var page = 1; page <= 99; ++page) {
					...
					if (data.page !== undefined && data.page[page] !== undefined) {
						...
						system.emit('page_set_noredraw', page, { name: 'PAGE $(page)' });
					}
	...
	system.on('http_req', function (req, res, done) {
		...
		if (match = req.url.match(/^\/bank_export\/((\d+)\/(\d+))?/)) {
			...
			res.writeHeader(200, { 'Content-Type': 'application/json', 'Content-Disposition': 'attachment; filename="' + os.hostname() + '_bank ' + page + '-' + bank + '_' + getTimestamp() + '.companionconfig"' });
		...
		if (match = req.url.match(/^\/page_export\/((\d+))?/)) {
			...
			res.writeHeader(200, { 'Content-Type': 'application/json', 'Content-Disposition': 'attachment; filename="' + os.hostname() + '_page ' + page + '_' + getTimestamp() + '.companionconfig"' });
			res.end(JSON.stringify(exp));
		...
		if (match = req.url.match(/^\/full_export/)) {
			...
			res.writeHeader(200, { 'Content-Type': 'application/json', 'Content-Disposition': 'attachment; filename="' + os.hostname() + '_full-config' + '_' + getTimestamp() + '.companionconfig"' });
			res.end(JSON.stringify(exp));
