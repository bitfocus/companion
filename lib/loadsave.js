...
function loadsave(_system) {
		...
		io.on('connect', function (socket) {
			...
			socket.on('loadsave_import_page', function (topage, frompage, data) {
				if (data.type == 'full') {
					...
					data.actions = data.action === undefined ? [] : data.actions[frompage];
				...
				for (var bank in data.config) {
					var obj = {};
					obj.config = data.config[bank];
					obj.actions = data.actions[bank];
					obj.release_actions = data.release_actions[bank];
					obj.feedbacks = data.feedbacks[bank];
	...
	system.on('http_req', function (req, res, done) {
		...
		if (match = req.url.match(/^\/full_export/)) {
			...
			exp.bank_release_actions = self.bank_release_actions;
