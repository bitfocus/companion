...
function loadsave(_system) {
	...
	function version_check(obj) {
		...
		if (obj.version === 1) {
			...
			// upgrade further
			return version_check(data)
		} else if (obj.version === 2) {
			// actions & release_actions to action_sets
			// TODO - actions-sets upgrade
		}

		// Version 3 == no changes needed
		return obj
	}
		...
		system.on('io_connect', function (socket) {
			...
			socket.on('loadsave_import_page', function (topage, frompage, data) {
				...
				if (!data.action_sets) {
					...
					for (var page = 1; page <= 99; page++) {
						if (data.action_sets[page] === undefined) {
							data.action_sets[page] = {}
						}
						const page_config = (data.config || {})[page] || {}
						const page_actions = (data.actions || {})[page] || {}
						const page_release_actions = (data.release_actions || {})[page] || {}
						...
						for (var bank = 1; bank <= global.MAX_BUTTONS; ++bank) {
							self.system.emit(
								...
								function (res) {
									data.action_sets[page][bank] = res
								}
							)
						}
					}
