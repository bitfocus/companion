...
exports = module.exports = function (system, cfgdir) {
	return new InterfaceUpdate(system, cfgdir)
}

class InterfaceUpdate {
	debug = require('debug')('lib/Interface/Update')
	...
	uuid() {
		...
		if (fs.existsSync(this.cfgdir + 'machid')) {
			...
			try {
				...
				if (text) {
					uuid = text.toString()
					this.debug('read uuid', uuid)
				}
			} catch (e) {
				this.debug('error reading uuid-file', e)
			}
		} else {
			this.debug('creating uuid file')
			fs.writeFileSync(this.cfgdir + 'machid', uuid)
		}

		this.cached_uuid = uuid
