...
function server_tcp(system) {
	...
	system.on('graphics_set_bank_bg', (page, bank, bgcolor) => {
		if(self.clients) {
			...
			debug(`bank_bg send to all open sockets {type: bank_bg, page: ${page}, bank: ${bank}, red: ${color.red}, green: ${color.green}, blue: ${color.blue}}`)
			self.clients.forEach(socket => {
				socket.write(`{type: bank_bg, page: ${page}, bank: ${bank}, red: ${color.red}, green: ${color.green}, blue: ${color.blue}}` + '\n')
			});
		}
	})
