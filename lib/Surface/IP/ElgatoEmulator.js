...
import { formatCoordinate, oldBankIndexToCoordinate, xyToOldBankIndex } from '../../Shared/ControlId.js'
...
class SurfaceIPElgatoEmulator extends EventEmitter {
	...
	constructor(registry, emulatorId) {
		...
		this.imageCache = {}
		for (let key = 1; key <= this.info.keysTotal; key++) {
			const coord = oldBankIndexToCoordinate(key)
			this.imageCache[coord] = null
		}
	}
	...
	draw(x, y, buffer, style) {
		...
		if (key !== null) {
			...
			this.imageCache[coordinate] = buffer || null
	...
	clearDeck() {
		...
		this.imageCache = {}
		for (let key = 1; key <= this.info.keysTotal; key++) {
			const coordinate = oldBankIndexToCoordinate(key)
			this.imageCache[coordinate] = null
		}
