...
class SurfaceIPElgatoEmulator extends EventEmitter {
	...
	constructor(registry, devicepath) {
		...
		this.info = {
			...
			configFields: ['emulator_control_enable'],
			keysPerRow: 8,
			keysTotal: 32,
	...
	clientConnect(client) {
		client.onPromise('emulator:startup', () => {
	...
	setConfig(config) {
		if (this.io.countRoomMembers(EmulatorRoom) > 0) {
			const patch = jsonPatch.compare(this.#lastSentConfigJson || {}, config || {})
			if (patch.length > 0) {
				this.io.emitToRoom(EmulatorRoom, `emulator:config`, config)
			}
		}
