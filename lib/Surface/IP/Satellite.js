...
class SurfaceIPSatellite extends EventEmitter {
	...
	constructor(deviceInfo) {
		...
		this.#writeQueue = new ImageWriteQueue(
			this.#logger,
			async (/** @type {number} */ key, /** @type {Buffer} */ buffer, /** @type {*} */ style) => {
	...
	draw(x, y, render) {
		...
		if (this.#streamBitmapSize) {
			// Images need scaling
			this.#writeQueue.queue(key, render.buffer, render.style)
		} else {
