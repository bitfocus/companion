...
class SurfaceController extends CoreBase {
	 ...
	 * @type {Record<string, SurfaceHandler | undefined>}
	 * @access private
	 */
	#surfaceHandlers = {}
	 ...
	 * @type {Record<string, SurfaceGroup | undefined>}
	 * @access private
	 */
	#surfaceGroups = {}
	...
	constructor(registry) {
		...
		setImmediate(() => {
			...
			for (const groupId of Object.keys(groupsConfigs)) {
				...
				this.#surfaceGroups[groupId] = newGroup
			}
	...
	#createSurfaceHandler(surfaceId, integrationType, panel) {
		...
		// TODO-group - add/create surfacegroup
		if (!isLocked) {
		...
		const rawSurfaceGroupId = handler.getGroupId()
		const surfaceGroupId = rawSurfaceGroupId || handler.deviceId
		const existingGroup = this.#surfaceGroups[surfaceGroupId]
		if (existingGroup) {
			existingGroup.addSurface(handler)
		} else {
			const newGroup = new SurfaceGroup(this.registry, surfaceGroupId, !rawSurfaceGroupId ? handler : null)
			this.#surfaceGroups[surfaceGroupId] = newGroup
		}
	}
	...
	clientConnect(client) {
		...
		client.onPromise('surfaces:group-remove', (groupId) => {
			const group = this.#surfaceGroups[groupId]
			if (!group) throw new Error(`Group does not exist`)

			delete this.#surfaceGroups[groupId]
	...
	getDevicesList() {
		...
		for (const group of Object.values(this.#surfaceGroups)) {
			if (group) {
				res.groups[group.groupId] = {
					id: group.groupId,
		...
		offlineDevicesInfo.forEach((info, index) => {
			...
			// TODO-group this is bad and has bad edge cases
			const groupId = info.groupId || info.id
			if (!res.groups[groupId]) {
				res.groups[groupId] = {
					id: groupId,
					displayName: `${info.name || info.type} (${info.id}) - Offline`,
					isAutoGroup: true,
				}
			}
		})
	...
	removeDevice(devicePath, purge) {
		const surfaceHandler = this.#surfaceHandlers[devicePath]
		if (surfaceHandler !== undefined) {
			this.logger.silly('remove device ' + devicePath)

			// Cleanup an auto surface group
			const deviceId = surfaceHandler.deviceId
			if (this.#surfaceGroups[deviceId]) {
				delete this.#surfaceGroups[deviceId]
			}
	...
	#getGroupForId(deviceOrGroupId, looseIdMatching) {
		const matchingGroup = this.#surfaceGroups[deviceOrGroupId]
		...
		if (device) {
			...
			return this.#surfaceGroups[groupId]
		}
