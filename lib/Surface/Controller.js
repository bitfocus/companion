...
class SurfaceController extends CoreBase {
				...
				if (this.userconfig.getKey('link_lockouts')) {
					...
					for (const surface of this.#surfaceHandlers.values()) {
						if (this.#isSurfaceTimedOut(surface.deviceId, timeout)) {
							doLockout = true
							this.#surfacesLastInteraction.delete(surface.deviceId)
						}
					}
		...
		surfaceGroups.forEach((group, index) => {
			...
			const groupResult = {
				...
				surfaces: group.surfaceHandlers.map((handler) =>
					translateSurfaceConfig(handler.deviceId, handler.getFullConfig(), handler)
				),
			}
	...
	setAllLocked(locked, forceUnlock = false) {
		...
		for (const surface of this.#surfaceHandlers.values()) {
			this.#surfacesLastInteraction.set(surface.deviceId, Date.now())
	...
	#getGroupForId(surfaceOrGroupId, looseIdMatching = false) {
		...
		if (surface) {
			const groupId = surface.getGroupId() || surface.deviceId
	...
	#getSurfaceHandlerForId(surfaceId, looseIdMatching) {
		...
		let device = surfaces.find((d) => d.deviceId === surfaceId)
