...
class SurfaceController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('surfaces:set-name', (id, name) => {
			const group = this.#surfaceGroups[id]
	...
	getDevicesList() {
		...
		const surfaceGroups = Object.values(this.#surfaceGroups)
		...
		const definedGroupIds = new Set(result.map((group) => group.id))
		...
		for (const [surfaceId, surface] of Object.entries(config)) {
			...
			if (definedGroupIds.has(groupId)) continue

			result.push({
				id: groupId,
				displayName: `${surface.name || surface.type} (${surface.id}) - Offline`,
				isAutoGroup: true,
