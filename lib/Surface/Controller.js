...
class SurfaceController extends CoreBase {
			...
			this.#surfaceLockoutTimer = setInterval(() => {
				if (this.userconfig.getKey('link_lockouts')) {
					...
					for (const surface of this.#surfaceHandlers.values()) {
						if (this.#isSurfaceTimedOut(surface.surfaceId, timeout)) {
							doLockout = true
							this.#surfacesLastInteraction.delete(surface.surfaceId)
						}
					}
					...
					for (const surface of this.#surfaceHandlers.values()) {
						if (this.#isSurfaceTimedOut(surface.surfaceId, timeout)) {
							this.#surfacesLastInteraction.delete(surface.surfaceId)
							this.setDeviceLocked(surface.surfaceId, true)
						}
					}
				}
			}, 1000)
	...
	setAllLocked(locked, forceUnlock = false) {
		...
		for (const surface of this.#surfaceHandlers.values()) {
			this.#surfacesLastInteraction.set(surface.surfaceId, Date.now())
