				...
				await Promise.allSettled([
					HID.devices().map(async (deviceInfo) => {
						if (this.instances[deviceInfo.path] === undefined) {
							...
							} else if (deviceInfo.vendorId === 0x0b33 &&
								// TODO(Peter): Get the VID/PID from the upstream module
                                                                   (deviceInfo.productId === 0x0010 || deviceInfo.productId === 0x0020 || deviceInfo.productId === 0x0030)) {
								if (this.userconfig.getKey('shuttle_enable')) {
									await this.#addDevice(deviceInfo, 'shuttle')
								}
							}
						}
					}),
