...
class SurfaceController extends CoreBase {
			...
			this.#surfaceLockoutTimer = setInterval(() => {
				if (this.userconfig.getKey('link_lockouts')) {
					...
					for (const device of this.#surfaceHandlers.values()) {
						if (this.#isSurfaceTimedOut(device.surfaceId, timeout)) {
							doLockout = true
							this.#surfacesLastInteraction.delete(device.surfaceId)
						}
					}
					...
					for (const device of this.#surfaceHandlers.values()) {
						if (this.#isSurfaceTimedOut(device.surfaceId, timeout)) {
							this.#surfacesLastInteraction.delete(device.surfaceId)
							this.setDeviceLocked(device.surfaceId, true)
						}
					}
				}
			}, 1000)
	 ...
	 * @returns {boolean}
	 */
	#isSurfaceTimedOut(surfaceId, timeout) {
		if (!this.isPinLockEnabled()) return false

		const lastInteraction = this.#surfacesLastInteraction.get(surfaceId) || 0
	...
	#createSurfaceHandler(surfaceId, integrationType, panel) {
		const panelSurfaceId = panel.info.deviceId
		...
		if (this.isPinLockEnabled()) {
			...
			if (this.userconfig.getKey('link_lockouts')) {
				...
				isLocked = this.#isSurfaceTimedOut(panelSurfaceId, timeout)
			} else {
				isLocked = !this.#surfacesLastInteraction.has(panelSurfaceId)
			}
		}
		...
		handler.on('interaction', () => {
			this.#surfacesLastInteraction.set(panelSurfaceId, Date.now())
		})
	...
	getDevicesList() {
		...
		for (const instance of this.#surfaceHandlers.values()) {
			instanceMap.set(instance.surfaceId, instance)
		}
	 ...
	 * @returns {void}
	 */
	devicePageUp(surfaceId, looseIdMatching = false) {
	 ...
	 * @returns {void}
	 */
	devicePageDown(surfaceId, looseIdMatching = false) {
		const device = this.#getSurfaceHandlerForId(surfaceId, looseIdMatching)
		if (device) {
			device.doPageDown()
		}
	 ...
	 * @returns {void}
	 */
	devicePageSet(surfaceId, page, looseIdMatching = false, defer = false) {
		const device = this.#getSurfaceHandlerForId(surfaceId, looseIdMatching)
		if (device) {
	 ...
	 */
	devicePageGet(surfaceId, looseIdMatching = false) {
		const device = this.#getSurfaceHandlerForId(surfaceId, looseIdMatching)
		if (device) {
	...
	setAllLocked(locked, forceUnlock = false) {
		...
		for (const device of this.#surfaceHandlers.values()) {
			this.#surfacesLastInteraction.set(device.surfaceId, Date.now())
	 ...
	 * @returns {void}
	 */
	setDeviceLocked(surfaceId, locked, looseIdMatching = false) {
		if (!this.isPinLockEnabled()) return
		...
		if (this.userconfig.getKey('link_lockouts')) {
			...
			if (locked) {
				this.#surfacesLastInteraction.delete(surfaceId)
			} else {
				this.#surfacesLastInteraction.set(surfaceId, Date.now())
			}
	...
	#getSurfaceHandlerForId(surfaceId, looseIdMatching) {
		...
		let device = instances.find((d) => d.surfaceId === surfaceId)
		...
		device = instances.find((d) => d.surfaceId === surfaceId2)
		if (device) return device
		...
		return instances.find((d) => d.surfaceId === surfaceId2)
	}
}
