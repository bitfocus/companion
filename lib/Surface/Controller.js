...
class SurfaceController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('surfaces:add-to-group', (groupId, surfaceId) => {
			const group = groupId ? this.#surfaceGroups[groupId] : null
			if (groupId && !group) throw new Error(`Group does not exist: ${groupId}`)

			const surfaceHandler = Object.values(this.#surfaceHandlers).find((handler) => handler.deviceId === surfaceId)
	...
	#detachSurfaceFromGroup(surfaceHandler) {
		...
		const existingGroup = existingGroupId ? this.#surfaceGroups[existingGroupId] : null
		...
		if (existingGroup.isAutoGroup) {
			...
			delete this.#surfaceGroups[existingGroupId]
		}
	}
