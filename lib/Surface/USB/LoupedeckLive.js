...
import { oldBankIndexToXY, splitCoordinate } from '../../Shared/ControlId.js'
import { toGlobalKey } from '../../Resources/Util.js'
...
const loupedeckLiveInfo = {
	...
	encoders: ['0x0', '0x1', '0x2', '7x0', '7x1', '7x2'],
	buttons: ['0x3', '1x3', '2x3', '3x3', '4x3', '5x3', '6x3', '7x3'],
}
...
function buttonToCoordinate(modelInfo, info) {
...
function rotaryToCoordinate(modelInfo, info) {
...
class SurfaceUSBLoupedeckLive extends EventEmitter {
	constructor(devicepath, loupedeck, modelInfo, serialnumber) {
		...
		this.loupedeck.on('down', (info) => {
			if (this.modelInfo.lcdAsButtons) {
				...
				const coordinate = buttonToCoordinate(this.modelInfo, info) ?? rotaryToCoordinate(this.modelInfo, info)
				if (coordinate === undefined) {
					return
				}

				this.#emitClickCoordinate(coordinate, true)
			}
		})
		...
		this.loupedeck.on('up', (info) => {
			if (this.modelInfo.lcdAsButtons) {
				...
				this.#emitClickCoordinate(coordinate, false)
			}
		})
		this.loupedeck.on('rotate', (info, delta) => {
			const coordinate = rotaryToCoordinate(this.modelInfo, info)
			if (coordinate === undefined) {
				return
			}
	...
	#emitClickCoordinate(coordinate, state) {
		const xy = splitCoordinate(coordinate)
	...
	draw(x, y, buffer, style) {
		...
		const buttonIndex = this.modelInfo.buttons.indexOf(`${x}x${y}`)
		if (buttonIndex >= 0) {
