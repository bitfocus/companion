...
class SurfaceUSBShuttle {
	...
	static async create(ipcWrapper, devicepath) {
		...
		try {
			let deviceInfo = null
			let info = null
			console.log("Device count", shuttle.getDeviceList().length)
			if (shuttle.getDeviceList().length >= 1) {
				// TODO(Peter): Fix this to fetch the correct device
				deviceInfo = shuttle.getDeviceList()[0]
