...
class SurfaceUSBInfinitton {
	debug = createDebug('infinitton', 'lib/Surface/USB/Infinitton')
	...
	draw(key, buffer, style) {
		try {
			...
			this.ipcWrapper.debug('Infinitton error: ' + e.message)
			this.ipcWrapper.remove()
		}
	...
	clearDeck() {
		this.ipcWrapper.debug('infinitton.prototype.clearDeck()')
