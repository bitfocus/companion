...
const debug = createDebug('usb-controller', 'lib/Surface/USB/Controller')
...
class SurfaceUSBController extends EventEmitter {
	constructor(type, devicepath, cb) {
		...
		this.debug = createDebug('lib/Surface/USB/' + type)
		...
		child.on('message', (data) => {
			if (data.cmd == 'ready') {
				...
				this.emit('log', `device(${this.serialnumber || devicepath})`, data.level, data.message)
