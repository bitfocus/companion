...
import xkeys_settings from '../../Resources/XKeysProducts.js'
import HID from 'node-hid'
import debug0 from 'debug'

class SurfaceUSBXKeys extends SurfaceBase {
	debug = debug0('lib/Surface/USB/XKeys')
	...
	createXkeysDevice(devicepath) {
		...
		this.info.serialnumber = this.myXkeysPanel.deviceType.identifier
		...
		// Mapping buttons
		for (let leftRight = 0; leftRight < this.config.keysPerRow; leftRight++) {
			for (let topBottom = 0; topBottom < this.config.bits; topBottom++) {
			...
			for (const buttonStateKey in buttonStates) {
				...
				if ((this._buttonStates[buttonStateKey] || false) !== buttonStates[buttonStateKey]) {
					if (buttonStates[buttonStateKey]) {
						...
						this.system.emit('device_click', devicepath, newKey, true)
						...
						let newKey = this.setPageKey(key)
						this.system.emit('device_click', devicepath, newKey, false)
	...
	setPageKey(key) {
		if (key > 31) {
			...
			this.system.emit('device_page_set', this.info.serialnumber, pageNumber)
			return key
		} else {
			this.system.emit('device_page_set', this.info.serialnumber, this.config.page)
			return key
		}
	}
