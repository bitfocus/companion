...
const xkeys_settings = require('../../Resources/XKeysProducts')
const HID = require('node-hid')

class SurfaceUSBXKeys extends SurfaceBase {
	debug = require('debug')('lib/Surface/USB/XKeys')
			...
			for (const buttonStateKey in buttonStates) {
				...
				if ((this._buttonStates[buttonStateKey] || false) !== buttonStates[buttonStateKey]) {
					if (buttonStates[buttonStateKey]) {
						...
						this.system.emit('elgato_click', devicepath, newKey, true)
						...
						let newKey = this.setPageKey(key)
						this.system.emit('elgato_click', devicepath, newKey, false)
...
exports = module.exports = SurfaceUSBXKeys
