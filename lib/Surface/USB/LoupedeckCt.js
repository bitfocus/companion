...
class SurfaceUSBLoupedeckCt extends EventEmitter {
	constructor(devicepath, loupedeck, modelInfo, serialnumber) {
		...
		this.loupedeck.on('touchmove', async (data) => {
			let touch = data.changedTouches.find(touch => touch.target.screen == LoupedeckDisplayId.Right || touch.target.screen == LoupedeckDisplayId.Left)
			if (touch && touch.target.screen == LoupedeckDisplayId.Right) {
				...
				try {
					await this.loupedeck.drawSolidColour(LoupedeckDisplayId.Right, { red: 0, green: 0, blue: 0 }, 60, val + 7, 0, 0)
					await this.loupedeck.drawSolidColour(LoupedeckDisplayId.Right, { red: 0, green: 127, blue: 0 }, 60, 262 - val, 0, val + 7)
				} catch (err) {
				...
				try {
					await this.loupedeck.drawSolidColour(LoupedeckDisplayId.Left, { red: 0, green: 0, blue: 0 }, 60, val + 7, 0, 0)
					await this.loupedeck.drawSolidColour(LoupedeckDisplayId.Left, { red: 127, green: 0, blue: 0 }, 60, 262 - val, 0, val + 7)
				} catch (err) {
		...
		this.key_write_queue = new ImageWriteQueue(this.logger, async (key, buffer) => {
			...
			try {
				if (key !== 35) {
					...
					await this.loupedeck.drawBuffer(LoupedeckDisplayId.Wheel, newbuffer, LoupedeckBufferFormat.RGB, 240, 240, 0, 0)
				}
			} catch (e) {
