...
// Some streamdecks require libusb mode on linux
// Enable it here before HID has a chance to be loaded by anything else, and hope it doesnt cause issues for other device types
HID.setDriverType('libusb')

exports = module.exports = function (system) {
	return new Surface(system)
}

class Surface {
	debug = require('debug')('lib/Surface')
	...
	refreshDevices(cb) {
		...
		let streamdeckDisabled = !!this.config.elgato_plugin_enable
	...
	addDevice(device, type) {
		...
		if (type === 'elgatoEmulator') {
			this.instances[device.path] = new SurfaceHandler(
				this.system,
				new SurfaceIPElgatoEmulator(this.system, device.path)
			)

			this.updateDevicesList()
		} else if (type === 'streamdeck_plugin') {
			this.instances[device.path] = new SurfaceHandler(this.system, new SurfaceIPElgatoPlugin(this.system, device.path))

			this.updateDevicesList()
		} else if (type === 'satellite_device2') {
			this.instances[device.path] = new SurfaceHandler(this.system, new SurfaceIPSatellite(this.system, device))
			...
			new SurfaceUSBController(this.system, type, device.path, (dev) => {
				...
				this.instances[device.path] = new SurfaceHandler(this.system, dev)
