...
class SurfaceHandler extends CoreBase {
	...
	constructor(registry, panel) {
		...
		this.lockedout = this.userconf.pin_enable
		...
		this.timeouttimer = setInterval(() => {
			if (
				this.userconf.pin_timeout != 0 &&
				this.userconf.pin_enable == true &&
				Date.now() >= this.lockoutat &&
				!this.lockedout
			) {
				if (this.userconf.link_lockouts) {
			...
			if (this.lockedout && !this.userconf.pin_enable) {
	...
	onDevicePageDown(deviceid) {
		if (this.userconf.page_direction_flipped === true) {
	...
	onElgatoClick(devicepath, key, state) {
		...
		if (!this.lockedout) {
			this.lastinteraction = Date.now()
			this.lockoutat = this.userconf.pin_timeout * 1000 + Date.now()
			...
			this.system.emit(
				...
				'debug',
				'Button ' + this.currentPage + '.' + (key + 1) + ' ' + (state ? 'pressed' : 'released')
			)
		} else {
			if (state) {
				...
				if (this.currentpin == this.userconf.pin.toString()) {
					this.lockedout = false
					this.currentpin = ''
					this.lastinteraction = Date.now()
					this.lockoutat = this.userconf.pin_timeout * 1000 + Date.now()
				...
				} else if (this.currentpin.length >= this.userconf.pin.toString().length) {
					this.currentpin = ''
				}
			}
