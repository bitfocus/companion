...
class SurfaceHandler extends CoreBase {
	...
	constructor(registry, integrationType, panel, isLocked, surfaceConfig) {
		...
		this.#currentPage = 1 // The current page of the surface
		...
		setImmediate(() => {
			...
			this.surfaces.emit('surface_page', this.surfaceId, this.#currentPage)
	...
	doPageDown() {
		if (this.userconfig.getKey('page_direction_flipped') === true) {
			this.#surfaceIncreasePage()
		} else {
			this.#surfaceDecreasePage()
		}
	}
