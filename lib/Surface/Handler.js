...
class SurfaceHandler extends CoreBase {
	...
	#getCurrentOffset() {
		if (this.userconfig.getKey('experiment_enlarged_grid')) {
			return {
	...
	updateMaxOffset() {
		...
		if (gridSize.columns && gridSize.rows) {
			const maxRows = LEGACY_BUTTONS_PER_COLUMN
			this.panelInfo.xOffsetMax = Math.max(Math.floor(LEGACY_BUTTONS_PER_ROW - gridSize.columns), 0)
