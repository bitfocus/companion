...
import { toDeviceKey } from '../Resources/Util.js'
import { formatCoordinate, oldBankIndexToXY, splitCoordinate } from '../Shared/ControlId.js'
import { cloneDeep } from 'lodash-es'

const PINCODE_NUMBER_POSITIONS = [
	// 0
	formatCoordinate(4, 1),
	// 1 2 3
	formatCoordinate(1, 2),
	formatCoordinate(2, 2),
	formatCoordinate(3, 2),
	// 4 5 6
	formatCoordinate(1, 1),
	formatCoordinate(2, 1),
	formatCoordinate(3, 1),
	// 7 8 9
	formatCoordinate(1, 0),
	formatCoordinate(2, 0),
	formatCoordinate(3, 0),
]
const PINCODE_CODE_POSITION = formatCoordinate(0, 1)
...
class SurfaceHandler extends CoreBase {
	...
	drawPage() {
		if (this.panel) {
			if (this.isSurfaceLocked) {
				...
				const [x, y] = splitCoordinate(PINCODE_CODE_POSITION)
				this.panel.draw(x, y, buffers.code.buffer)

				PINCODE_NUMBER_POSITIONS.forEach((coordinate, i) => {
					if (buffers[i]) {
				...
				for (let y = 0; y < rows; y++) {
					for (let x = 0; x < this.panel.info.keysPerRow; x++) {
						const coordinate = formatCoordinate(x + xOffset, y + yOffset)
						const image = this.graphics.getBank(this.currentPage, coordinate)
	...
	onBankInvalidated(page, coordinate, render) {
		...
		if (this.#xkeysPageCount > 0) {
			...
			if (pageOffset >= 0 && pageOffset < this.#xkeysPageCount) {
				const [x, y] = splitCoordinate(coordinate)
				this.panel.drawColor(pageOffset, x, y, render.style?.bgcolor || 0)
			}
			...
			const [x, y] = splitCoordinate(coordinate)
			this.panel.draw(x - xOffset, y - yOffset, render.buffer, render.style)
		}
	}
	...
	#onDeviceClick(x, y, pressed, pageOffset) {
		if (this.panel) {
			if (!this.isSurfaceLocked) {
				...
				const controlId = this.page.getControlIdAt(thisPage, coordinate)
				...
				if (pressed) {
					const pressCode = PINCODE_NUMBER_POSITIONS.indexOf(formatCoordinate(x, y))
					if (pressCode !== -1) {
				...
				if (this.isSurfaceLocked) {
					...
					const [x, y] = splitCoordinate(PINCODE_CODE_POSITION)
					this.panel.draw(x, y, datap.code.buffer)
				}
			}
		}
	}
	...
	#onDeviceRotate(x, y, direction, pageOffset) {
		if (this.panel) {
			if (!this.isSurfaceLocked) {
				...
				this.logger.debug(`Rotary ${thisPage}.${coordinate} rotated ${direction ? 'right' : 'left'}`)
			} else {
	...
	#xkeysDrawPages() {
		for (let page = 0; page < this.#xkeysPageCount; page++) {
			for (let bank = 0; bank < global.MAX_BUTTONS; bank++) {
				...
				if (xy) {
					const render = this.graphics.getBank(this.currentPage + page, formatCoordinate(...xy))
