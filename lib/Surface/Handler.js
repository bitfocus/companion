	...
	drawPage() {
		if (this.panel) {
			if (this.isSurfaceLocked) {
				...
				this.panel.draw(PINCODE_CODE_POSITION[0], PINCODE_CODE_POSITION[1], buffers.code.buffer)

				PINCODE_NUMBER_POSITIONS.forEach(([x, y], i) => {
					if (buffers[i]) {
	...
	#onDeviceClick(x, y, pressed, pageOffset) {
		if (this.panel) {
			if (!this.isSurfaceLocked) {
				...
				if (pressed) {
					const pressCode = PINCODE_NUMBER_POSITIONS.findIndex((pos) => pos[0] == x && pos[1] == y)
					if (pressCode !== -1) {
				...
				if (this.isSurfaceLocked) {
					...
					this.panel.draw(PINCODE_CODE_POSITION[0], PINCODE_CODE_POSITION[1], datap.code.buffer)
				}
			}
		}
	}
