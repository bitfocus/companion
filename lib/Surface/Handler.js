...
class SurfaceHandler extends CoreBase {
	debug = createDebug('lib/Surface/Handler')
	...
	constructor(registry, panel) {
		...
		this.panelconfig = this.db.getKey('deviceconfig', {})[this.deviceId]
		if (!this.panelconfig) {
			this.panelconfig = {}
			this.debug(`Creating config for newly discovered device ${this.deviceId}`)
		} else {
			this.debug(`Reusing config for device ${this.deviceId} was on page ${this.currentPage}`)
		}
	...
	onDeviceClick(key, pressed) {
		if (!this.isSurfaceLocked) {
			...
			this.bank.action.pressBank(thisPage, key + 1, pressed, this.deviceId)
			this.log('debug', 'Button ' + thisPage + '.' + (key + 1) + ' ' + (pressed ? 'pressed' : 'released'))
		} else {
			if (pressed) {
	...
	unload() {
		...
		this.debug('unloading for ' + this.panel.info.devicepath)
