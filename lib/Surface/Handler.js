...
class SurfaceHandler extends CoreBase {
	...
	drawPage() {
		if (this.panel) {
			if (this.isSurfaceLocked) {
				...
				this.#drawButtonAtKey(PINCODE_CODE_POSITION, buffers.code.buffer)

				PINCODE_NUMBER_POSITIONS.forEach((key, i) => {
					if (buffers[i]) {
						this.#drawButtonAtKey(key, buffers[i].buffer)
					}
				})
				...
				for (let y = 0; y < rows; y++) {
					for (let x = 0; x < this.panel.info.keysPerRow; x++) {
						...
						this.#drawButtonAtXY(x, y, image.buffer, image.style)
					}
				}
			}
		}
	...
	#drawButtonAtKey(key, buffer, style) {
	...
	#drawButtonAtXY(x, y, buffer, style) {
		if (x < 0 || x >= this.panel.info.keysPerRow || y < 0) return

		const key = y * this.panel.info.keysPerRow + x
		if (key >= 0 && key <= this.panel.info.keysTotal) {
			this.panel.draw(key, buffer, style)
		}
	}
	...
	onBankInvalidated(page, coordinate, render) {
		...
		if (this.#xkeysPageCount > 0) {
			...
			const [x, y] = splitCoordinate(coordinate)
			this.#drawButtonAtXY(x - xOffset, y - yOffset, render.buffer, render.style)
		}
	}
		...
		if (this.panel) {
			if (!this.isSurfaceLocked) {
				...
				if (this.isSurfaceLocked) {
					...
					this.#drawButtonAtKey(PINCODE_CODE_POSITION, datap.code.buffer)
				}
			}
		}
	}
