...
class BankFeedback extends CoreBase {
	...
	clientConnect(client) {
		...
		client.on('bank_get_feedbacks', (page, bank, answer) => {
			...
			answer(page, bank, this.feedbacks[page][bank])
		})
	}
	...
	addItem(client, page, bank, feedback, answer) {
		...
		const definition = this.instance.definitions.getFeedbackDefinition(s[0], s[1])
		if (definition) {
		...
		if (fb.instance_id === 'internal') {
			this.internalModule.feedbackUpdate(fb, CreateBankControlId(page, bank), page, bank)
		} else {
			const instance = this.instance.moduleHost.getChild(fb.instance_id)
			if (instance) {
				instance.feedbackUpdate(fb, CreateBankControlId(page, bank), page, bank).catch((e) => {
	...
	changeItemOpton(page, bank, feedbackId, option, value) {
		this.logger.silly('bank_update_feedback_option', page, bank, feedbackId, option, value)
			...
			for (const n in feedbacks) {
				...
				if (feedback !== undefined && feedback.id === feedbackId) {
					...
					if (feedback.instance_id === 'internal') {
						...
						if (instance) {
							instance.feedbackUpdate(feedback, CreateBankControlId(page, bank), page, bank).catch((e) => {
								this.logger.silly(`feedback_update to connection failed: ${e.message}`)
							})
						}
					}
	...
	changeStyleSelection(page, bank, feedbackId, selected, answer) {
		this.logger.silly('bank_update_feedback_style_selection', page, bank, feedbackId, selected)
		let feedbacks = this.feedbacks[page][bank]
		if (feedbacks !== undefined) {
			...
			for (const n in feedbacks) {
				...
				if (feedback !== undefined && feedback.id === feedbackId) {
					...
					const feedbackSpec = this.instance.definitions.getFeedbackDefinition(feedback.instance_id, feedback.type)
	...
	setItemStyle(page, bank, feedbackId, key, value, answer) {
		this.logger.silly('bank_update_feedback_style_set', page, bank, feedbackId, key, value)
		let feedbacks = this.feedbacks[page][bank]
		if (feedbacks !== undefined) {
			if (key === 'png64') {
