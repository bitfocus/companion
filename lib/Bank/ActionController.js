...
var debug          = require('debug')('lib/Bank/ActionController');
var CoreBase       = require('../Core/Base');
var BankActionItem = require('./ActionItem');
...
class BankActionController extends CoreBase {
	...
	constructor(registry, bank) {
		...
		this.bank           = bank;
		this.actions        = new BankActionItem(registry, 'action',         'bank_actions');
		this.releaseActions = new BankActionItem(registry, 'release_action', 'bank_release_actions');
	...
	abortDelayedActions() {
		...
		var actionsRunning = this.actionsRunning.slice(0); //clone hack
		this.actionsRunning = []; // clear the array

		for (var bid in actionsRunning) {
			var a = actionsRunning[bid].split("_");
			this.system.emit('graphics_bank_invalidate', a[0], a[1]);
		}
	}

	bankPressed(page, bank, direction, deviceid) {
		var bankConfig = this.bank.getBank(page, bank);
		...
		for (var n in obj) {
			var a = obj[n];
			var thisDelay = parseInt(a.delay === undefined || a.delay === '' ? 0 : a.delay);
		...
		for (var n in obj) {
			var a = obj[n];
			var delay = parseInt(a.effectiveDelay === undefined ? 0 : a.effectiveDelay);
				...
				if (delay > 0) {
					...
					(function(action, delayTime, n) {
						var timer = setTimeout(function() {
							...
							var idx = this.timersRunning.indexOf(timer);
							if (idx !== -1) {
	...
	checkBankStatus(page, bank, invalidate = true) {
		...
		status = this.actions.getBankStatus(page, bank, status);
		status = this.releaseActions.getBankStatus(page, bank, status);
	...
	deleteInstance(id) {
		...
		checkQueue = this.releaseActions.deleteInstance(is, checkQueue);

		this.checkBanks(checkQueue);
	}

	getBankActions(clone = false) {
		this.actions.getAll(clone);
	}

	getBankReleaseActions(clone = false) {
	...
	getRunningActions(page, bank) {
		this.actionsRunning.indexOf(page + '_' + bank) !== -1;
	}
	...
	runAction(action, extras) {
		...
		if (action !== undefiend && action.instance !== undefined && this.definitions[action.instance]) {
			var instance = this.instance().getInstance(action.instance); //returns { config: {}, instance: Object }
