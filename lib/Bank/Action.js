...
class BankAction extends CoreBase {
	constructor(registry) {
		...
		this.actions = {}
		this.bank_action_sets = {} // [page][bank][set]. when style is step, set is a number. when style is press, set is 'down' or 'up'
		...
		this.system.emit('db_get', 'bank_action_sets', (res) => {
			if (res !== undefined) {
				this.bank_action_sets = res
			}
		})
		...
		this.system.on('action_setup_bank', (page, bank, style) => {
			...
			this.io2.emit('bank_action_sets_list', page, bank, Object.keys(this.bank_action_sets[page][bank]))
		})
		...
		this.system.on('instance', (obj) => {
			...
			this.system.emit('db_set', 'bank_action_sets', this.bank_action_sets)
		...
		this.system.on('instance_save', () => {
			setImmediate(() => {
				this.io2.emit('actions', this.actions)
			})
		})
		...
		this.system.on('bank_pressed', (page, bank, direction, deviceid) => {
			...
			system.emit('get_bank', page, bank, (config) => {
				bank_config = config
			})
	...
	bank_set_step_change(page, bank, value) {
		...
		// notify ui
		this.io2.emit('bank_action_sets_step', page, bank, value)
	}
	...
	bank_set_swap(page, bank, id1, id2) {
		...
		this.io2.emit('bank_action_sets_reload', page, bank, [id1, id2])
	}
