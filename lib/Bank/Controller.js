...
class BankController extends CoreBase {
	...
	constructor(registry) {
		...
		this.actions  = new BankActionController(registry, this);
		this.feedback = new BankFeedbackController(registry, this);
	...
	checkVersion() {
		...
		} else if (res > BankController.CurrentVersion) {
	...
	upgrade15to32() {
		...
		old_config = cloneDeep(this.config);
		this.config = this.db.getKey('bank', {});

		old_bank_actions = cloneDeep(this.actions.getBankActions());
		old_bank_release_actions = cloneDeep(this.actions.getBankReleaseActions());
