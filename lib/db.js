...
function db(system,cfgDir) {
	...
	system.on('db_save', function (callback) {

		if (callback !== undefined) {
			save_callbacks.push(callback);
		}

		if (save_callbacks.length || Date.now() - lastsave > saveInterval) {

			if (save_working) {
				debug("db_save","working, skipping turn");
				return;
			}
			...
			fs.copy(cfgDir + '/db', cfgDir + '/db.bak', function (err) {
				...
				fs.writeFile(cfgDir + '/db.tmp', JSON.stringify(self.db), function (err) {
					...
					debug("db_save","written(callback):",  callback);

					fs.rename(cfgDir + '/db.tmp', cfgDir + '/db', function(err) {

						save_working = false;
						debug("db_save","renamed(callback):", callback);

						if (err) {
							debug("db_save","db save erroroooorororoor(callback):", callback);
							self.system.emit('log', 'CORE(cb)', 'error', 'db.tmp->db failed: ' + err);
						}
						else {

							debug("db_save","db saved(callback):", callback);
							...
							if (dirty) {
								...
								debug("db_save", "running callbacks!");
								for (var c in save_callbacks) {
									var callback = save_callbacks[c];
									callback();
								}
								save_callbacks = [];
							}
