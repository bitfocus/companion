...
class xkeys {
	constructor(system, devicepath) {
		...
		// this.info.serialnumber = '' // This gets set later
		this.info.keysPerRow = 10
		this.info.keysPerColumn = 8
		...
		this.system.on('graphics_set_bank_bg', (page, bank, bgcolor) => {
			...
			let buttonNumber = (parseInt(page) - 1) * 32 + parseInt(bank)
		...
		setupXkeysPanel(devicepath)
			.then((xkeysPanel) => {
				...
				// Mapping buttons
				for (var leftRight = 1; leftRight < this.info.keysPerColumn + 2; leftRight++) {
					for (var topBottom = 0; topBottom < this.info.keysPerRow - 2; topBottom++) {
				...
				this.info.serialnumber = this.serialnumber = this.myXkeysPanel.product.productId
				this.finish_add()
				...
				this.myXkeysPanel.on('down', (keyIndex, metadata) => {
					...
					console.log('companion press', key)
	...
	setPageKey(key) {
		...
		console.log('pageNumber', key, pageNumber, this.serialnumber)
	...
	setConfig(config) {
		if (this.config.brightness != config.brightness && config.brightness !== undefined) {
			this.myXkeysPanel.setBacklightIntensity(config.brightness)
		} else if (config.brightness === undefined) {
			this.myXkeysPanel.setBacklightIntensity(10)
		}
