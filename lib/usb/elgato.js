...
elgato.prototype.draw = function (key, buffer, style, attempts) {
	var self = this
	...
	if (drawKey !== undefined && drawKey >= 0 && drawKey < self.info.keysTotal) {
		self.streamDeck.fillKeyBuffer(drawKey, buffer).catch(function (e) {
			...
			setTimeout(self.draw.bind(self), 20, key, buffer, style, attempts)
