...
var debug = require('debug')('lib/usb/loupedeck-live-s')
...
class LoupedeckLiveS {
	constructor(system, devicepath) {
		util.inherits(LoupedeckLiveS, common)
		...
		self.info.keysPerRow = 7
		self.info.keysTotal = 7 * 3

		self.config = {
			brightness: 100,
		}

		debug('Adding Loupedeck Live S USB device', devicepath)
		...
		openLoupedeck(devicepath, { waitForAcks: true })
			.then(async (loupedeck) => {
				...
				self.system.emit('log', `device(${self.serialnumber})`, 'debug', `Loupedeck Live S detected`)
...
exports = module.exports = LoupedeckLiveS
