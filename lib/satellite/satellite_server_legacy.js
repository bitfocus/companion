...
function satelliteServerLegacy(system) {
	var self = this
	...
	system.emit('elgatodm_get', function(_elgatoDM) {
		self.elgatoDM = _elgatoDM
	})
...
satelliteServerLegacy.prototype.initSocket = function (socket) {
	var self = this
	...
	socket.on('close', function () {
		for (var key in self.devices) {
			if (self.devices[key].socket === socket) {
				this.elgatoDM.removeDevice(self.devices[key].id)
...
satelliteServerLegacy.prototype.findPackets = function (socket) {
	var self = this
...
satelliteServerLegacy.prototype.parsePacket = function (socket, header) {
	var self = this
...
satelliteServerLegacy.prototype.handleVersion = function (packet, socket) {
	var self = this
...
satelliteServerLegacy.prototype.handleButton = function (packet, socket) {
	var self = this
...
satelliteServerLegacy.prototype.findNewId = function () {
	var self = this
	var i = 1
...
satelliteServerLegacy.prototype.removeDevice = function (packet, socket) {
	var self = this
	...
	if (self.devices[data.deviceId] !== undefined) {
		...
		this.elgatoDM.removeDevice(self.devices[data.deviceId].id)
...
satelliteServerLegacy.prototype.newDevice = function (packet, socket) {
	var self = this
...
satelliteServerLegacy.prototype.newDevice2 = function (packet, socket) {
	var self = this
...
satelliteServerLegacy.prototype.newDevice_inner = function (data, socket) {
	var self = this
