...
satellite_device.prototype.draw = function (key, buffer, style) {
	var self = this
	...
	if (self.socket !== undefined) {
		...
		if (self.streamBitmaps) {
			if (buffer === undefined || buffer.length != 15552) {
				...
				cmd += ` BITMAP=${self.handleBuffer(buffer).toString('base64')}`
			}
		}

		self.socket.write(`KEY-STATE DEVICEID=${self.deviceId} KEY=${keyIndex2} ${params}\n`)
	}
