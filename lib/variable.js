...
function variable(system) {
	...
	system.emit('io_get', function (_io) {
		...
		io.on('connect', function (socket) {
			socket.on('variable_instance_definitions_get', function () {
				socket.emit('variable_instance_definitions_get:result', null, self.variable_definitions);
			});

			socket.on('variables_get', function () {
				var vars = {};
	...
	// Everybody stand back. I know regular expressions. - xckd #208
