...
function variable(system) {
	...
	system.on('parse_variable_expression', function (expression, cb) {
		try {
			const result = self.parse_variable_expression(expression)
			cb(result)
		} catch (e) {
			cb(Number.NaN)
		}
	...
	system.on('custom_variable_set_expression', function (name, expression) {
		if (self.custom_variables[name]) {
			try {
				const value = self.parse_variable_expression(expression)
...
variable.prototype.parse_variable_expression = function (expression) {
	var self = this
	...
	const values = temp
		...
		.reduce((previous, { name }) => {
			...
			let value
			self.system.emit('variable_get', label, variable, function (store) {
				value = store
			})
			return { ...previous, [name]: value }
		}, {})

	const result = resolve(temp, values)
