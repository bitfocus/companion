...
function variable(system) {
	...
	system.on('custom_variable_set_value', self.custom_variable_set_value.bind(self))

	system.on('custom_variable_set_expression', self.custom_variable_set_expression.bind(self))

	system.on('custom_variable_store_variable', self.custom_variable_store_variable.bind(self))

	// extract value from the stored json response data, assign to target variable
	system.on('custom_variable_set_via_jsonpath', self.custom_variable_set_via_jsonpath.bind(self))
	...
	system.on('variable_get', function (label, variable, cb) {
		self.variable_get(label, variable, cb)
	})
...
variable.prototype.variable_get = function (label, variable, cb) {
	var self = this	
	if (self.variables[label] !== undefined) {
...
variable.prototype.getCustomVariable = function (name, cb) {
	var self = this
	self.variable_get('internal', `custom_${name}`, cb)
}

variable.prototype.custom_variable_set_value = function (name, value) {
	var self = this
...
variable.prototype.custom_variable_set_expression = function (name, expression) {
	var self = this

	if (self.custom_variables[name]) {
		const fullname = `${custom_variable_prefix}${name}`
		const variablePattern = /^\$\(((?:[^:$)]+):(?:[^)$]+))\)/
		const justname = name	// name is overwritten somehow below
		...
		try {
			...
			const values = temp
				...
				.reduce((previous, { name }) => {
					let value
					self.getCustomVariable(justname, function (store) {
						value = store
					})
...
variable.prototype.custom_variable_store_variable = function (name, variable) {
	var self = this

	let value = ''
	const id = variable.split(':')
	self.variable_get(id[0], id[1], (v) => (value = v))
	self.custom_variable_set_value(name, value)
}

variable.prototype.custom_variable_set_via_jsonpath = function (name, jsonResultDataVariable, jsonPath) {
	var self = this

	// get the json response data from the custom variable that holds the data
	let jsonResultData = ''
	self.getCustomVariable (jsonResultDataVariable, (value) => {
	...
	try {
		...
		self.system.emit('log', 'custom_variable', 'error', `${id.toUpperCase()} Cannot create JSON object, malformed JSON data (${e.message})`)
		return
	}
	...
	let valueToSet = ''
	try {
		valueToSet = jp.query(objJson, jsonPath)
	} catch (error) {
		self.system.emit('log', 'custom_variable', 'error', `${id.toUpperCase()} Cannot extract JSON value (${e.message})`)
		return
	}

	self.custom_variable_set_value(name, valueToSet)
	return
}
// END REFACTORING ------------------------------------------------------------
