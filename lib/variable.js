...
function parseVariablesInString(string, variableValues) {
	if (string === undefined) {
		return undefined
	}
	...
	while ((matches = reg.exec(string))) {
		...
		if (variableValues[matches[1]] !== undefined) {
			// console.log('replacing ', matches)

			if (variableValues[matches[1]][matches[2]] !== undefined) {
				string = string.replace(matches[0], variableValues[matches[1]][matches[2]])
			} else {
