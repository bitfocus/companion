...
function action(system) {
		...
		for (var page = 1; page <= 99; page++) {
			...
			for (var bank = 1; bank <= global.MAX_BUTTONS; ++bank) {
				if (self.bank_action_sets[page][bank] === undefined) {
					...
					console.log('do', page, bank, page_actions[bank], page_release_actions[bank])
				...
				if (actions_sets[page][bank] !== undefined) {
					for (var set in actions_sets[page][bank]) {
						if (actions_sets[page][bank][set] !== undefined) {
							res[page][bank][set] = []
							for (var i = 0; i < actions_sets[page][bank][set].length; ++i) {
								const action = actions_sets[page][bank][set][i]
	...
	self.system.on('action_setup_bank', function (page, bank, style) {
		...
		self.io.emit('bank_action_sets_step', page, bank, self.bank_cycle_step[page][bank])
	...
	if (bank_config && bank_config.style === 'step') {
		...
		if (oldKeys.length > 1) {
			...
			if (newIndex !== oldIndex) {
				...
				self.io.emit('bank_action_sets_step', page, bank, oldKeys[newIndex])
			}
		}
	}
...
action.prototype.bank_set_next_step = function (page, bank, set) {
	...
	if (self.bank_action_sets[page][bank][set]) {
		...
		if (bank_config && bank_config.style === 'step') {
			...
			self.io.emit('bank_action_sets_step', page, bank, set)
		}
	}
}
