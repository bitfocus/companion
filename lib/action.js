...
function action(system) {
	...
	setInterval(function() {
		if (self.bank_actions['1'] !== undefined && self.bank_actions['1']['1'] !== undefined) {
			console.log("bankactions", self.bank_actions['1']['1']);
		}
	}, 5000)
	...
	self.system.emit('io_get', function(io) {
		...
		self.io.on('connect', function(client) {
			client.on('get_actions', function() {
				...
				debug('get_actions',self.actions);
			});
				...
				if (bp !== undefined) {
					for (var n in bp) {
						...
						if (obj !== undefined && obj.id === action) {
							...
							console.log("LOL", option,value,"LOL")
							self.bank_actions[page][bank][n].options[option] = value;
							console.log("aftermath", self.bank_actions[page][bank]);
	...
	self.system.on('instance_actions', function(id, actions) {
		...
		debug('instance_actions', id, actions, self.actions);
		self.io.emit('actions', self.actions);
	});
