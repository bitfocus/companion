...
function action(system) {
	...
	function checkBank(page, bank) {
		...
		console.log("meh", self.bank_release_actions);

		// TODO: håkon, the status= thingy here will probably be a bug.. how to solve? what does this do?
/*
		for (var i = 0; i < self.bank_release_actions[page][bank].length ; ++i) {
			var action = self.bank_release_actions[page][bank][i];
			system.emit('instance_status_get', action.instance, function (instance_status) {
...
*/
	...
	self.system.on('bank-pressed', function(page, bank, direction) {
		system.emit('graphics_indicate_push', page, bank, direction);
