...
function action(system) {
	...
	self.system.on('instance_delete', function (id) {
		...
		self.io.emit('action_instance_definitions_set', id, undefined)
	})
			...
			self.system.emit('instance_get', action.instance, function (instance) {
				if (
					...
					try {
						...
						// Shallow clone the action
						action = { ...action, options: { ...action.options } }
						// Do some parameter processing
						if (definition !== undefined && definition.options !== undefined) {
							for (const option of definition.options) {
								const currentValue = action.options[option.id]
								if (option.type == 'number' && option.allowExpression && currentValue !== undefined) {
									// If its not a number, it must be an expression
									if (typeof currentValue !== 'number') {
										self.system.emit('parse_variable_expression', currentValue, (newVal) => {
											action.options[option.id] = newVal
										})
									}
								}
							}
	...
	self.system.on('instance_actions', function (id, actions) {
		...
		self.io.emit('action_instance_definitions_set', id, actions)
	})
