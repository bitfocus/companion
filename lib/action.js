...
function action(system) {
	...
	self.system.on('action_get_bank_active_step', function (page, bank, cb) {
		let out = 0

		if (self.bank_cycle_step[page] !== undefined && self.bank_cycle_step[page][bank] !== undefined) {
			out = self.bank_cycle_step[page][bank]
		}
	...
	self.system.on('bank_pressed', function (page, bank, direction, deviceid) {
		...
		system.emit('action_run_multiple', actions, bankId, bank_config.relative_delay, {
			deviceid: deviceid,
			page: page,
			bank: bank,
		})
