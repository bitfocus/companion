...
function action(system) {
	...
	// Dette trenger vi ikke lenger da tydeligvis.. (bare litt cleanup-testing)
	self.system.on('action_delayed_timer_cleanup', function() {
		console.log("action_delayed_timer_cleanup(): beginning - ");

		for (var i = 0; i < self.timers_running.length; ++i) {
			if (!self.timers_running[i]) {
				self.timers_running.splice(i, 1);
				debug("Removing timer index", i, self.timers_running[i]);
				i--;
			}
			else {
				debug("Not removing timer index", i, self.timers_running[i]);
			}
		}
			...
			if (self.instance !== undefined && self.instance.store !== undefined && self.instance.store.db !== undefined) {
				if (self.instance.store.db[a.instance] !== undefined && self.instance.store.db[a.instance].enabled !== false) {
					...
					if (delay > 0) {
						...
						var timer = (function(action, delay_time, n) {
							...
							return setTimeout(function() {
								...
								self.system.emit('action_delayed_timer_cleanup');
