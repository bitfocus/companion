...
function action(system) {
	...
	self.system.on('action_delayed_abort', function() {
		...
		for (var bid in actions_running) {
			...
			self.system.emit('graphics_invalidate_bank', a[0], a[1]);
		}
	...
	self.system.on('bank-pressed', function(page, bank, direction, deviceid) {
		...
		// Loop through each action, and if we find 'delay_actions', increase each subsequent
		//  actions' delay.
		var add_delay = 0;
		for (var n in obj[page][bank]) {
			var a = obj[page][bank][n];

			if(a.instance === 'bitfocus-companion' && a.action === 'delay_actions') {
				// Set/increase the delay for all subsequent actions
				add_delay += parseInt(a.delay);
			} else {
				// Increase the delay.
				// We create the property .effective_delay se we don't change the user's
				//  .delay properly.  We remove it below.
				a.effective_delay = parseInt(a.delay === undefined || a.delay === '' ? 0 : a.delay) + add_delay;
			}
		...
		for (var n in obj[page][bank]) {
			...
			if(a.instance === 'bitfocus-companion' && a.action === 'delay_actions') {
				// Skip this action. Its purpose is only to add a delay for subsequent actions.
				continue;
			}
	...
	self.system.emit('io_get', function(io) {
		...
		self.io.on('connect', function(client) {
			...
			client.on('bank_update_release_action_option_order', function(page, bank, old_index, new_index) {
				var bp = self.bank_release_actions[page][bank];
				if (bp !== undefined) {
					bp.splice(new_index, 0, bp.splice(old_index, 1)[0]);
					self.system.emit('release_action_save');
