	...
	self.system.emit('io_get', function(io) {
		...
		self.system.on('io_connect', function(client) {
			...
			client.on('bank_action_add', function(page,bank,action) {
				...
				if (self.actions[s[0]]+':'+[s[1]] !== undefined) {
					var definition = self.actions[s[0]]+':'+[s[1]];

					if (definition.options !== undefined && definition.options.length > 0) {
						for(var j in definition.options) {
							var opt = definition.options[j];
							action.options[opt.id] = opt.default;
						}
					}
				}

				self.bank_actions[page][bank].push(action);
			...
			client.on('bank_addReleaseAction', function(page,bank,action) {
				...
				self.bank_release_actions[page][bank].push(action);
