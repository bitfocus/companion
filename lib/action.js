...
function action(system) {
	...
	self.system.on('action_abort_one', function(bank) {

		var bid = bank[0] + '_' + bank[1];
		var b;
		var c = 0;

		while ((b = self.actions_running.indexOf(bid)) >= 0) {
			if (c == 0) {
				debug("Aborting button ",bank[0],",",bank[1]);
			}
			self.actions_running.splice(b,1);
			clearTimeout(self.timers_running[b]);
			self.timers_running.splice(b,1);
			c += 1;
		}

		if (c > 0) {
			self.system.emit('graphics_bank_invalidate', bank[0], bank[1]);
		}
	});
