...
function action(system) {
	...
	self.system.on('bank_pressed', function(page, bank, direction, deviceid) {
		...
		// magic keys only respond to push so ignore release
		// prevent 'pushed' indication since 'unpush'
		// will re-draw the button on the wrong page
		if (pageStyles.includes(bank_config.style)) {
			...
			// no actions on page buttons so we're done
			return;
