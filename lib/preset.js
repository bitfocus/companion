...
function preset(system) {
	...
	self.system.on('instance_enable', function (id, state) {
		if (state === false) {
			...
			self.io.emit('instance_presets_patch', id, null)
		}
	})
	...
	self.system.on('preset_instance_definitions_set', function (instance, presets) {
		const presetsBefore = self.presets[instance.id]
		self.presets[instance.id] = cloneDeep(presets)

		// TODO: This won't diff well, because the lib doesnt like arrays. I don't think we have a stable id to use to help it out
		const patch = jsonPatch.compare(presetsBefore || [], presets || [])
		if (patch.length > 0) {
			self.io.emit('instance_presets_patch', instance.id, patch)
		}
	})
