...
function instance(system) {
	...
	self.system.on('http_req', function (req, res, done) {
		...
		if (match = req.url.match(/^\/help\/([^/]+)\/(.+?)(\?.+)?$/)) {
			var path = require('app-root-path') + '/node_modules/companion-module-';
	...
	system.on('instance_init', function() {
		...
		var path = require('app-root-path') + '/node_modules';
		...
		for (var i = 0; i < module_folders.length; ++i) {
			...
			if (folder.match(/companion-module-/)) {
				...
				try {
					...
					var packagefile = fs.readFileSync(path +'/companion-module-'+ folder + '/package.json');
					var moduleconfig = JSON.parse(packagefile);
					var main = path + '/companion-module-' + folder + '/' + moduleconfig.main;
					if (fs.existsSync(path +'/companion-module-'+ folder + '/HELP.md')) {
					...
					if (process.env.LOAD_ALL_MODULES !== undefined) {
						var mod = require(path + '/companion-module-' + folder + '/' + moduleconfig.main);
...
instance.prototype.connect = function (client) {
	...
	client.on('instance_get_help_md', function (module, answer) {
		if (self.modules_main[module] !== undefined) {
			const basePath = require('app-root-path') + '/node_modules/companion-module-';
