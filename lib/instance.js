...
function instance(system) {
	...
	system.on('instance_init', function() {
		...
		var path = require('app-root-path') + '/lib/instance';
		...
		for (var i = 0; i < module_folders.length; ++i) {
			...
			try {
				...
				system.emit('skeleton-log', 'Unable to load module ' + folder);
			}
		}
		...
		for (var id in self.store.db) {
			(function (id) {
				...
				if (self.modules[config.instance_type] !== undefined) {
					...
					system.emit('skeleton-log', "Configured instance " + config.instance_type + " could not be loaded, unknown module");
				}
			})(id);
		}
