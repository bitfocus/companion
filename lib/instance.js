	...
	system.on('instance_init', function() {
		...
		for (var i = 0; i < module_folders.length; ++i) {
			...
			try {
				...
				system.emit('log', 'loaded', 'info', folder+'@'+moduleconfig.version +": " + moduleconfig.label + ' by ' + moduleconfig.author);
	...
	system.on('instance_config_put', function (id, config) {
		...
		if (self.active[id] !== undefined) {
			if (self.active[id].label != self.store.db[id].label) {
				system.emit('variable_instance_label_rename', self.active[id].label, self.store.db[id].label);
			}
