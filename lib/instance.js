	...
	system.on('instance_init', function () {
		...
		for (var id in self.store.db) {
			...
			if (self.checkModuleLoaded(config.instance_type)) {
				...
				self.active[id].upgradeConfig(mod.GetUpgradeScripts)
				if (mod.GetUpgradeScripts !== undefined) {
					// Module isn't using new flow of upgrade scripts, so make sure it isnt using the old flow
					self.active[id].disableInstanceUpgradeScripts()
				}
	...
	system.on(
		...
		function (instance_id, instance_created, instance_type, imported_config, actions, feedbacks) {
			if (self.checkModuleLoaded(instance_type)) {
				...
				if (mod.GetUpgradeScripts) {
					const upgrade_scripts = mod.GetUpgradeScripts()

					// config should only be upgraded if the instance was created from the import
					const upgrade_config = instance_created ? instance_config : null
					const instance_config = self.store.db[instance_id]

					// If the instance was just created, we want to run everything. If it already exists, we want to match the version in the stored config
					const targetIdx = instance_created
						? upgrade_scripts.length
						: Math.max(instance_config._configIdx + 1, upgrade_scripts.length)
					const fromIdx = imported_config._configIdx ?? -1

					if (idx + 1 < upgrade_scripts.length) {
						debug('upgradeImported(' + instance_type + '): ' + (fromIdx + 1) + ' to ' + targetIdx)
					}

					let changed = false
					for (var i = idx + 1; i < targetIdx; ++i) {
						debug('UpgradeConfig: Upgrading to version ' + (i + 1))

						try {
							const result = upgrade_scripts[i](upgrade_config, actions, release_actions, feedbacks)
							changed = changed || result
						} catch (e) {
							debug('Upgradescript in ' + instance_type + ' failed', e)
						}
						instance_config._configIdx = i
						...
						for (const feedback of feedbacks) {
							feedback.instance_id = instance_id
						}
					}

					if (idx + 1 < targetIdx) {
						// Save config, but do not automatically call this module's updateConfig again
						// Either only _configIdx changed, or the module is not active yet
						self.system.emit('instance_config_put', instance_id, instance_config, true)
					}
