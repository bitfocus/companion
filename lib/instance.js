...
class instance {
	...
	connect(client) {
		...
		client.on('instance_edit', (id, answer) => {
			...
			SendResult(client, answer, 'instance_edit:result', id, this.store, configFields, this.store.db[id])
		...
		for (var i = 0; i < module_folders.length; ++i) {
			...
			try {
				...
				if (moduleconfig.name != folder) {
					debug('ERROR: Module ' + folder + ' identifies itthis as ' + moduleconfig.name)
					console.log('ERROR: Module ' + folder + ' identifies itthis as ' + moduleconfig.name)
					process.exit()
				}
