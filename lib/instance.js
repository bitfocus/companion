...
instance.prototype.setupModule = function(folder, path) {
	...
	try {
		...
		if (moduleconfig.manufacturer !== undefined && moduleconfig.name !== 'bitfocus-companion') {
			...
			debug('ERROR: Module ' + folder + ' is missing a manufacturer in it\'s package.json');
			self.system.emit('log', 'module('+folder+')', 'error', 'Module is missing a manufacturer tag and has been disabled.');
			//process.exit();
		}
		else {
