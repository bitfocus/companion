...
function instance(system) {
	...
	self.system.on('instance_http_req', function (req, res) {
		let instance
		res.set('Content-Type', 'text/plain')

		for (const id in self.active) {
			if (id === req.params.label || self.active[id].label === req.params.label) {
				instance = self.active[id]
				break
			}
		}

		if (instance && typeof instance.handleHttpRequest === 'function') {
			const requestData = {
				baseUrl: req.baseUrl,
				body: req.body,
			...
			Promise.race([httpTimeout(), instance.handleHttpRequest(requestData)])
				.then(responseData => {
					const data = {
						status: 200,
						headers: {},
...
instance.prototype.activate_module = function (id, modin, is_being_created) {
	...
	// Ensure the 'legacy' instance_type has been updated, just in case it was skipped earlier
	const pending_rename = self.mod_redirects[self.store.db[id].instance_type]
	if (pending_rename) {
		self.store.db[id].instance_type = pending_rename
	}
