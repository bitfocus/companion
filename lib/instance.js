...
instance.prototype.connect = function (client) {
	...
	client.on('instance_order', function(page, bank, old_index, new_index) {
		var instances = self.instances[page][bank];
		if (instances !== undefined) {
			instances.splice(new_index, 0, instances.splice(old_index, 1)[0]);
			self.system.emit('instance_save');
		}
	});
