	...
	system.on('instance_init', function () {
		...
		for (var id in self.store.db) {
			...
			if (self.checkModuleLoaded(config.instance_type)) {
				...
				self.upgrade_instance_config(id, mod, config)
	...
	system.on(
		...
		function (instance_id, instance_created, instance_type, imported_config, actions, feedbacks) {
			if (self.checkModuleLoaded(instance_type)) {
				...
				const upgrade_config = instance_created ? imported_config : null
				self.upgrade_instance_config_partial(instance_id, mod, upgrade_config, actions, feedbacks)
			}
		}
	)
...
instance.prototype.upgrade_instance_config = function (id, mod) {
	var self = this
	...
	const changed = self.upgrade_instance_config_partial(id, mod, config, [...actions, ...release_actions], feedbacks)
...
instance.prototype.upgrade_instance_config_partial = function (id, mod, upgrade_config, actions, feedbacks) {
	var self = this
	...
	const instance_config = upgrade_config || self.store.db[instance_id]

	var idx = instance_config._configIdx
	if (idx === undefined) {
		idx = -1
	}
	...
	var debug = require('debug')('instance:' + package_info.name + ':' + id)

	var upgrade_scripts = mod.GetUpgradeScripts() || []
	if (idx + 1 < upgrade_scripts.length) {
		debug('upgradeConfig(' + package_info.name + '): ' + (idx + 1) + ' to ' + upgrade_scripts.length)
	}
