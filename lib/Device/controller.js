...
const debug       = require('debug')('lib/Device/controller');
...
const DeviceSoftwareElgatoEmulator  = require('./Software/elgato-emulator');
const DeviceSoftwareElgatoPlugin    = require('./Software/elgato-plugin');
const DeviceSoftwareSatelliteDevice = require('./Software/satellite-device');
...
class DeviceController extends CoreBase {
	...
	addDevice(device, type) {
		...
		if (type === 'DeviceSoftwareElgatoEmulator') {
		...
		else if (type === 'DeviceSoftwareSatelliteDevice') {
			this.instances[device.path] = new DeviceSoftwareSatelliteDevice(this.system, device.path, device);
			this.instances[device.path].deviceHandler = new DeviceHandler(this.registry, this.instances[device.path]);
			...
			try {
				...
				this.system.emit('log', 'USB(' + type.deviceType + ')', 'error', 'Found device, but no access. Please quit any other applications using the device, and try again.');
