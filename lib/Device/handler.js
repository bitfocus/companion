...
var debug = require('debug')('lib/Device/handler');
...
class DeviceHandler {
	...
	constructor(registry, panel) {
		...
		this.system.on('graphics_bank_invalidated', this.onBankInvalidated.bind(this));

		this.system.on('graphics_page_controls_invalidated', this.onGraphicsPageControlsInvalidated.bind(this));

		//this.system.on('ready', this.onReady.bind(this));  not a system call

		this.system.on('lockoutall', this.lockoutAll.bind(this));

		this.system.on('unlockoutall', this.unlockoutAll.bind(this));

		this.timeouttimer = setInterval(this.checkTimeout.bind(this), 1000);

		this.system.on('device_redraw', this.onDeviceRedraw.bind(this));

		this.system.on('elgato_ready',this.onElgatoReady.bind(this));

		this.system.on('device_page_set', this.onDevicePageSet.bind(this));

		this.system.on('lockout_device', this.lockoutDevice.bind(this));

		this.system.on('unlockout_device', this.unlockoutDevice.bind(this));

		this.system.on('device_page_down', this.onDevicePageDown.bind(this));

		this.system.on('device_page_up', this.onDevicePageUp.bind(this));

		this.system.on('elgato_click', this.onElgatoClick.bind(this));
	...
	checkTimeout() {
		...
		if (this.userconfig.pin_timeout != 0 && this.userconfig.pin_enable == true && Date.now() >= this.lockoutat && !this.lockedout) {
	...
	*/
	...
	onElgatoClick(devicepath, key, state, obj) {
		...
		if (!this.lockedout) {
			...
			this.system.emit('log', 'device('+this.panel.serialnumber+')', 'debug', 'Button '+this.page+'.' + (parseInt(key) + 1) + ' ' + (state ? 'pressed' : 'released'));
		}
		else {
			if (state) {
	...
	*/
	...
	unload() {
		...
		//this.system.removeListener('ready', this.onReady);
