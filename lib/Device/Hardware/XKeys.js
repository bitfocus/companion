...
var debug = require('debug')('lib/Device/Hardware/XKeys')
const { XKeysModels } = require('../../Resources/XKeysProducts')
...
class DeviceHardwareXKeys extends DeviceBase {
	...
	createXkeysDevice(devicepath) {
		...
		this.myXkeysPanel.deviceType = XKeysModels.models[0]
		devices.forEach((element) => {
			if (element.path == devicepath) {
				let productId = element.productId
				XKeysModels.models.forEach((model) => {
