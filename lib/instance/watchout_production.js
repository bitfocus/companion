var debug	 = require('debug')('instance/watchout_production');
...
instance.prototype.action = function(action) {
	...
	switch (action.action) {
		...
		case 'kill':
			if (action.options.timeline != '')
				...
				self.system.emit('skeleton-log', 'Error: Kill command for Watchout production triggered without timeline name');
			}
			break;
		...
		case 'gototime':
			if (action.options.time != '') {
				...
				self.system.emit('skeleton-log', 'Error: Gototime command for Watchout production triggered without entering time');
			}
			break;
		...
		case 'gotocue':
			if (action.options.cuename != '') {
				...
				self.system.emit('skeleton-log','Error: GotoControlCue command for Watchout production triggered without entering cue');
			}
			break;
		...
		case 'setinput':
			if (action.options.inputname != '' && action.options.inputvalue != '') {
				...
				self.system.emit('skeleton-log','Error: setInput command for Watchout production triggered without entering input name or input value');
			}
			break;
	...
	if (cmd !== undefined) {
		...
		if ( !self.socket.readable )
				self.socket.connect(3040,self.config.host);
