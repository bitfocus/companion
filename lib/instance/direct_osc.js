...
instance.prototype.action = function(action) {
	...
	if (action.action == 'send_bool') {
		debug('sending',self.config.host, self.config.port, action.options.string, bol);
		var bol = actions.options.bool === 0 ? true : false;
		self.system.emit('osc_send', self.config.host, self.config.port, action.options.string, bol)
	}
