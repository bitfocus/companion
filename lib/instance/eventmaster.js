var debug       = require('debug')('instance/eventmaster');
			...
			if (check.isValid === true) {
				...
				ping.sys.probe(self.config.host, function(isAlive){
					if (isAlive == true) {
						...
						self.system.emit('skeleton-log', 'EventMaster: No ping reply from '+self.config.host+'??')
					}
				}, cfg);
			}
		}
...
instance.prototype.actions = function(system) {
	...
	if (self.eventmaster !== undefined) {
		self.eventmaster.listPresets(-1, -1, function(obj, res) {
			...
			self.system.emit('skeleton-log', 'EventMaster Error: '+ err);
		});
	}
}
