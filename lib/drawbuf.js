...
drawbuf.prototype.drawText = function(x,y,text,color,fontindex,spacing, double) {
	var self = this;
	...
	for (var i in chars) {
		...
		if (!(chars[i] == " " && just_wrapped == true)) {
			x_pos += (double ? 4 : 2) + self.drawLetter(x+x_pos,y_pos,chars[i],color,fontindex,double);
		}
		just_wrapped = false;
	}
...
drawbuf.prototype.drawLetter = function(x,y,letter,color,fontindex,double) {
	var self = this;
	...
	if (num == 46) return 2;	// return blanks for space
