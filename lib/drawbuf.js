...
var debug   = require('debug')('lib/drawbuf');
...
drawbuf.prototype.drawText = function(x,y,text,color,fontindex,spacing, double, dummy) {
	var self = this;
...
drawbuf.prototype.drawCenterText = function(x,y,text,color,fontindex,spacing, double) {
	var self = this;
	var orig_x = x;
...
drawbuf.prototype.drawLetter = function(x,y,letter,color,fontindex,double,dummy) {
	var self = this;
	...
	if (fontindex !== 'icon') {
		num = letter.toUpperCase().charCodeAt(0);
	}
	else {
		num = letter;
	}
...
drawbuf.prototype.bufferAndTime = function () {
	var self = this;

	return { updated: self.lastUpdate, buffer: self.buffer() };
}

drawbuf.rgb = drawbuf.prototype.rgb;
exports = module.exports = drawbuf;
