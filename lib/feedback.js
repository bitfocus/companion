...
function feedback(system) {
	...
	self.feedbacks = {};/*
		'1': {
			'9': [{ id: 'shortid1', type: 'input_bg', instance_id: 'BJSU3dbz7', input: 3, output: 19 }],
			'10': [{ id: 'shortid2', type: 'input_bg', instance_id: 'BJSU3dbz7', input: 4, output: 19 }, { id: 'shortid3', type: 'input_bg', instance_id: 'BJSU3dbz7', input: 5, output: 19 }]
		}
	};*/
		...
		client.on('bank_delFeedback', function(page, bank, id) {
			...
			for (var i = 0; i < feedbacks.length; ++i) {
				if (feedbacks[i].id == id) {
					feedbacks.splice(n, 1);
					break;
				}
			}

			system.emit('feedback_save');
			client.emit('bank_get_actions:result', page, bank, self.feedbacks[page][bank] );
			// feedback_instance_check
		});
				...
				for (var n in feedbacks) {
					...
					if (feedback !== undefined && feedback.id === feedbackid) {
						if (feedback.options === undefined) {
							self.feedbacks[page][bank][n].options = {};
						}
						self.feedback[page][bank][n].options[option] = value;
