...
function feedback(system) {
	...
	system.on('feedback_instance_definitions_set', function (instance, feedbacks) {
		const feedbacksBefore = self.feedback_definitions[instance.id]
		self.feedback_definitions[instance.id] = _.cloneDeep(feedbacks)

		const patch = jsonPatch.compare(feedbacksBefore || {}, feedbacks || {})
		if (patch.length > 0) {
			io.emit('feedback_instance_definitions_patch', instance.id, patch)
		}
	})
	...
	system.on('feedback_check_bank', function (page, bank, constraint) {
		...
		if (self.feedbacks[page][bank] !== undefined) {
			system.emit('get_bank', page, bank, function (bank_obj) {
				...
				let removeTopBar = !bank_obj.show_topbar
				if (bank_obj.show_topbar === 'default' || bank_obj.show_topbar === undefined) {
					system.emit('get_userconfig_key', 'remove_topbar', (_key) => (removeTopBar = _key))
				}

				if (removeTopBar === true) {
					width = 72
					height = 72
				} else {
					width = 72
	...
	system.on('feedback_check_all', self.checkAll.bind(self))

	system.on('feedback_check_all_banks', self.checkAll.bind(self))
	...
	system.on('instance_delete', function (id) {
		...
		io.emit('feedback_instance_definitions_patch', id, null)
	})
	...
	system.on('io_connect', function (client) {
		...
		client.on('feedback_instance_definitions_get', function (answer) {
			answer(self.feedback_definitions)
		})
...
feedback.prototype.checkAll = function (constraints) {
	var self = this

	if (self.feedbacks !== undefined) {
		for (var page in self.feedbacks) {
			for (var bank in self.feedbacks[page]) {
				// Iterate through feedbacks on this bank
				self.system.emit('feedback_check_bank', page, bank, constraints)
			}
		}
	}
}
