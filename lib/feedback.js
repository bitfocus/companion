...
function feedback(system) {
						...
						if (feedback.instance_id == instance.id) {
							system.emit('get_bank', page, bank, function (bank_obj) {
								...
								try {
									...
									self.system.emit('log', 'instance('+instance.label+')', 'warn', 'Error checking feedback: ' + e.message);
								}
							});
						}
					}
	...
	system.on('feedback_delete', function (page, bank, index) {
		if(self.feedbacks[page] !== undefined && self.feedbacks[page][bank] !== undefined && self.feedbacks[page][bank][index]!== undefined) {
			self.unsubscribeFeedback(self.feedbacks[page][bank][index])
			self.feedbacks[page][bank].splice(index, 1);
		}
		if (self.feedback_styles[page] !== undefined && self.feedback_styles[page][bank] !== undefined && self.feedback_styles[page][bank][index] !== undefined) {
