...
button.prototype.drawPage = function() {
	var self = this;
	...
	for (var num in b) {
		...
		console.log("bank", bank, parseInt(num), parseInt(num) + 1);
		...
		if (self.config[self.page] !== undefined && self.config[self.page][bank] !== undefined) {
			...
			img.drawText(2,3,self.page+"."+bank,img.rgb(30,30,30),0);
...
button.prototype.drawControls = function() {
	var self = this;
...
button.prototype.updatePageButton = function() {
	var self = this;
	var img = new Image(72,72);
	img.backgroundColor(img.rgb(15,15,15));
...
button.prototype.quit = function () {

};

exports = module.exports = function (system, panel) {
	return new button(system, panel);
};
