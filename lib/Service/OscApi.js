...
export class ServiceOscApi extends CoreBase {
	 ...
	 * @returns {void}
	 */
	#locationRotateLeft = (match, message) => {
		const { location, controlId } = this.#locationParse(match)
	 ...
	 * @returns {void}
	 */
	#locationRotateRight = (match, message) => {
		const { location, controlId } = this.#locationParse(match)
	...
	#locationSetStyleText = (match, message) => {
		...
		const control = this.controls.getControl(controlId)
		if (!control || typeof control.styleSetFields !== 'function') return

		control.styleSetFields({ text: text })
	}

	/**
	 * Perform control style cp;pr change
	 * @param {Record<string, string>} match
	 * @param {*} message
	 * @returns {void}
	 */
