...
class ServiceOscListener extends ServiceOscBase {
	...
	#setupNewOscRoutes() {
		// controls
		this.#router.addPath('/controls/:page([0-9]{1,2})/:row(-?[0-9]+)/:column(-?[0-9]+)/press', this.#controlPress)
		this.#router.addPath('/controls/:page([0-9]{1,2})/:row(-?[0-9]+)/:column(-?[0-9]+)/down', this.#controlDown)
		this.#router.addPath('/controls/:page([0-9]{1,2})/:row(-?[0-9]+)/:column(-?[0-9]+)/up', this.#controlUp)
		this.#router.addPath(
		...
		// this.#router.addPath('/controls/:page([0-9]{1,2})/:row(-?[0-9]+)/:column(-?[0-9]+)/style', this.#controlStyle)
	 ...
	 * @returns {void}
	 */
	#controlRotateLeft = (match, message) => {
		const location = {
			pageNumber: Number(match.page),
			row: Number(match.row),
			column: Number(match.column),
	 ...
	 * @returns {void}
	 */
	#controlRotateRight = (match, message) => {
		const location = {
			pageNumber: Number(match.page),
			row: Number(match.row),
			column: Number(match.column),
	...
	#customVariableSetValue = (match, message) => {
		...
		this.registry.instance.variable.custom.setValue(variableName, variableValue)
	}
}
