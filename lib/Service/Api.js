	...
	parseApiCommand(data, response_cb) {
		...
		if ((match = command.match(/^page-set (\d+) ([a-z0-9]{3,32})\n?$/i))) {
			...
			if (page > 0 && page <= 99 && bank > 0 && bank <= global.MAX_BUTTONS) {
				this.logger.debug(func + ': ' + page + '.' + bank)

				if (func == 'bank-press') {
					this.logger.info('Got /press/bank/ (trigger)', page, 'button', bank)
					this.bank.action.pressBank(page, bank, true)

					setTimeout(() => {
						this.logger.info('Auto releasing /press/bank/ (trigger)', page, 'button', bank)
