...
class ServiceArtnet extends CoreBase {
	constructor(registry) {
		super(registry, 'artnet', 'lib/Service/Artnet')
	...
	updateUserconfig(key, value) {
		if (key == 'artnet_enabled') {
			if (this.current_state == false && value == true) {
				try {
					this.current_state = true
					this.listen(6454)
					this.log('debug', 'Listening')
			...
			} else if (this.current_state == true && value == false) {
				this.current_state = false
				this.log('debug', 'Destroying artnet socket')
				if (this.sock) {
					try {
						this.sock.close()
					} catch (e) {}
