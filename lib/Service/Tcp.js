	...
	start_listening() {
		...
		this.server = net.createServer((socket) => {
			socket.on('end', () => {
				...
				this.log('debug', 'Client disconnected: ' + socket.name)
			})

			socket.on('error', () => {
				this.clients.splice(this.clients.indexOf(socket), 1)
				this.debug('Client debug disconnected: ' + socket.name)
				this.log('error', 'Client errored/died: ' + socket.name)
			})
			...
			this.log('debug', 'Client connected: ' + socket.name)
		...
		this.server.on('listening', () => {
			...
			this.log('info', 'Ready for commands')
		})

		this.server.on('error', (e) => {
			this.debug('TCP Server got error: ' + e)
			this.log('error', 'Server failed: ' + e)
		})

		try {
			this.debug(`Trying: listen to tcp ${port}`)
			this.server.listen(port)
			this.log('info', `Listening for TCP commands on port ${port}`)
		} catch (e) {
			this.log('error', `Couldn't bind to TCP port ${port}`)
		}
	}

	stop_listening() {
		this.debug(`Stopping tcp server`)
		this.log('info', `Stopping listening for TCP commands`)
