...
class ServiceSatellite extends ServiceBase {
	...
	addDevice(socket, params) {
		...
		const keysTotal = params.KEYS_TOTAL ? parseInt(params.KEYS_TOTAL) : LEGACY_MAX_BUTTONS
		if (isNaN(keysTotal) || keysTotal > LEGACY_MAX_BUTTONS || keysTotal <= 0) {
			socket.write(`ADD-DEVICE ERROR DEVICEID="${params.DEVICEID}" MESSAGE="Invalid KEYS_TOTAL"\n`)
			return
		}

		const keysPerRow = params.KEYS_PER_ROW ? parseInt(params.KEYS_PER_ROW) : LEGACY_BUTTONS_PER_ROW
		if (isNaN(keysPerRow) || keysPerRow > LEGACY_MAX_BUTTONS || keysPerRow <= 0) {
