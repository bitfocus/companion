...
class ServiceSatellite extends ServiceBase {
	...
	addDevice(socket, params) {
		...
		this.logger.silly(`add device "${id}" for ${socket.remoteAddress}`)
	...
	handleCommand(socket, line) {
		if (!line.trim().toUpperCase().startsWith('PING')) {
			this.logger.silly(`received "${line}" from ${socket.name}`)
		}
	...
	listen() {
		...
		try {
			...
			this.logger.debug('ERROR opening port 16622 for companion satellite devices')
		}
	}
	 ...
	 */
	initSocket(socket) {
		this.logger.debug(`new satellite connection from ${socket.name}`)
		...
		socket.on('error', (e) => {
			this.logger.silly('socket error:', e)
		})
