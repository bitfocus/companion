...
const debug          = require('debug')('lib/Service/Satellite');
...
class ServiceSatellite extends ServiceTcpBase {
		...
		let abort = () => {
			...
			for (let key in this.devices) {
				if (this.devices[key].client === client) {
					this.deviceController().removeDevice(this.devices[key].id);
	...
	newDeviceInner(data, client) {
		...
		this.deviceController().addDevice({ path: id, keysTotal: data.keysTotal, keysPerRow: data.keysPerRow }, 'DeviceSoftwareSatellite');
	...
	removeDevice(packet, client) {
		...
		if (this.devices[data.deviceId] !== undefined) {
			...
			this.deviceController().removeDevice(this.devices[data.deviceId].id);
