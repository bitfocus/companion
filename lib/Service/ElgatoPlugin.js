...
import { splitCoordinate } from '../Shared/ControlId.js'
...
class ServiceElgatoPlugin extends ServiceBase {
	 ...
	 */
	handleButtonDrawn(page, coordinate, render) {
		page = Number(page)
		// bank = parseInt(bank) - 1

		if (this.client && this.client.button_listeners) {
			if (this.client.button_listeners.has(`${page}_${coordinate}`)) {
				this.client.apicommand('fillImage', { page: page, coordinate: coordinate, data: render.buffer })
			}
	...
	initAPI2(socket) {
		...
		socket.on('request_button', (args) => {
			...
			if (args.coordinate) {
				socket.button_listeners.add(`${args.page}_${args.coordinate}`)

				socket.apireply('request_button', { result: 'ok' })

				this.handleButtonDrawn(args.page, args.coordinate, this.graphics.getBank(args.page, args.coordinate))
			} else {
		...
		socket.on('unrequest_button', (args) => {
			...
			if (args.coordinate) {
				socket.button_listeners.delete(`${args.page}_${args.coordinate}`)
			} else {
