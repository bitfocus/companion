...
class ServiceElgatoPlugin extends ServiceBase {
	...
	#handleButtonDrawn(location, render) {
		...
		if (this.client && this.client.buttonListeners) {
			if (this.client.buttonListeners.has(`${location.pageNumber}_${location.column}_${location.row}`)) {
			...
			if (location.column >= 0 && location.row >= 0 && location.column < cols && location.row < rows) {
				...
				if (this.client.buttonListeners.has(`${location.pageNumber}_${bank}`)) {
					this.client.apicommand('fillImage', {
						page: location.pageNumber,
						bank: bank,
						keyIndex: bank,
	...
	#initAPI2(socket) {
		...
		socket.once(
			'new_device',
			(/** @type {string | import('../Surface/IP/ElgatoPlugin.js').ElgatoPluginClientInfo} */ info) => {
				try {
		...
		socket.on('request_button', (args) => {
			...
			if ('column' in args || 'row' in args) {
				...
				this.#handleButtonDrawn(location, this.graphics.getBank(location))
			} else {
