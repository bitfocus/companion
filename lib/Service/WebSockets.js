...
const debug = require('debug')('lib/Service/WebSockets')
...
class ServiceWebSockets extends CoreBase {
	...
	initAPIv1(socket) {
		...
		socket.once('new_device', (id) => {
			...
			this.deviceController().addDevice({ path: id }, 'DeviceSoftwareStreamdeckPlugin')
			...
			socket.on('close', () => {
				this.deviceController().removeDevice(id)
