...
import { oldBankIndexToCoordinate, splitCoordinate, xyToOldBankIndex } from '../Shared/ControlId.js'
...
class ServiceEmberPlus extends ServiceBase {
	...
	constructor(registry) {
		...
		this.graphics.on('button_drawn', this.updateBankText.bind(this))
	...
	getPagesTree() {
		...
		for (let page = 1; page <= 99; page++) {
			...
			for (let bank = 1; bank <= 32; bank++) {
				const controlId = this.page.getControlIdAt(page, oldBankIndexToCoordinate(bank))
	 ...
	 * @param {number} coordinate - the bank number
	 ...
	 */
	updateBankState(page, coordinate, pushed, deviceid) {
		...
		if (this.server) {
			...
			if (node) {
				this.server.update(node, { value: pushed })
			}
		}
	}
	 ...
	 * @param {number} coordinate - the bank number
	 */
	updateBankText(page, coordinate, render) {
		const bank = xyToOldBankIndex(...splitCoordinate(coordinate))
