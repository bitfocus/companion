...
class ServiceHttps extends CoreBase {
	currentState = false
	debug = require('debug')('lib/Service/Https')
	...
	constructor(registry, express, io) {
		super(registry, 'https', 'lib/Service/Https')
	...
	startServer(credentials) {
		try {
			...
			this.socket.on('error', this.handleSocketError.bind(this)).listen(this.port, this.bind_ip)
			...
			this.log('info', `Listening at https://${this.bind_ip}:${this.port}`)
			this.debug(`Listening at https://${this.bind_ip}:${this.port}`)
		} catch (e) {
