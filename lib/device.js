...
function device(_system, panel) {
	...
	if (self.deviceconfig[self.panel.serialnumber]) {
		...
		if (!self.panelconfig.config) self.panelconfig.config = {}
		...
		} else if (self.panelconfig.page !== undefined) {
			// use last page if defined
			self.page = self.panelconfig.page
		} else {
			// otherwise start on page 1
			self.page = 1
		}
...
device.prototype.setPanelConfig = function(newconfig) {
	...
	if (newconfig.page !== undefined && newconfig.page !== self.panelconfig.config.page) {
		// Startup page has changed, so change over to it
		self.storeNewDevicePage(newconfig.page)
	}

	self.storeNewDeviceConfig(newconfig)
	
	self.panel.setConfig(newconfig, function(redraw) {
		if (redraw) {
			// device wants a redraw
			self.drawPage()
		}
...
device.prototype.storeNewDeviceConfig = function (newconfig) {
	var self = this
...
device.prototype.drawPage = function () {
	...
	console.log(`Drawing page: ${self.page} for ${self.panel.serialnumber}`)
