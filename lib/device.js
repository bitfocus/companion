...
function device(_system, panel) {
	...
	self.on_elgato_click = function(devicepath, key, state, obj) {
		...
		if (devicepath != self.devicepath) {
			console.log("wrong devicepath", devicepath,"vs", self.devicepath);
			return;
		}

		if (!self.lockedout) {

			console.log("!self.lockedout");
			...
			if (state == true) {
				...
				system.emit('bank_pressed', self.page, parseInt(key) + 1, false, self.panel.serialnumber);
				system.emit('log', 'device('+self.panel.serialnumber+')', 'debug', 'Button '+self.page+'.' + (parseInt(key) + 1) + ' released');
			}
...
device.prototype.updateBank = function(page, bank) {
	...
	debug('is locked out: ' + (self.lockedout ? 'yes' : 'no'));

	if (!self.lockedout) {
		var img = graphics.getBank(page, bank);

		debug('Update bank ' + page + '.' + bank);
		debug("Is same page? " + page + " == " + self.page);
		if (page == self.page) {
			setImmediate(function () {
				debug('panel draw: ' + page + '.' + bank);
				self.panel.draw(bank - 1, img.buffer, img.ts);
			});
		}
	} else {
		self.datap = graphics.getImagesForPincode(self.currentpin);
