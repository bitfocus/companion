...
const debug = require('debug')('lib/server_https')
const fs = require('fs')
...
class server_https {
	...
	enableModule() {
		if (this.initialized === true) {
			...
			try {
				...
				debug(`Error starting https server`, e)
			}
		}
	}
			...
			if (this.userconfig.https_cert_type == 'external') {
				if (this.userconfig.https_ext_private_key != '' && this.userconfig.https_ext_certificate != '') {
					try {
						...
						debug(`Read private key file: ${this.userconfig.https_ext_private_key}`)
						...
						debug(`Read certificate file: ${this.userconfig.https_ext_certificate}`)
						...
						if (this.userconfig.https_ext_chain != '' && fs.existsSync(this.userconfig.https_ext_chain)) {
							try {
								...
								debug(`Read chain file: ${this.userconfig.https_ext_chain}`)
								credentials.ca = ca
							} catch (e) {
								this.log('warn', `Couldn't read chain field: ${e}`)
								debug(`Couldn't read chain field: ${e}`)
							}
						}
						...
						debug(`Could not start: ${e}`)
					}
				} else {
					this.log('error', `Could not start: Private Key and/or Certificate files not set`)
					debug(`Could not start: Private Key and/or Certificate files not set`)
				}
			} else {
				if (this.userconfig.https_self_cert_private != '' && this.userconfig.https_self_cert != '') {
					...
					debug(`Could not start: Incomplete or no self-signed certificate on file`)
				}
			}
		}
	}
	...
	startServer(credentials) {
		try {
			...
			debug(`Listening at https://${this.config.bind_ip}:${this.port}`)
		} catch (e) {
			this.log('error', `Couldn't bind to port ${this.port}`)
			debug(`Couldn't bind to port ${this.port}: ${e}`)
			delete this.socket
		}
	}
