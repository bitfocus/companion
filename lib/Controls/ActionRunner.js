...
export default class ActionRunner extends CoreBase {
	...
	#runAction(action, extras) {
		//console.log("action run",action,extras);

		const reg = /\$\(([^:$)]+):([^)$]+)\)/g

		const bank = extras.page + '_' + extras.bank
		let actionClone = {}

		let tmp = {}
		for (var option in action.options) {
			let value = action.options[option]
		...
		actionClone = { ...action, ...{ options: tmp } }

		if (action.instance === 'internal') {
			this.internalModule.executeAction(actionClone, extras)
		} else {
			const instance = this.instance.moduleHost.getChild(action.instance)
			if (instance) {
				instance.actionRun(actionClone, extras).catch((e) => {
