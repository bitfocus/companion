...
export default class ActionRunner extends CoreBase {
	 ...
	 * @param {string[]} skipCoordinates coordinats to skip
	 * @access public
	 */
	abortPageDelayed(pageNumber, skipCoordinates) {
		const controlIds = new Set(this.page.getAllControlIdsOnPage(pageNumber))

		// Remove any skipped coordinates
		for (const coordinate of skipCoordinates) {
			controlIds.delete(this.page.getControlIdAt(pageNumber, coordinate))
		}
