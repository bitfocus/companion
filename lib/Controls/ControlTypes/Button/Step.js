...
export default class ControlButtonStep extends ButtonControlBase {
	...
	constructor(registry, controlId, storage, isImport) {
		...
		if (!storage) {
			...
			this.#current_step_id = Object.keys(this.actions.action_sets)[0]
	...
	actionSetAdd() {
		...
		if (existingKeys.length === 0) {
			...
			this.actions.action_sets[`${max + 1}`] = []
		}
	...
	actionSetSwap(setId1, setId2) {
		if ((this.actions.action_sets[setId1], this.actions.action_sets[setId2])) {
			const tmp = this.actions.action_sets[setId1]
			this.actions.action_sets[setId1] = this.actions.action_sets[setId2]
			this.actions.action_sets[setId2] = tmp
	 ...
	 */
	getActiveStepIndex() {
		const out = Object.keys(this.actions.action_sets).indexOf(this.#current_step_id)
	...
	pressControl(pressed, deviceId) {
		...
		if (changed && pressed) {
			...
			const steps = Object.keys(this.actions.action_sets)
			if (steps.length > 0) {
				steps.sort()
				...
				if (this.options.stepAutoProgress) {
