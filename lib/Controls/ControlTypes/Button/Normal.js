...
export default class ControlButtonNormal extends ButtonControlBase {
	 ...
	 * @type {import('../../../Data/Model/ActionModel.js').ActionStepOptions}
	 * @access public
	 * @static
	 */
	 ...
	 * @type {import('../../../Data/Model/ButtonModel.js').NormalButtonOptions}
	 */
	options
	 ...
	 * @param {import('../../../Data/Model/ButtonModel.js').NormalButtonModel | null} storage - persisted storage object
	 ...
	 * @returns {void}
	 */
	abortRunningHoldTimers(deviceId) {
		if (deviceId) {
			const existingState = this.#surfaceHoldState.get(deviceId)
			if (existingState) {
			...
			this.#surfaceHoldState.delete(deviceId)
		} else {
	 ...
	 * @returns {number | undefined} The index of current step
	 * @access public
	 */
	getActiveStepIndex() {
	...
	/**
	 * @param {import('../../../Data/Model/ActionModel.js').ActionSetsModel=} existingActions
	 * @param {import("../../../Data/Model/ActionModel.js").ActionStepOptions=} existingOptions
	 */
	#getNewStepValue(existingActions, existingOptions) {
		/** @type {import('../../../Data/Model/ActionModel.js').ActionSetsModel} */
	...
	collectReferencedInstances(foundInstanceIds, foundInstanceLabels) {
		...
		for (const feedback of allFeedbacks) {
			foundInstanceIds.add(feedback.instance_id)
		}
		for (const action of allActions) {
			foundInstanceIds.add(action.instance)
		}

		const visitor = new VisitorReferencesCollector(foundInstanceIds, foundInstanceLabels)

		this.registry.data.importExport.visitControlReferences(visitor, this.feedbacks.baseStyle, allActions, allFeedbacks)
	}
	 ...
	 * @access public
	 */
	pressControl(pressed, deviceId, force) {
		...
		if (deviceId) {
			...
			if (pressed) {
				this.abortRunningHoldTimers(deviceId)
				...
				this.#surfaceHoldState.set(deviceId, holdState)
			} else {
				const state = this.#surfaceHoldState.get(deviceId)
				if (state) {
	 ...
	 * @returns {import('../../../Data/Model/ButtonModel.js').NormalButtonModel}
	 * @access public
	 */
	toJSON(clone = true) {
		/** @type {import('../../../Data/Model/ButtonModel.js').NormalButtonSteps} */
		const stepsJson = {}
