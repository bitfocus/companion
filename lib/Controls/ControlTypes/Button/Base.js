...
export default class ButtonControlBase extends ControlBase {
	...
	getDrawStyle() {
		...
		if (style.text) {
			...
			if (location) {
				...
				injectedVariableValues[`$(internal:b_text_${location.pageNumber}_${location.coordinate})`] = '$RE'
			}
	...
	setPushed(direction, deviceId) {
		...
		if (this.pushed !== wasPushed) {
			...
			const location = this.page.getLocationOfControlId(this.controlId)
			if (location) {
				this.services.emberplus.updateBankState(location.pageNumber, location.coordinate, this.pushed, deviceId)
			}
