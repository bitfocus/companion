...
export default class ButtonControlBase extends ControlBase {
	...
	getDrawStyle() {
		...
		if (style.text) {
			...
			let bank = `${parsedControlId.page}_${parsedControlId.bank}`
			...
			if (style.textExpression) {
				try {
					...
					while ((matches = reg.exec(value))) {
						...
						if (instanceId === 'button') {
							value = value.replace(fullId, '$(internal:custom_bank_' + bank + '_' + variableId + ')')
						}
					}

					const parseResult = this.instance.variable.parseExpression(value, undefined, injectedVariableValues)
				...
				const parseResult = this.instance.variable.parseVariables(value, injectedVariableValues)
