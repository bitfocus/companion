...
export default class ControlTrigger extends ControlBase {
	...
	#setupEvents() {
		...
		this.#variablesEvents.setEnabled(this.config.enabled)
	 ...
	 * @access public
	 */
	configSetField(key, value, forceSet) {
	...
	triggerRedraw = debounceFn(
		() => {
			try {
				...
				if (this.config.enabled && this.#conditionCheckEvents.size > 0 && !this.#conditionCheckLastValue && newStatus) {
					setImmediate(() => {
