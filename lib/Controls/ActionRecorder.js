...
export default class ActionRecorder extends EventEmitter {
	...
	clientConnect(client) {
		...
		client.onPromise(
			...
			(/** @type {string} */ sessionId, /** @type {string[]} */ connectionIds) => {
				...
				this.setSelectedInstanceIds(connectionIds)
	...
	instanceAvailabilityChange(instanceId, running) {
		if (!running) {
			if (this.#currentSession) {
				...
				const newIds = this.#currentSession.connectionIds.filter((id) => id !== instanceId)
	 ...
	 */
	setSelectedInstanceIds(connectionIds0) {
		if (!Array.isArray(connectionIds0)) throw new Error('Expected array of instance ids')
