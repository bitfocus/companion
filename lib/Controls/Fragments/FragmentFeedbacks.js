...
export default class FragmentFeedbacks extends CoreBase {
	 ...
	 * @type {import('../../Data/Model/StyleModel.js').ButtonStyleProperties}
	 * @access public
	 * @static
	 */
	static DefaultStyle = {
	...
	#cleanupFeedback(feedback) {
		...
		if (instance) {
			instance.feedbackDelete(feedback).catch((/** @type {any} */ e) => {
	...
	#feedbackSubscribe(feedback) {
		if (!feedback.disabled) {
			if (feedback.instance_id === 'internal') {
				...
				const instance = this.instance.moduleHost.getChild(feedback.instance_id, true)
				if (instance) {
					instance.feedbackUpdate(feedback, this.controlId).catch((/** @type {any} */ e) => {
	 ...
	 * @returns {import('../../Data/Model/StyleModel.js').UnparsedButtonStyle} the unprocessed style
	 * @access public
	 */
	getUnparsedStyle() {
		if (this.#booleanOnly) throw new Error('FragmentFeedbacks not setup to use styles')

		/** @type {import('../../Data/Model/StyleModel.js').UnparsedButtonStyle} */
		let style = {
			...this.baseStyle,
			imageBuffers: [],
		}
