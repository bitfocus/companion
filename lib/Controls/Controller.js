...
class ControlsController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('triggers:create', () => {
			...
			const newControl = new ControlTrigger(this.registry, controlId, null, false)
		...
		client.onPromise('triggers:clone', (controlId) => {
			...
			if (fromControl) {
				...
				const newControl = this.#createClassForControl(newControlId, controlJson.type, controlJson, true)
				if (newControl) {
	...
	#createClassForControl(controlId, controlType, controlObj, isImport) {
		switch (controlType) {
			...
			case 'trigger':
				return new ControlTrigger(this.registry, controlId, controlObj, isImport)
			default:
				// Unknown type
