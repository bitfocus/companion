...
class ControlsController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('controls:feedback:learn', function (controlId, setId, id) {
			const control = this.getControl(controlId)
			if (!control) return false

			if (typeof control.feedbackLearn === 'function') {
				return control.feedbackLearn(setId, id)
			} else {
				throw new Error(`Control "${controlId}" does not support feedbacks`)
			}
		})
		...
		client.onPromise('controls:action:learn', function (controlId, setId, id) {
			const control = this.getControl(controlId)
			if (!control) return false
