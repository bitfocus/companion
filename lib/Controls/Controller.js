...
class ControlsController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('controls:set-config-field', (controlId, key, value) => {
			const control = this.getControl(controlId)
			if (!control) return false

			if (typeof control.configSetField === 'function') {
				return control.configSetField(key, value)
			} else {
				throw new Error(`Control "${controlId}" does not support config`)
			}
		})
		...
		client.onPromise('triggers:set-order', (triggerIds) => {
			...
			triggerIds.forEach((id, index) => {
				...
				if (control && typeof control.configSetField === 'function') control.configSetField('sortOrder', index, true)
			})

			// Fill in for any which weren't specified
			const updatedTriggerIds = new Set(triggerIds)
			const triggerControls = this.getAllControls().filter((c) => c.type === 'trigger')
			triggerControls.sort((a, b) => a.config.sortOrder - b.config.sortOrder)

			let nextIndex = triggerIds.length
			for (const control of allKntriggerControlsownIds) {
				if (!updatedTriggerIds.has(control.controlId) && typeof control.configSetField === 'function') {
					control.configSetField('sortOrder', nextIndex++, true)
				}
			}
