...
class ControlsController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('controls:hot-press', (location, direction, deviceId) => {
			this.logger.silly(
				`being told from gui to hot press ${location.pageNumber}-${location.column}-${location.row} ${direction} ${deviceId}`
			)
		...
		client.onPromise('controls:hot-rotate', (location, direction, deviceId) => {
			this.logger.silly(
				`being told from gui to hot rotate ${location.pageNumber}-${location.column}-${location.row} ${direction} ${deviceId}`
			)
