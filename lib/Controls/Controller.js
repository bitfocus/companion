...
/**
 * @typedef {import('../Data/Model/ButtonModel.js').SomeButtonModel | import('../Data/Model/TriggerModel.js').TriggerModel} SomeControlModel
 */
...
class ControlsController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise(
			...
			(fromLocation, toLocation) => {
				...
				const newControl = this.#createClassForControl(newControlId, 'bank', controlJson, true)
				if (newControl) {
		...
		client.onPromise(
			 ...
			 * @returns {boolean}
			 */
			(controlId, instanceId, feedbackId) => {
				const control = this.getControl(controlId)
				if (!control) return false
		...
		client.onPromise(
			 ...
			 * @returns {boolean}
			 */
			(controlId, stepId, setId, instanceId, actionId) => {
				const control = this.getControl(controlId)
				if (!control) return false
		...
		client.onPromise('triggers:subscribe', () => {
			...
			/** @type {Record<string, import('../Data/Model/TriggerModel.js').TriggerModel>} */
			const triggers = {}
		...
		client.onPromise(
			...
			(triggerIds) => {
				...
				triggerIds.forEach((id, index) => {
					...
					if (control && typeof control.optionsSetField === 'function')
						control.optionsSetField('sortOrder', index, true)
				})
	 ...
	 * @param {import('../Data/Model/ButtonModel.js').SomeButtonModel} definition object to import
	...
	importControl(location, definition, forceControlId) {
		...
		const newControl = this.#createClassForControl(newControlId, 'bank', definition, true)
		if (newControl) {
			this.#controls.set(newControlId, newControl)
	 ...
	 * @param {import('../Data/Model/TriggerModel.js').TriggerModel} definition object to import
	 * @returns
	 * @access public
	 */
	...
	deleteControl(controlId) {
		...
		if (location) {
			...
			this.services.emberplus.updateBankState(location, false, undefined)
	...
	createBankControl(location, newType) {
		...
		const newControl = this.#createClassForControl(controlId, 'bank', newType, false)
