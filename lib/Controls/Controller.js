...
class ControlsController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('controls:subscribe', (controlId) => {
			...
			setImmediate(() => {
				...
				if (location) {
					const img = this.graphics.getBank(location.pageNumber, location.coordinate)
		...
		client.onPromise('controls:reset', (pageNumber, coordinate, type) => {
			const controlId = this.page.getControlIdAt(pageNumber, coordinate)

			if (controlId) {
				this.deleteControl(controlId)
			}

			this.createBankControl(pageNumber, coordinate, type)
		})
		client.onPromise('controls:copy', (fromPageNumber, fromCoordinate, toPageNumber, toCoordinate) => {
			// Don't try copying over itself
			if (fromPageNumber === toPageNumber && fromCoordinate === toCoordinate) return false

			// Make sure target page number is valid
			if (!this.page.isPageValid(toPageNumber)) return false

			// Make sure there is something to copy
			const fromControlId = this.page.getControlIdAt(fromPageNumber, fromCoordinate)
			...
			const toControlId = this.page.getControlIdAt(toPageNumber, toCoordinate)
			if (toControlId) {
				this.deleteControl(toControlId)
			}

			const newControlId = CreateBankControlId(nanoid(), nanoid())
			...
			if (newControl) {
				...
				this.page.setControlIdAt(toPageNumber, toCoordinate, newControlId)
		...
		client.onPromise('controls:move', (fromPageNumber, fromCoordinate, toPageNumber, toCoordinate) => {
			...
			// Perform the move
			this.page.setControlIdAt(fromPageNumber, fromCoordinate, null)
			this.page.setControlIdAt(toPageNumber, toCoordinate, fromControlId)

			// Force a redraw
			this.graphics.invalidateBank(fromPageNumber, fromCoordinate)
			this.graphics.invalidateBank(toPageNumber, toCoordinate)

			return false
		})
		client.onPromise('controls:swap', (fromPageNumber, fromCoordinate, toPageNumber, toCoordinate) => {
			// Don't try moving over itself
			if (fromPageNumber === toPageNumber && fromCoordinate === toCoordinate) return false

			// Make sure both page numbers are valid
			if (!this.page.isPageValid(toPageNumber) || !this.page.isPageValid(fromPageNumber)) return false
			...
			// Perform the swap
			this.page.setControlIdAt(toPageNumber, toCoordinate, null)
			this.page.setControlIdAt(fromPageNumber, fromCoordinate, toControlId)
			this.page.setControlIdAt(toPageNumber, toCoordinate, fromControlId)
		...
		client.onPromise('controls:hot-press', (pageNumber, coordinate, direction, deviceId) => {
			this.logger.silly(`being told from gui to hot press ${pageNumber}-${coordinate} ${direction} ${deviceId}`)
		...
		client.onPromise('controls:hot-rotate', (pageNumber, coordinate, direction, deviceId) => {
			this.logger.silly(`being told from gui to hot rotate ${pageNumber}-${coordinate} ${direction} ${deviceId}`)
	 ...
	 * @access public
	 */
	importControl(pageNumber, coordinate, definition, forceControlId) {
		...
		const oldControlId = this.page.getControlIdAt(pageNumber, coordinate)
		if (oldControlId) {
			this.deleteControl(oldControlId)
		}

		const newControlId = forceControlId || CreateBankControlId(nanoid(), nanoid())
		const newControl = this.#createClassForControl(newControlId, 'bank', definition.type, definition, true)
		if (newControl) {
			this.#controls[newControlId] = newControl

			this.page.setControlIdAt(pageNumber, coordinate, newControlId)
	...
	deleteControl(controlId) {
		...
		const location = this.page.getLocationOfControlId(controlId)
		if (location) {
			this.page.setControlIdAt(location.pageNumber, location.coordinate, null)

			// Notify interested parties
			this.services.emberplus.updateBankState(location.pageNumber, location.coordinate, false)

			// Force a redraw
			this.graphics.invalidateBank(location.pageNumber, location.coordinate)
		}
	}
	 ...
	 * @access public
	 */
	createBankControl(pageNumber, coordinate, newType) {
		if (!this.page.isPageValid(pageNumber)) return null

		const controlId = CreateBankControlId(nanoid(), nanoid())
		const newControl = this.#createClassForControl(controlId, 'bank', newType, null, false)
		if (!newControl) return null

		this.#controls[controlId] = newControl
		this.page.setControlIdAt(pageNumber, coordinate, controlId)

		// Notify interested parties
		this.services.emberplus.updateBankState(pageNumber, coordinate, false)
