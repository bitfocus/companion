...
class DataImportExport extends CoreBase {
	...
	constructor(registry) {
		...
		const get_grid_dimensions = () => {
			return {
				minColumn: this.userconfig.getKey('grid_min_column'),
				maxColumn: this.userconfig.getKey('grid_max_column'),
				minRow: this.userconfig.getKey('grid_min_row'),
				maxRow: this.userconfig.getKey('grid_max_row'),
			}
		}
		...
		this.registry.api_router.get('/export/page/:page', (req, res, next) => {
			...
			if (isNaN(page)) {
				...
				pageInfo.gridSize = get_grid_dimensions()
	...
	clientConnect(client) {
		...
		const doPageImport = (pageInfo, topage, instanceIdMap) => {
			...
			if (pageInfo) {
				console.log(this, this.userconfig)
				...
				if (this.userconfig.getKey('grid_min_column') > requiredSize.minColumn)
					this.userconfig.setKey('grid_min_column', Number(requiredSize.minColumn))
				if (this.userconfig.getKey('grid_max_column') < requiredSize.maxColumn)
					this.userconfig.setKey('grid_max_column', Number(requiredSize.maxColumn))
				if (this.userconfig.getKey('grid_min_row') > requiredSize.minRow)
					this.userconfig.setKey('grid_min_row', Number(requiredSize.minRow))
				if (this.userconfig.getKey('grid_max_row') < requiredSize.maxRow)
					this.userconfig.setKey('grid_max_row', Number(requiredSize.maxRow))
	...
	async #reset(config, skipNavButtons = false) {
		...
		if (!config || config.buttons) {
			...
			// reset the size
			this.userconfig.resetKey('grid_min_column')
			this.userconfig.resetKey('grid_max_column')
			this.userconfig.resetKey('grid_min_row')
			this.userconfig.resetKey('grid_max_row')
		}
