...
import { CreateBankControlId, CreateTriggerControlId, ParseControlId, formatCoordinate } from '../Shared/ControlId.js'
 ...
 */
const default_nav_buttons_coordinates = {
	[formatCoordinate(0, 0)]: 'pageup',
	[formatCoordinate(0, 1)]: 'pagenum',
	[formatCoordinate(0, 2)]: 'pagedown',
}
...
class DataImportExport extends CoreBase {
			...
			if (isNaN(page)) {
				...
				for (const [coordinate, controlId] of Object.entries(pageInfo.controls)) {
					...
					if (controlId && control && control.type !== 'trigger') {
						pageInfo.controls[coordinate] = control.toJSON(false)
	...
	clientConnect(client) {
		...
		client.onPromise('loadsave:control-preview', (pageNumber, coordinate) => {
			const importObject = client.pendingImport?.object

			const controlObj =
				importObject?.type === 'page'
					? importObject?.page?.controls?.[coordinate]
					: importObject?.pages?.[pageNumber]?.controls?.[coordinate]
		...
		client.onPromise('loadsave:reset-page-clear', (pageNumber) => {
			...
			for (const [coordinate, type] of Object.entries(default_nav_buttons_coordinates)) {
				this.controls.createBankControl(pageNumber, coordinate, type)
			}
		...
		client.onPromise('loadsave:reset-page-nav', (pageNumber) => {
			...
			for (const [coordinate, style] of Object.entries(default_nav_buttons_coordinates)) {
				if (style) {
					this.controls.createBankControl(pageNumber, coordinate, style)
				}
			}
		...
		const doPageImport = (pageInfo, topage, instanceIdMap) => {
			...
			if (pageInfo) {
				...
				// Import the controls
				for (const [coordinate, control] of Object.entries(pageInfo.controls)) {
					if (control) {
						// Import the control
						const fixedControlObj = this.#fixupControl('bank', cloneDeep(control), instanceIdMap)
						this.controls.importControl(topage, coordinate, fixedControlObj)
					}
				}
			}
		}
	...
	async #reset(config, skipNavButtons = false) {
		...
		if (!config || config.buttons) {
			...
			for (let pageNumber = 1; pageNumber <= 99; ++pageNumber) {
				...
				if (!skipNavButtons) {
					for (const [coordinate, newStyle] of Object.entries(default_nav_buttons_coordinates)) {
						this.controls.createBankControl(pageNumber, coordinate, newStyle)
					}
				}
			}
		}
