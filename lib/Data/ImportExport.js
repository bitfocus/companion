...
class DataImportExport extends CoreBase {
		...
		this.registry.api_router.get('/trigger_export/:id', (req, res, next) => {
			...
			if (data) {
				...
				const filename = encodeURI(
					`${os.hostname()}_trigger_${data.title.toLowerCase().replace(/\W/, '')}_${getTimestamp()}.companionconfig`
				)
	...
	#fixupControl(control, instanceIdMap) {
		...
		if (control.actions && control.actions.action_sets) {
			for (const [setId, action_set] of Object.entries(control.actions.action_sets)) {
				const newActions = []
				for (const action of action_set) {
				...
				control.actions.action_sets[setId] = newActions
			}
		}
