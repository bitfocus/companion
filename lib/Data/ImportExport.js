	...
	constructor(registry) {
		...
		const generate_export_for_triggers = (triggerControls) => {
			...
			for (const control of triggerControls) {
				triggersExport.push(control.toJSON(false))
	...
	clientConnect(client) {
		...
		client.onPromise('loadsave:prepare-import', (dataStr) => {
			...
			const clientObject = {
				...
				customVariables: !!object.customVariables,
			...
			if (object.type === 'page') {
				...
				clientObject.triggers = object.triggers // TODO
			}
		...
		client.onPromise('loadsave:import-full', async (config) => {
			return this.checkOrRunImportTask('import', async () => {
				...
				// Destroy old stuff
				await this.#reset(undefined, true)
		...
		client.onPromise('loadsave:import-triggers', (idsToImport0, instanceRemapping, replaceExisting) => {
			return this.checkOrRunImportTask('import', async () => {
				...
				for (const id of idsToImport) {
					...
					const controlId = CreateTriggerControlId(Array.isArray(data.triggers) ? nanoid() : id)
					const fixedControlObj = this.#fixupControl(cloneDeep(trigger), instanceIdMap)
					this.controls.importTrigger(controlId, fixedControlObj)
				}
