 ...
 * @param {import("./Model/ExportModel.js").SomeExport4} data
...
/**
 *
 * @param {import('./Model/ExportModel.js').ExportPageContent4} pageInfo
...
class DataImportExport extends CoreBase {
	...
	constructor(registry) {
		 ...
		 * @returns {import('./Model/ExportModel.js').ExportInstances4}
		 */
		const generate_export_for_referenced_instances = (
			...
			/** @type {import('./Model/ExportModel.js').ExportInstances4} */
		 ...
		 * @returns {import('./Model/ExportModel.js').ExportTriggersList4}
		 */
		const generate_export_for_triggers = (triggerControls) => {
			/** @type {Record<string, import('./Model/ExportModel.js').ExportTriggerContent4>} */
		...
		this.registry.api_router.get('/export/triggers/all', (req, res, next) => {
			const triggerControls = Array.from(this.controls.getAllControls().values()).filter((c) => c.type === 'trigger')
		...
		this.registry.api_router.get('/export/page/:page', (req, res, next) => {
			...
			if (isNaN(page)) {
				...
				/** @type {import('./Model/ExportModel.js').ExportPageModel4} */
				const exp = {
					version: FILE_VERSION,
					type: 'page',
					page: pageExport,
		 ...
		 * @param {Set<string>} referencedInstanceIds
		 * @param {Set<string>} referencedInstanceLabels
		 * @returns {import('./Model/ExportModel.js').ExportPageContent4}
		 */
		const generatePageExportInfo = (pageInfo, referencedInstanceIds, referencedInstanceLabels) => {
			/** @type {import('./Model/ExportModel.js').ExportPageContent4} */
			const pageExport = {
				name: pageInfo.name,
				controls: {},
		...
		const generateCustomExport = (config) => {
			// Export file protocol version
			/** @type {import('./Model/ExportModel.js').ExportFull4} */
			const exp = {
				version: FILE_VERSION,
				type: 'full',
			}
			...
			if (!config || !isFalsey(config.buttons)) {
				...
				for (const [pageNumber, rawPageInfo] of Object.entries(pageInfos)) {
					exp.pages[Number(pageNumber)] = generatePageExportInfo(rawPageInfo, referencedInstanceIds, referencedInstanceLabels)
				}
			}
	...
	clientConnect(client) {
		...
		client.onPromise('loadsave:prepare-import',
			/**
			 * 
			 * @param {*} dataStr 
		...
		client.onPromise(
			 ...
			 * @returns {string | null} Image as data-url
			 */
			(location) => {
		...
		client.onPromise('loadsave:reset',
			/**
		...
		client.onPromise('loadsave:import-full',
			/**
		...
		client.onPromise('loadsave:import-page',
			/**
			 * @param {number} topage 
		...
		client.onPromise('loadsave:import-triggers',
			/**
	...
	/**
	 * @param {import('./Model/ExportModel.js').ExportInstances4 | undefined} instances
	 * @param {InstanceRemappings} instanceRemapping
	 ...
	 * @param {Record<string, any> | undefined} style Style object of the control (if any)
 ...
 * 
 * @typedef {{
 *   object: import('./Model/ExportModel.js').SomeExport4
 *   timeout: null
 * }} ClientPendingImport
 */