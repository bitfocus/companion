			...
			this.system.on('io_connect', (socket) => {
				socket.on('loadsave_import_config', (data, answer) => {
					...
					try {
						...
						sendResult(socket, answer, 'loadsave_import_config:result', 'File is corrupted or unknown format')
						return
					}
					...
					if (object.type == 'bank') {
						sendResult(socket, answer, 'loadsave_import_config:result', 'Cannot load single banks')
						return
					}
					...
					sendResult(socket, answer, 'loadsave_import_config:result', null, object)
				})
				...
				socket.on('reset_all', (answer) => {
					...
					sendResult(socket, answer, 'reset_all:result', null, 'ok')
				})
				...
				socket.on('loadsave_import_full', (data, answer) => {
					...
					sendResult(socket, answer, 'loadsave_import_full:result', null, 'ok')
				})
