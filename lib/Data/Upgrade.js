...
var debug    = require('debug')('lib/Data/Upgrade');
...
class DataUpgrade extends CoreBase {
	...
	checkDbVersion() {
		...
		if (res < 2 && db.length > 0) {
			this.upgradeV2(db);
		} else if (res > this.registry.getFileVersion()) {
			var dialog = require('electron').dialog;
			dialog.showErrorBox('Error starting companion', 'You have previously installed a newer version of companion. Since the configuration files are incompatible between major versions of companion, you need to remove the old config before continuing with this version.');
			process.exit(1);
		}
	}
