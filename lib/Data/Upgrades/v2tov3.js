...
function convertSchedulerToControls(db) {
	if (db.hasKey('scheduler')) {
		...
		Object.entries(scheduler).forEach(([id, entry], index) => {
			if (entry) {
				...
				const control = {
					type: 'trigger',
					config: cloneDeep(ControlTrigger.DefaultConfig),
				...
				control.config.name = entry.title
				control.config.enabled = !entry.disabled
				control.config.sortOrder = index
				...
				switch (entry.type) {
					...
					case 'instance':
						switch (entry.config.run) {
							...
							default:
								// TODO - logger
								console.warn(`Unable to upgrade trigger of unknown type: ${entry.config.run}`)
								control.events.push({
									id: nanoid(),
					...
					default:
						// TODO - logger
						console.warn(`Unable to upgrade trigger of unknown type: ${entry.type}`)
						control.events.push({
							id: nanoid(),
							type: entry.type,
							enabled: false,
