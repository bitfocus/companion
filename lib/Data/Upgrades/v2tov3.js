...
function convertTriggerToControl(entry, index) {
	const actions = []
	const control = {
		type: 'trigger',
...
function convertSchedulerToControls(db, logger) {
	if (db.hasKey('scheduler')) {
		...
		Object.entries(scheduler).forEach(([id, entry], index) => {
			if (entry) {
				controls[CreateTriggerControlId(id)] = convertTriggerToControl(entry, index)
			}
		})
...
function convertImportToSets(obj) {
	if (obj.type == 'full') {
		...
		if (obj.triggers) {
			newObj.triggers = obj.triggers.map(convertTriggerToControl)
		}
