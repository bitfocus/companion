...
class DataStoreBase extends CoreBase {
	...
	async doSave(withBackup) {
		...
		try {
			...
			this.debug(`${this.name}_save`, `Error renaming ${this.name}.tmp: ` + err)
	...
	save(withBackup = true) {
		if (this.saving === false) {
			this.debug(`${this.name}_save`, 'begin')
			this.saving = true
			...
			this.doSave(withBackup)
				.catch((err) => {
					try {
						...
						this.debug(`${this.name}_save`, 'Error reporting save failue: ' + err2)
					}
				})
				.then(() => {
