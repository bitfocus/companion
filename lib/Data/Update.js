...
const debug    = require('debug')('lib/Data/Update');
...
class Update extends CoreBase {

	constructor(registry) {
		super(registry, 'update');
		...
		const build = fs.readFileSync(this.registry.getAppRoot() + '/BUILD').toString();
		...
		fs.readFile(this.registry.getAppRoot() + '/package.json', 'utf8', (err, data) => {
			if (err) {
				throw err;
			}
