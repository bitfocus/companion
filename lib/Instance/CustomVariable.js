...
class InstanceCustomVariable extends CoreBase {
	constructor(registry, base) {
		...
		this.system.on('custom_variable_set_expression', (name, expression) => {
			if (this.custom_variables[name]) {
				...
				try {
					...
					this.setValue(name, resolve(temp, values))
				} catch (error) {
		...
		if (Object.keys(this.custom_variables).length > 0) {
			...
			this.base.set_variables('internal', newValues)
		}
	}

	setValue(name, value) {
		const fullname = `${custom_variable_prefix}${name}`
		this.base.set_variables('internal', {
			[fullname]: value,
		})
	}
...
exports = module.exports = InstanceCustomVariable
