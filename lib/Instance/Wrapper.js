...
class SocketEventsHandler {
	constructor(registry, instanceStatus, socket, connectionId) {
		this.debug = createDebug('instance-wrapper', `lib/Instance/Wrapper/${connectionId}`)
	...
	async #handleLogMessage(msg) {
		this.registry.log.add('instance(' + this.label + ')', msg.level, msg.message)
	}
	/**
	...
	async #handleSaveConfig(msg) {
		...
		this.registry.instance.setInstanceConfig(
			this.id,
			{
				...msg.config,
