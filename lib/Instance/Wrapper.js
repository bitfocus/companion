...
class SocketEventsHandler {
		...
		this.ipcWrapper = new IpcWrapper(
			{
				...
				updatePropertyValues: this.#handleUpdatePropertyValues.bind(this),
	...
	async init(config) {
		...
		this.usesProperties = !!msg.usesProperties
		...
		await this.ipcWrapper.sendWithCb('updateActions', {
			actions: {
				[action.id]: {
					...
					page: parsedId?.page ?? null,
					bank: parsedId?.bank ?? null,
				},
			},
		})
	}
	...
	/**
	 * Handle updating property values from the child process
	 */
	async #handleUpdatePropertyValues(msg) {
		if (!this.usesProperties) throw new Error('Module cannot use properties and variables simultaneously')

		const newVariables = {}

		for (const variable of msg.values) {
			if (variable.isSingleValue) {
				newVariables[variable.id] = { [null]: variable.values }
			} else {
				newVariables[variable.id] = variable.values
			}
		}

		this.registry.instance.variable.setVariableValues(this.label, newVariables)
	}
	...
	async #handleSetVariableValues(msg) {
		...
		for (const variable of msg.newValues) {
			variables[variable.id] = { [null]: variable.value }
		}
	...
	async #handleSetVariableDefinitions(msg) {
		...
		this.registry.instance.definitions.setPropertyDefinitions(this.connectionId, newProperties)
	...
	async #handleSetPropertyDefinitions(msg) {
		...
		for (const rawProperty of msg.properties || []) {
			properties[rawProperty.id] = {
				name: rawProperty.name,
				description: rawProperty.description,
				type: rawProperty.type,

				instanceIds: rawProperty.instanceIds ?? null,
		...
		this.registry.instance.definitions.setPropertyDefinitions(this.connectionId, properties)
	}
