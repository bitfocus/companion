...
class InstanceDefinitions extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('presets:preview_render', (/** @type {string } */ instanceId, /** @type {string } */ presetId) => {
	 ...
	 * @returns {import('../Resources/EventDefinitions.js').EventInstance | null}
	 */
	createEventItem(eventType) {
		const definition = EventDefinitions[eventType]
		if (definition) {
			/** @type {import('../Resources/EventDefinitions.js').EventInstance} */
			const event = {
				id: nanoid(),
				type: eventType,
				enabled: true,
	...
	importPresetToBank(connectionId, presetId, location) {
		const definition = cloneDeep(this.#presetDefinitions[connectionId]?.[presetId])
		if (definition) {
			if (definition.steps) {
				/** @type {any} */
				const newSteps = {}
			...
			if (definition.feedbacks) {
				for (let i = 0; i < definition.feedbacks.length; ++i) {
					...
					// @ts-ignore
					definition.feedbacks[i].instance_id = connectionId
				}
			} else {
			...
			if (!definition.options) {
				// This shouldnt be possible, but does happen
				definition.options = {}
			}

			this.controls.importControl(location, definition)
		}
	}
