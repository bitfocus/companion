...
class InstanceDefinitions extends CoreBase {
	...
	clientConnect(client) {
		client.on('presets:subscribe', (answer) => {
		...
		client.on('presets:unsubscribe', () => {
			client.leave(PresetsRoom)
		})

		client.on('action-definitions:subscribe', (answer) => {
			client.join(ActionsRoom)

			answer(this.actionDefinitions)
		})
		client.on('action-definitions:unsubscribe', (answer) => {
			client.leave(ActionsRoom)
		})

		client.on('feedback-definitions:subscribe', (answer) => {
			client.join(FeedbacksRoom)

			answer(this.feedbackDefinitions)
		})
		client.on('feedback-definitions:unsubscribe', (answer) => {
			client.leave(FeedbacksRoom)
		})

		client.on('presets:import_to_bank', this.importPresetToBank.bind(this))

		client.on('presets:preview_render', (instance_id, preset_id, answer) => {
