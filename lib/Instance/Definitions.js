...
const PropertiesRoom = 'property-definitions'
...
class InstanceDefinitions extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('property-definitions:subscribe', () => {
			client.join(PropertiesRoom)

			return this.#propertyDefinitions
		})
		client.onPromise('property-definitions:unsubscribe', () => {
			client.leave(PropertiesRoom)
		})
	...
	forgetInstance(instance_id) {
		...
		delete this.#propertyDefinitions[instance_id]
		this.io.emitToRoom(PropertiesRoom, 'property-definitions:update', instance_id, undefined)
	}
	 ...
	 * @access public
	 */
	getPropertyDefinition(instanceId, propertyId) {
		if (this.#propertyDefinitions[instanceId]) {
			return this.#propertyDefinitions[instanceId][propertyId]
		} else {
			return undefined
		}
	}
	...
	/**
	 * Set the property definitions for an instance
	 * @param {string} instanceId - the instance ID
	 * @param {object} properties - the property definitions
	 * @access public
	 */
	setPropertyDefinitions(instanceId, properties) {
		this.#propertyDefinitions[instanceId] = properties
