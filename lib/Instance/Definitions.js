...
class InstanceDefinitions extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('presets:preview_render', (instance_id, preset_id) => {
			...
			if (definition) {
				const render = this.graphics.drawPreview({
					...definition.style,
		...
		client.onPromise('action-definitions:learn-single', (actionObj) => {
			if (actionObj) {
				// TODO - this!
		...
		client.onPromise('feedback-definitions:learn-single', (feedbackObj) => {
			if (feedbackObj) {
				// TODO - this!
				// self.action_learn(actionObj).then((newOptions) => {
				// 	answer(newOptions)
	...
	setPresetDefinitions(instance_id, label, rawPresets) {
		...
		for (const [id, rawPreset] of Object.entries(rawPresets)) {
			const action_sets = {}
			for (const [setId, set] of Object.entries(rawPreset.actions)) {
