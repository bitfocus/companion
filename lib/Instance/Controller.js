...
class Instance extends CoreBase {
	constructor(registry) {
		...
		// this.moduleHost.queueStopAllConnections()
		...
		setTimeout(async () => {
			...
			const legacyCandidates = await Instance.#loadInfoForModulesInDir(path.join(rootPath, '/module-legacy/manifests'))
			...
			for (const searchDir of searchDirs) {
				const candidates = await Instance.#loadInfoForModulesInDir(searchDir)
	 ...
	 */
	static async #loadInfoForModulesInDir(searchDir) {
		if (await fs.pathExists(searchDir)) {
			...
			for (const candidate of candidates) {
				...
				ps.push(Instance.#loadInfoForModule(candidatePath))
			}
	 ...
	 * @param {string} fullpath - Fullpath to the module
	 */
	static async #loadInfoForModule(fullpath) {
		try {
