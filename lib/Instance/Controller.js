...
class Instance extends CoreBase {
	...
	powerStatusChange(event) {
		if (event == 'resume') {
			this.registry.log.add('system(power)', 'info', 'Resuming')
			...
			this.registry.log.add('system(power)', 'info', 'Suspending')
	...
	async initInstances(extraModulePath) {
		...
		for (const id of Object.keys(this.known_modules).sort()) {
			const moduleInfo = this.known_modules[id]
			this.log('debug', `${moduleInfo.display.id}@${moduleInfo.display.version}: ${moduleInfo.display.name}`)
		}
	...
	logInstance(instanceId, level, message) {
		try {
			...
			this.logger.silly(`instance(${instanceId}) ${level}: ${message}`)
		}
	}
	...
	activate_module(id, is_being_created) {
		...
		if (!moduleInfo) {
			this.logger.silly('Configured instance ' + config.instance_type + ' could not be loaded, unknown module')
			this.logInstance(
				id,
				'error',
			...
			this.moduleHost.queueRestartConnection(id, config, moduleInfo).catch((e) => {
				this.logger.silly('Configured instance ' + config.instance_type + ' failed to start: ' + e.message)
	...
	async #loadInfoForModule(fullpath) {
		try {
			...
			this.log('error', `Error loading module from "${fullpath}": ` + e)
		}
	}
}
