...
const { from15to32, rgb, rgbRev, sendResult, serializeIsVisibleFn } = require('../Resources/Util')
...
class Instance extends CoreBase {
	constructor(registry) {
		...
		this.moduleHost = new ModuleHost(registry)
		...
		setTimeout(async () => {
			...
			const searchDirs = [
				path.join(rootPath.toString(), '/module-tmp'),
		...
		this.system.on('instance_config_put', (id, config, skip_notify_instance) => {
			...
			if (!skip_notify_instance && this.asctive[id] !== undefined) {
	...
	calculateInstanceErrors() {
		...
		this.system.emit('instance_errorcount', [numOk, numWarn, numError, this.status])
	}
	...
	loadInfoForModule(fullpath) {
		try {
			const manifestJsonStr = fs.readFileSync(path.join(fullpath, 'companion/manifest.json'))
