...
class Instance extends CoreBase {
	 ...
	 * @param {boolean} clone
	 * @returns {import('../Data/Model/ExportModel.js').ExportInstanceFullv4 | import('../Data/Model/ExportModel.js').ExportInstanceMinimalv4}
	 */
	...
	clientConnect(client) {
		...
		client.onPromise('instances:edit', async (/** @type {string} */ id) => {
		...
		client.onPromise('instances:set-enabled', (/** @type {string} */ id, /** @type {boolean} */ state) => {
			this.enableDisableInstance(id, !!state)
		})

		client.onPromise('instances:delete', async (/** @type {string} */ id) => {
			await this.deleteInstance(id)
		})

		client.onPromise('instances:add', (/** @type {CreateConnectionData} */ module) => {
			const id = this.addInstance(module, false)
			return id
		})

		client.onPromise('instances:set-order', async (/** @type {string[]} */ instanceIds) => {
