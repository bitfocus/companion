...
class Instance extends CoreBase {
	...
	async initInstances(extraModulePath) {
		...
		for (const searchDir of searchDirs) {
			await processDir(searchDir, false)
		}

		if (extraModulePath) {
			this.logger.info(`Looking for extra modules in: ${extraModulePath}`)
			const foundCount = await processDir(path.resolve(extraModulePath), true)
			this.logger.info(`Found ${foundCount} extra modules`)
		}
		...
		client.onPromise('instances:edit', async (id) => {
			...
			if (instance) {
				try {
					...
					return {
						fields,
						label: this.store.db[id]?.label,
						config: this.store.db[id]?.config,
					}
				} catch (e) {
