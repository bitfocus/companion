...
class Instance extends CoreBase {
	constructor(registry) {
		...
		setTimeout(async () => {
			...
			const legacyCandidates = await this.loadInfoForModulesInDir(path.join(rootPath, '/module-legacy/manifests'), true)
	...
	async loadInfoForModulesInDir(searchDir, isLegacy) {
		if (await fs.pathExists(searchDir)) {
			...
			for (const candidate of candidates) {
				...
				ps.push(this.loadInfoForModule(candidatePath, isLegacy))
			}
	 ...
	 */
	async loadInfoForModule(fullpath, isLegacy) {
		try {
