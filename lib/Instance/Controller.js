...
class Instance extends CoreBase {
	constructor(registry) {
		...
		for (const [instanceId, instanceConfig] of Object.entries(this.store.db)) {
			// TODO - make this be a one shot rather than every launch
			if (instanceConfig.config === undefined) {
	...
	clientConnect(client) {
		...
		client.on('instance_edit', (id, answer) => {
			...
			if (instance) {
				instance
					.requestConfigFields()
					.then((fields) => {
						answer(id, fields, this.store.db[id]?.label, this.store.db[id]?.config)
					})
					.catch((e) => {
		...
		client.on('instance_config_put', (id, label, config, answer) => {
			for (const inst in this.store.db) {
				if (inst != id && this.store.db[inst].label == label) {
		...
		client.on('instance_delete', async (id) => {
