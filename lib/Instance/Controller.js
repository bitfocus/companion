...
class InstanceController extends CoreBase {
	...
	addInstance(data, cb) {
		...
		if (this.checkModuleLoaded(module)) {
			...
			this.io().emit('instance_add:result', id, this.instances);
	...
	clientConnect(client) {
		...
		client.on('instance_get', () => {
			client.emit('instance', this.instance, {
		...
		client.on('instance_enable', (id,state) => {
			...
			client.emit('instance_status', this.status);
			client.emit('instance', this.instances, {
				manufacturer: this.modules_manufacturer,
				category: this.modules_category,
				name: this.modules_name,
				config: this.modules_config
	...
	enableInstance(id, state) {
		this.system.emit('log', 'instance('+id+')', 'info', (state ? 'Enable' : 'Disable') + ' instance ' + this.instances[id].label);
	...
	init() {
		...
		for (var i = 0; i < module_folders.length; ++i) {
			...
			try {
				...
				this.modules_config.push(moduleconfig);
		...
		else if (this.instances['bitfocus-companion'] !== undefined) { // Bugfix of corrupted configs
	...
	saveInstanceConfig(id, config, internal) {
		...
		this.system.emit('instance_save');
		this.io().emit('instance_db_update', this.instances);
