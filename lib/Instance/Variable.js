...
const expressionFunctions = {
	...
	ceil: (v) => Math.ceil(v),
	// substr: (str, start, end) => { // TODO - needs more implementation work
...
class InstanceVariable extends CoreBase {
	...
	parseExpression(str, requiredType) {
		const getVariableValue = (variableId) => {
			const [label, variable] = SplitVariableId(variableId)
			return this.getVariableValue(label, variable)
		}

		const result = ResolveExpression(ParseExpression(str), getVariableValue, expressionFunctions)

		if (requiredType && typeof result !== requiredType) {
