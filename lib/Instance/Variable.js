...
const debug = debug0('variable', 'lib/Instance/Variable')
...
class InstanceVariable extends CoreBase {
	constructor(registry) {
		...
		// 	this.setVariableValues(instance.label, { [variable]: value })
		// })
	}

	getDefinitions() {
		// Note: old event name 'variable_get_definitions'
		return this.variable_definitions
	}

	getVariableValue(label, name) {
		// Note: old event name 'variable_get'
	...
	clientConnect(client) {
		...
		client.on('variable_values_for_instance', (label, answer) => {
			answer(this.variable_values[label])
		})
	}
