...
export function parseVariablesInString(string, rawVariableValues, cachedVariableValues) {
	...
	while ((matches = reg.exec(string))) {
		...
		const [instanceId, variableId, subId] = matches[1].split(':')
		referencedVariableIds.push(matches[1])
		...
		if (cachedVariableValues[fullId] === undefined) {
			...
			if (rawVariableValues[instanceId] && rawVariableValues[instanceId][variableId] !== undefined) {
				const rawValue = rawVariableValues[instanceId][variableId]?.[subId ?? null]
...
class InstanceVariable extends CoreBase {
	...
	getVariableValue(label, name, subId) {
		return this.variable_values[label]?.[name]?.[subId ?? null]
	}
	...
	#findAllVariableNamesForInstanceLabel(label) {
		const variableNames = []
		for (const [name, values] of Object.entries(this.variable_values[label] || {})) {
			if (values[null]) {
				variableNames.push(`${label}:${name}`)
			} else {
				for (const subId of Object.keys(values)) {
	...
	instanceLabelRename(labelFrom, labelTo) {
		...
		for (const fullName of newNames) {
			changedVariables[fullName] = this.getVariableValue(...fullName.split(':'))
		}
	...
	setVariableValues(label, variableValues) {
		...
		for (const [name, newValues] of Object.entries(variableValues || {})) {
			// We require all values of a property to be provided at once
			const oldValues = this.variable_values[label][name]
			this.variable_values[label][name] = newValues

			const allKeys = new Set([...Object.keys(oldValues || {}), ...Object.keys(newValues || {})])

			// TODO - using 'null' as a key is not working great...

			for (const subId of allKeys) {
				const oldValue = oldValues?.[subId]
				const newValue = newValues?.[subId]
				if (oldValue !== newValue) {
					const fullId = `${label}:${name}` + (subId === null || subId === 'null' ? '' : `:${subId}`)
					...
					if (!(label === 'internal' && name.startsWith('time_'))) {
