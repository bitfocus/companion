...
class InstanceVariable extends CoreBase {
	...
	forgetInstance(id, label) {
		if (label !== undefined) {
			if (this.variable_values[label] !== undefined) {
				...
				this.#emitVariablesChanged({}, removed_variables, new Set(removed_variables))
			}
	...
	instanceLabelRename(labelFrom, labelTo) {
		...
		if (this.variable_values[labelFrom] !== undefined) {
			...
			for (let variable in this.variable_values[labelFrom]) {
				...
				const fromId = `${labelFrom}:${variable}`
				removed_variables.push(fromId)
				all_changed_variables_set.add(fromId)

				const toId = `${labelTo}:${variable}`
				changed_variables[toId] = this.variable_values[labelTo][variable]
				all_changed_variables_set.add(toId)
			}

			delete this.variable_values[labelFrom]
			this.#emitVariablesChanged(changed_variables, removed_variables, all_changed_variables_set)
		}
	...
	setVariableValues(label, variables) {
		...
		for (const variable in variables) {
			...
			if (moduleValues[variable] !== value) {
				...
				const variable_id = `${label}:${variable}`
				all_changed_variables_set.add(variable_id)

				if (value === undefined) {
					removed_variables.push(variable_id)
				} else {
					changed_variables[variable_id] = value
				}
	...
	#emitVariablesChanged(changed_variables, removed_variables, all_changed_variables_set) {
		if (all_changed_variables_set.size > 0) {
			this.internalModule.variablesChanged(changed_variables, removed_variables, all_changed_variables_set)
			this.controls.onVariablesChanged(changed_variables, removed_variables, all_changed_variables_set)
			this.instance.moduleHost.onVariablesChanged(changed_variables, removed_variables, all_changed_variables_set)
		}
	}
}
