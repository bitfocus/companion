	...
	async queueRestartConnection(connectionId, config, moduleInfo) {
		...
		if (!child) {
			...
			child = {
				...
				logger: this.registry.logger.createLogger(`Instance/${config.label}`), // TODO - needs to update when label changes
			}
		...
		await child.lifeCycleQueue.add(async () => {
			if (config && config.enabled !== false) {
				...
				monitor.on('stdout', (data) => {
					this.logger.verbose(`Connection "${config.label}" stdout: ${data.toString()}`)
				})
				monitor.on('stderr', (data) => {
					this.logger.verbose(`Connection "${config.label}" stderr: ${data.toString()}`)
				})
