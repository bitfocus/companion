	...
	async queueRestartConnection(connectionId, config, moduleInfo) {
		...
		await child.lifeCycleQueue.add(async () => {
			if (config && config.enabled !== false) {
				...
				const moduleApiPath = isPackaged()
					? path.join(__dirname, 'ModuleInternalApi.js')
					: fileURLToPath(new URL('../../module-base-impl/dist/main.js', import.meta.url))
