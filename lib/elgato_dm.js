	...
	system.emit('io_get', function (io) {
		...
		system.on('io_connect', function (socket) {
			...
			socket.on('device_config_get', function (_id, answer) {
				for (var id in instances) {
					if (instances[id].id == _id) {
						sendResult(answer, 'device_config_get:result', null, instances[id].deviceHandler.getPanelConfig())
						return
					}
				}
			...
			socket.on('device_config_set', function (_id, config, answer) {
				for (var id in instances) {
					if (instances[id].id == _id) {
						instances[id].deviceHandler.setPanelConfig(config)
						answer(null, instances[id].deviceHandler.getPanelConfig())
						return
					}
				}
