...
elgatoDM.prototype.addDevice = function (device, type) {
	...
	if (type === 'elgatoEmulator') {
		...
		instances[device.path] = new deviceHandler(system, new satelliteDevice(system, device.path, device))

		self.updateDevicesList()
	} else if (type === 'satellite_device2') {
		instances[device.path] = new deviceHandler(system, new satelliteDeviceV2(system, device))
