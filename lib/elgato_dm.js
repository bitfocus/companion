...
elgatoDM.prototype.refreshDevices = function () {
	...
	for (var i = 0; i < devices.length; ++i) {
		(function (device) {
			...
			if (device.vendorId === 0x0fd9 && device.productId === 0x006c && instances[device.path] === undefined) {
			...
			if (device.vendorId === 0xffff && device.productId === 0x1f41 && instances[device.path] === undefined) {
				self.addDevice(device, 'infinitton');
			}
