...
function elgatoDM(_system) {
	...
	// Initial search for USB devices, if stream deck is not running on windows
	try {
		if (process.platform === 'win32') {
			findProcess('name', 'Stream Deck').then(function (list) {
				if (typeof list !== 'object' || list.length === 0) {
					...
					debug('Not scanning USB because we are on windows, and stream deck is running');
				}
			}).catch(function (err) {
				self.refreshDevices();
			});
		} else {
			debug('Not windows, initial USB scan');
			self.refreshDevices();
		}
	} catch (e) {
		// refresh usb anyways
...
elgatoDM.prototype.refreshDevices = function () {
	...
	for (var i = 0; i < devices.length; ++i) {
		(function (device) {
			...
			else if (device.vendorId === 0x0fd9 && device.productId === 0x0063 && instances[device.path] === undefined) {
				self.addDevice(device, 'elgato-mini');
			}
			else if (device.vendorId === 0x0fd9 && device.productId === 0x006c && instances[device.path] === undefined) {
				self.addDevice(device, 'elgato-xl');
			}
			else if (device.vendorId === 0x0fd9 && device.productId === 0x006d && instances[device.path] === undefined) {
...
elgatoDM.prototype.addDevice = function (device, type) {
	...
	if (type === 'elgatoEmulator') {
		...
		instances[device.path] = new satelliteDevice(system, device.path);
		instances[device.path].deviceHandler = new deviceHandler(system, instances[device.path]);
