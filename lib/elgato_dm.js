	...
	function scanUSB() {
		...
		for (const device of devices) {
			if (
				...
				!ignoreStreamDeck &&
				device.vendorId === 0x0fd9 &&
				device.productId === 0x0084 &&
				instances[device.path] === undefined
			) {
				self.addDevice(device, 'elgato-plus')
				...
				.then((devices) => {
					for (const device of devices) {
						if (instances[device.path] === undefined) {
							...
							} else if (device.model === loupedeck.LoupedeckModelId.LoupedeckLiveS) {
								self.addDevice(device, 'loupedeck-live-s', true)
							}
						}
					}
