...
class device_manager {
	...
	constructor(system) {
		...
		this.system.on('devices_list_get', (cb) => {
			...
			if (keys[0] != undefined) {
				ary.push({ id: this.instances[keys[0]].id, serialnumber: this.instances[keys[0]].serialnumber, type: insthis.instancestances[keys[0]].type, config: this.instances[keys[0]].config });
			}
