...
export default class Variables extends CoreBase {
	...
	constructor(registry, internalModule) {
		super(registry, 'internal', 'lib/Internal/Variables')
	...
	executeFeedback(feedback) {
		if (feedback.type == 'variable_value') {
			const id = feedback.options.variable.split(':', 2)

			const value = this.instance.variable.getVariableValue(id[0], id[1])

			return compareValues(feedback.options.op, value, feedback.options.value)
		} else if (feedback.type == 'variable_variable') {
			const id = feedback.options.variable.split(':', 2)
			const id2 = feedback.options.variable2.split(':', 2)
			const value = this.instance.variable.getVariableValue(id[0], id[1])
			const value2 = this.instance.variable.getVariableValue(id2[0], id2[1])
