...
export default class InternalController extends CoreBase {
	...
	setVariables(variables) {
		const variablesAsProperties = {}
		for (const [name, value] of Object.entries(variables)) {
			variablesAsProperties[name] = { [null]: value }
		}

		this.registry.instance.variable.setVariableValues('internal', variablesAsProperties)
	}
	...
	regenerateVariables() {
		...
		for (const fragment of this.fragments) {
			if (typeof fragment.getVariableDefinitions === 'function') {
				for (const variable of fragment.getVariableDefinitions()) {
		...
		this.registry.instance.definitions.setPropertyDefinitions('internal', properties)
	}
