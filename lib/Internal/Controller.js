...
export default class InternalController extends CoreBase {
	...
	init() {
		...
		for (const [controlId, control] of Object.entries(allControls)) {
			...
			if (control.actions && control.actions.actionReplace) {
				for (const action_set of Object.values(control.actions.action_sets)) {
					for (const action of action_set) {
						if (action.instance === 'internal') {
							// Try and run an upgrade
							const newAction = this.actionUpgrade(action)
							if (newAction) {
								control.actions.actionReplace(newAction)
							}
						}
					}
				}
		...
		for (const feedback of scheduler_feedbacks) {
			if (feedback.instance_id == 'internal') {
				this.feedbackUpdate(feedback, feedback.triggerId)
			}
		}

		// Find all the actions in triggers
		const scheduler_actions = this.triggers.getAllActions()
		for (const action of scheduler_actions) {
			if (action.instance == 'internal') {
				const newAction = this.actionUpgrade(action)
				if (newAction) {
					this.triggers.replaceActionItem(action.triggerId, newAction)
				}
			}
		}
