...
export default class InternalController extends CoreBase {
	 ...
	 * @returns {import('../Data/Model/ActionModel.js').ActionInstance | undefined} Updated action if any changes were made
	 */
	 ...
	 * @param {string} controlId
	 * @returns {import('../Data/Model/FeedbackModel.js').FeedbackInstance | undefined} Updated feedback if any changes were made
	 */
	 ...
	 * @param {import('../Data/Model/FeedbackModel.js').FeedbackInstance[]} feedbacks
	 */
		...
		for (const fragment of this.fragments) {
			if ('executeAction' in fragment && typeof fragment.executeAction === 'function') {
				try {
					...
					this.logger.warn(
						`Action execute failed: ${JSON.stringify(action)}(${JSON.stringify(extras)}) - ${e?.message ?? e} ${e?.stack
						}`
					)
				}
			}
