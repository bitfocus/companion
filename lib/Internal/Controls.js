...
export default class Controls extends CoreBase {
	...
	actionUpgrade(action, controlId) {
		...
		if (
			...
			action.action = 'button_pressrelease_if_expression'
			// TODO - what happens if value contains `$()` syntax?
			action.options.expression = `$(${action.options.variable}) ${op} "${action.options.value}"`
			delete action.options.variable
			delete action.options.op
