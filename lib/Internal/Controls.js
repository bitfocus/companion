...
/** @type {import('./Controller.js').InternalActionInputField} */
const CHOICES_PAGE = {
	type: 'internal:page',
	label: 'Page',
	id: 'page',
...
export default class Controls {
	...
	/**
	 * @returns {Record<string, import('./Controller.js').InternalActionDefinition>}
	 */
	...
	/**
	 * @param {import('../Instance/Definitions.js').FeedbackInstance} feedback
	 * @param {string} _controlId
	 * @returns {import('../Instance/Definitions.js').FeedbackInstance | void}
	 */
	feedbackUpgrade(feedback, _controlId) {
		...
		if (feedback.options.bank !== undefined) {
			...
			if (feedback.options.bank == 0) {
				...
				feedback.options.location_text = `${xy[1]}/${xy[0]}`
	 ...
	 * @param {import('./Controller.js').FeedbackInstanceExt} feedback
	 * @returns
	 */
	...
	executeAction(action, extras) {
		if (action.action === 'button_pressrelease') {
			...
			this.#controlsController.pressControl(theControlId, true, extras.deviceid, forcePress)
			this.#controlsController.pressControl(theControlId, false, extras.deviceid, forcePress)
			return true
			...
			const variable_value = this.#variableController.getVariableValue(instanceLabel, variableName)
			...
			this.#controlsController.pressControl(theControlId, true, extras.deviceid, !!action.options.force)
			return true
		} else if (action.action === 'button_release') {
			const { theControlId } = this.#fetchLocationAndControlId(action.options, extras.location, true)
			if (!theControlId) return true

			this.#controlsController.pressControl(theControlId, false, extras.deviceid, !!action.options.force)
			return true
		} else if (action.action === 'button_rotate_left') {
			const { theControlId } = this.#fetchLocationAndControlId(action.options, extras.location, true)
			if (!theControlId) return true

			this.#controlsController.rotateControl(theControlId, false, extras.deviceid)
			return true
		} else if (action.action === 'button_rotate_right') {
			const { theControlId } = this.#fetchLocationAndControlId(action.options, extras.location, true)
			if (!theControlId) return true

			this.#controlsController.rotateControl(theControlId, true, extras.deviceid)
			return true
	...
	/**
	 *
	 * @param {import('./Controller.js').InternalVisitor} visitor
	 * @param {import('../Instance/Definitions.js').ActionInstance[]} actions
