...
const previewButtonXyFn = ((options, info) => {
	...
	if (pageNumber === 0 || pageNumber === '0') pageNumber = info?.page ?? null
	if (options.x_use_this) theX = info?.x ?? null
	if (options.y_use_this) theY = info?.y ?? null
...
export default class Controls extends CoreBase {
	...
	getActionDefinitions() {
		return {
			...
			button_pressrelease_legacy: {
				label: 'Button: Trigger press and release (Deprecated)',
	...
	actionUpgrade(action, controlId) {
		...
		if (action.action === 'button_pressrelease') {
			if (action.options.bank_from_variable) {
				...
				changed = true
			} else if (action.options.x_value === undefined) {
