...
export default class Controls extends CoreBase {
	...
	#fetchLocationAndControlId(options, pressLocation, useVariableFields = false) {
		...
		const theControlId = result.location ? this.page.getControlIdAt(result.location) : null
	...
	executeFeedback(feedback) {
		if (feedback.type === 'bank_style') {
			...
			const render = theLocation && this.graphics.getBank(theLocation)
			if (render?.style) {
			...
			const control = theControlId && this.controls.getControl(theControlId)
			if (control) {
	...
	executeAction(action, extras) {
		if (action.action === 'button_pressrelease') {
			...
			this.controls.actions.abortPageDelayed(
				thePage,
			...
			if (controlId && ParseControlId(controlId)?.type === 'trigger') {
				this.controls.actions.abortControlDelayed(controlId, false)
			}
