		...
		switch (options.target) {
			...
			case 'text':
				if (useVariableFields) {
					const str = this.instance.variable.parseVariables(options.expression, injectedVariableValues).text

					theLocation = parseLocationString(str)
				} else {
					theLocation = null
				}
				break
			case 'expression':
				if (useVariableFields) {
					const str = this.instance.variable.parseExpression(options.expression, 'string', injectedVariableValues).value
