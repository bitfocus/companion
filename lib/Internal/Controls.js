...
import { formatCoordinate, oldBankIndexToCoordinate, oldBankIndexToXY, splitCoordinate } from '../Shared/ControlId.js'
...
const previewButtonXyFn = ((options, info) => {
	...
	if (options.x_mode === 'this' || options.x_use_this) theX = thisX ?? null
	if (options.y_mode === 'this' || options.y_use_this) theY = thisY ?? null
...
export default class Controls extends CoreBase {
	...
	#fetchPageAndButton(options, location) {
		...
		let theCoordinate = oldBankIndexToCoordinate(
			this.instance.variable.parseExpression(options.bank_variable, 'number').value
		)

		if (thePage === 0 || thePage === '0') thePage = location?.pageNumber ?? null
		if (theCoordinate === 0 || theCoordinate === '0') theCoordinate = location?.coordinate

		const theControlId = this.page.getControlIdAt(thePage, theCoordinate)
	...
	executeAction(action, location) {
		if (action.action === 'button_pressrelease') {
			...
			this.controls.actions.abortPageDelayed(
				thePage,
				action.options.ignoreSelf && location?.coordinate ? [location.coordinate] : undefined
			)
			return true
