...
export default class Instance {
	...
	executeAction(action, _extras) {
		if (action.action === 'instance_control') {
			...
			if (action.options.enable == 'toggle') {
				const curState = this.#instanceController.getInstanceStatus(action.options.instance_id)
	...
	updateVariables() {
		this.#internalModule.setVariables({
			instance_total: this.instance_total,
	...
	/**
	 *
	 * @param {Record<string, import('../Instance/Status.js').StatusEntry>} instance_statuses
	 * @returns {void}
	 */
	#calculateInstanceErrors(instance_statuses) {
		...
		this.#instanceStatuses = instance_statuses
		this.instance_total = numTotal
