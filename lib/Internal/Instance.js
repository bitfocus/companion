...
export default class Instance extends CoreBase {
	...
	executeAction(action) {
		if (action.action === 'instance_control') {
			this.registry.instance.enableDisableInstance(action.options.instance_id, opt.enable == 'true')
			return true
		}
	}
	...
	calculateInstanceErrors(instance_statuses) {
		...
		for (const i in instance_statuses) {
			...
			if (inn.category === 'good') {
				numOk++
			} else if (inn.category === 'warning') {
				numWarn++
			} else if (inn.category === 'error') {
				numError++
			}
		}
