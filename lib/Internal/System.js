...
export default class System extends CoreBase {
	...
	getActionDefinitions() {
		/** @type {Record<string, import('./Controller.js').InternalActionDefinition>} */
		const actions = {
			exec: {
				...
				exec(
					...
					(error, stdout, stderr) => {
						...
						if (action.options.targetVariable) {
							this.instance.variable.custom.setValue(action.options.targetVariable, stdout)
						}
					}
				)
			}
			return true
		} else if (action.action === 'app_restart' || action.action === 'app_exit') {
