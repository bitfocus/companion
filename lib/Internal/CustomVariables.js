...
export default class CustomVariables extends CoreBase {
	...
	actionUpgrade(action, controlId) {
		if (action.action2 === 'custom_variable_math_operation') {
			let op = '???'
			let reverse = false
			...
			const parts = [`$(${action.options.variable})`, op, `"${action.options.value}"`]
			...
			// TODO - encoding of this
			const wrappedValue = action.options.value
