...
export default class ActionRecorder extends CoreBase {
	...
	getActionDefinitions() {
		return {
			...
			action_recorder_save_to_button: {
				label: 'Action Recorder: Save to button',
				options: [
					{
					...
					{
						type: 'textwithvariables',
						label: 'Action Set (eg down, up, 1, 2)',
						tooltip: 'Press buttons use down/up, Stepped buttons use 1/2/3 etc',
						id: 'set',
						default: 'down',
					},
					{
	...
	executeAction(action, extras) {
		if (action.action === 'action_recorder_set_recording') {
			...
			const setId = this.instance.variable.parseVariables(action.options.set)
			...
			if (!isNaN(page) && !isNaN(bank) && setId) {
				if (page === 0) page = extras.page
				if (bank === 0) bank = extras.bank

				const controlId = CreateBankControlId(page, bank)
				console.log('save to', controlId, setId)

				try {
					this.registry.controls.actionRecorder.saveToControlId(controlId, setId, action.options.mode)
				} catch (e) {
