...
const CHOICES_BANK_WITH_VARIABLES = serializeIsVisibleFn([
	{
		type: 'checkbox',
		label: 'Use variables for bank',
...
export default class Bank extends CoreBase {
	...
	#fetchPageAndBank(options, info, useVariableFields) {
		let thePage = options.page
		let theBank = options.bank

		if (useVariableFields) {
			if (options.page_from_variable) {
				thePage = this.instance.variable.parseNumericExpression(options.page_variable)
			}
			if (options.bank_from_variable) {
				thePage = this.instance.variable.parseNumericExpression(options.bank_variable)
			}
		}
	...
	getActionDefinitions() {
		return {
			button_pressrelease: {
				...
				options: [...CHOICES_PAGE_WITH_VARIABLES, ...CHOICES_BANK_WITH_VARIABLES],
			},
	...
	getFeedbackDefinitions() {
		return {
			bank_style: {
				...
				previewBank: ['page', 'bank'],
				options: [CHOICES_PAGE, CHOICES_BANK],
			},
			bank_pushed: {
				type: 'boolean',
	...
	executeFeedback(feedback) {
		if (feedback.type === 'bank_style') {
			const { thePage, theBank, theControlId } = this.#fetchPageAndBank(feedback.options, feedback.info)
			...
			const { theControlId } = this.#fetchPageAndBank(feedback.options, feedback.info)
	...
	executeAction(action, extras) {
		if (action.action === 'button_pressrelease') {
			const { theControlId } = this.#fetchPageAndBank(action.options, extras, true)

			this.controls.pressControl(theControlId, true)
			this.controls.pressControl(theControlId, false)
			return true
		} else if ((action.action == 'button_pressrelease_condition', true)) {
			const { theControlId } = this.#fetchPageAndBank(action.options, extras, true)
			...
			const { thePage, theControlId } = this.#fetchPageAndBank(action.options, extras, true)
