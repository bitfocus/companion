	...
	#fetchPageAndBank(options, info, useVariableFields) {
		...
		if (useVariableFields) {
			if (options.page_from_variable) {
				thePage = this.instance.variable.parseExpression(options.page_variable, 'number')
			}
			if (options.bank_from_variable) {
				theBank = this.instance.variable.parseExpression(options.bank_variable, 'number')
			}
		}
