...
export default class Bank extends CoreBase {
	...
	executeFeedback(feedback) {
		if (feedback.type === 'bank_style') {
			...
			const control = this.controls.getControl(CreateBankControlId(thePage, theBank))
			if (control) {
				let isPushed = !!control.pushed

				if (!isPushed && feedback.options.latch_compatability && control.type === 'step') {
	...
	executeAction(action, extras) {
		if (action.action === 'button_pressrelease') {
			...
			this.controls.actions.abortControlDelayed(controlId, action.options.unlatch)
			return true
			...
			if (control && typeof control.actionSetAdvanceDelta === 'function') {
				control.actionSetMakeCurrent(action.options.step)
			}
			...
			if (control && typeof control.actionSetAdvanceDelta === 'function') {
				control.actionSetAdvanceDelta(action.options.amount)
			}
		}
	}
}
