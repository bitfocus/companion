...
export default class Bank extends CoreBase {
	...
	executeAction(action, extras) {
		if (action.action === 'button_pressrelease') {
			...
			const control = this.controls.getControl(theControlId)
			if (control && typeof control.configSetFields === 'function') {
				control.configSetFields({ bgcolor: action.options.color })
			}
			return true
			...
			const control = this.controls.getControl(theControlId)
			if (control && typeof control.configSetFields === 'function') {
				control.configSetFields({ color: action.options.color })
			}
			return true
			...
			const control = this.controls.getControl(theControlId)
			if (control && typeof control.configSetFields === 'function') {
				control.configSetFields({ text: action.options.label })
			}
