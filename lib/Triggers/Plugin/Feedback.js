const TriggersPluginBase = require('./Base')
const debug = require('debug')('lib/Trigger/Plugin/Feedback')
...
class TriggersPluginFeedback extends TriggersPluginBase {
	...
	add(id, data) {
		...
		for (const fb of ensure_array(data2.config)) {
			if (fb.instance_id === 'internal') {
				this.internalModule.feedbackUpdate(fb, id, undefined, undefined)
			} else {
				const instance = this.scheduler.instance.moduleHost.getChild(fb.instance_id)
				if (instance) {
					ps.push(instance.feedbackUpdate(fb, id, undefined, undefined))
				}
			}
		}
	...
	_cond_desc(feedback) {
		...
		let instanceLabel0
		this.scheduler.system.emit('instance_get_label', feedback.instance_id, (config) => {
			instanceLabel0 = config
		})
		const instanceLabel = instanceLabel0 || 'Missing'
...
module.exports = TriggersPluginFeedback
