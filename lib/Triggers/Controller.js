...
class TriggersController extends CoreBase {
	...
	action(id) {
		...
		if (event.actions) {
			...
			this.scheduler.bank.action.runMultipleActions(event.actions, triggerId, event.relative_delays ?? false, null)
		}

		// setTimeout(() => {
		// 	this.bank.action.pressBank(bank, button, false, 'scheduler')
		// 	this.logger.info(`Release button ${bank}.${button} via ${event.title}`)
	...
	test_actions(socket, title, actions, relative_delays) {
		...
		const triggerId = `trigger:test:${nanoid()}`
		this.scheduler.bank.action.runMultipleActions(actions, triggerId, relative_delays ?? false, null)
	}
