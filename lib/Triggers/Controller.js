...
class TriggersController extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('triggers:set-order', this.setTriggerOrder.bind(this))
	...
	load_plugins() {
		for (const plugin of Plugins) {
			try {
				...
				this.logger.silly(e)
				this.logger.error(`Error loading plugin ${plugin.constructor.name}`)
			}
		}
	}
	...
	save_schedule(socket, new_data) {
		...
		if (clean_data.sortOrder === undefined) {
			// try to preserve old
			clean_data.sortOrder = this.config[id]?.sortOrder
			if (clean_data.sortOrder === undefined) {
				clean_data.sortOrder = this.#getNextSortOrder()
			}
		}
	...
	async setTriggerOrder(triggerIds) {
		...
		const allKnownIds = Object.entries(this.config)
