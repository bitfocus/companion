...
/**
 * Rotate a square pixel buffer for the given orientation
 * @param {Buffer} buffer
...
export const rotateBuffer = (buffer, rotation) => {
	...
	let size = Math.sqrt(buffer.length / 3)
	...
	return Buffer.from(
		imageRs.ImageTransformer.fromBuffer(buffer, size, size, imageRs.PixelFormat.Rgb)
			.rotate(rotation2)
