...
class CloudController extends CoreBase {
	...
	constructor(registry) {
		...
		const uuid = this.clouddb.getKey('uuid')
		this.setState({ uuid })
	...
	getBanks(clone = false) {
		...
		const buttonControls = Object.values(allControls).filter((c) => c.type === 'button' && c.cloud !== true)

		const retval = buttonControls
			.map((control) => {
				const location = this.page.getLocationOfControlId(control.controlId)
				const bank = location ? xyToOldBankIndex(location.coordinate) : null
				if (bank) {
					return {
						page: location.pageNumber,
						// TODO - handle coordinates normaly
						bank: bank,
						data: {
							...control.toJSON(false).style,
							pushed: buttonControls.pushed,
							actions_running: buttonControls.has_actions_running,
							bank_status: buttonControls.bank_status,
							style: 'button',
						},
					}
				}
	 ...
	 * @access protected
	 */
	async handleCloudRegenerateUUID(client) {
		const newUuid = v4()
	 ...
	 * @access protected
	 */
	async updateBank(page, coordinate, render) {
		const bank = xyToOldBankIndex(...splitCoordinate(coordinate))
		if (render.style?.cloud !== true && bank) {
			const updateId = v4()
			for (let region in this.regionInstances) {
				if (this.regionInstances[region]?.socketTransmit) {
					this.regionInstances[region].socketTransmit('companion-banks:' + this.state.uuid, {
