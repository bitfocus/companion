...
class CloudController extends CoreBase {
	...
	getBanks(clone = false) {
		const buttonControls = Object.values(this.controls.getAllControls()).filter((c) => c.type === 'button')
		return buttonControls.map((control) => {
			const controlInfo = ParseControlId(control.controlId)
			return { page: controlInfo.page, bank: controlInfo.bank, style: control.toJSON(false).style }
		})
	}
	...
	async handleCloudInfrastructureRefresh() {
		try {
			const response = await got.get(CLOUD_URL + '/infrastructure/companion', {
				headers: {
	...
	readConnections(connections) {
		...
		if (connections) {
			for (let region in connections) {
				this.logger.silly(`Has region: ${region}`, region in this.regions)
