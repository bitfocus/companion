...
class CloudRegion {
	...
	async clientPushBank(args) {
		...
		if (args.bank && args.page) {
			const control = this.controls.getControl(CreateBankControlId(parseInt(args.page), parseInt(args.bank)))
			if (control) control.pressControl(true, 'cloud')
		}
		return true
	}
	...
	async clientReleaseBank(args) {
		...
		if (args.bank && args.page) {
			const control = this.controls.getControl(CreateBankControlId(parseInt(args.page), parseInt(args.bank)))
			if (control) control.pressControl(false, 'cloud')
		}
		return true
	}
