...
class CloudRegion {
	...
	async clientPushBank(args) {
		...
		if (args.bank && args.page) {
			const controlId = this.page.getControlIdAt(args.page, oldBankIndexToCoordinate(args.bank))
			if (controlId) {
				this.cloud.registry?.controls?.pressControl(controlId, true, 'cloud')
			}
		}
		return true
	}
	...
	async clientReleaseBank(args) {
		...
		if (args.bank && args.page) {
			const controlId = this.page.getControlIdAt(args.page, oldBankIndexToCoordinate(args.bank))
			if (controlId) {
				this.cloud.registry?.controls?.pressControl(controlId, false, 'cloud')
			}
		}
		return true
	}
