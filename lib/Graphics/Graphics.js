...
const debug         = require('debug')('lib/Graphics/Graphics');
		...
		if (c.style !== 'pageup' && c.style !== 'pagedown' && c.style !== 'pagenum') {
			...
			if (c.style == 'pageup') {
				...
				if (this.userconfig().getKey('page_plusminus')) {
					img.drawLetter(30, 20, this.userconfig().getKey('page_direction_flipped') ? '-' : '+', Image.rgb(255,255,255), 0, 1);
				}
				else {
				...
				if (this.userconfig().getKey('page_plusminus')) {
					img.drawLetter(30, 40, this.userconfig().getKey('page_direction_flipped') ? '+' : '-', Image.rgb(255,255,255), 0, 1);
				}
				else {
