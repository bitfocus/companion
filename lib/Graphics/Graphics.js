...
var debug    = require('debug')('lib/Graphics/Graphics');
...
class Graphics {
	...
	constructor(registry) {
		...
		this.actions = registry.actions;
		this.feedback = registry.feedback;

		this.buffers = {};
		this.page_direction_flipped = this.registry.userconfig.getKey('page_direction_flipped');
		this.page_plusminus = this.registry.userconfig.getKey('page_plusminus');
	...
	drawBankImage(c, page, bank) {
		...
		if (c.style !== 'pageup' && c.style !== 'pagedown' && c.style !== 'pagenum') {
			...
			if (c.style == 'pageup') {
				...
				if (this.page_plusminus) {
					img.drawLetter(30, 20, this.page_direction_flipped ? '-' : '+', Image.rgb(255,255,255), 0, 1);
				}
				else {
				...
				if (this.page_plusminus) {
					img.drawLetter(30, 40, this.page_direction_flipped ? '+' : '-', Image.rgb(255,255,255), 0, 1);
				}
				else {
		...
		if (page === undefined) { // Preview (no page/bank)
			...
			img.drawTextLine(3, 3, "x.x", Image.rgb(255, 198, 0), 0);
		}
		else if (this.pushed[page+'_'+bank] !== undefined) { // Pushed
			if (c.style !== 'pageup' && c.style !== 'pagedown' && c.style !== 'pagenum') {
				img.boxFilled(0, 0, 71, 14, Image.rgb(255, 198, 0));
				img.drawTextLine(3, 3, page + "." + bank, Image.rgb(0, 0, 0), 0);
			}
		}
		else { // not pushed
			if (c.style !== 'pageup' && c.style !== 'pagedown' && c.style !== 'pagenum') {
				img.drawTextLine(3, 3, page + "." + bank, Image.rgb(255, 198, 0), 0);
			}
		}

		if (!notStatic) {
			let colors = [0, Image.rgb(255, 127, 0), Image.rgb(255, 0, 0)];
			let status = this.actions.getBankStatus(page, bank);

			if (status > 0) {
				img.boxFilled(62, 2, 70, 10, colors[status]);
			}

			if (this.actions.getRunningActions(page, bank)) {
	...
	getImagesForPincode(pincode) {
		...
		if (!(pincode === undefined)) {
			img.drawAlignedText(0,15,72,72,pincode.replace(/[a-z0-9]/gi, '*'),Image.rgb(255,255,255),18,undefined,44,'center','center');
		}
	...
	getPageButton(page) {
		...
		img.backgroundColor(Image.rgb(15,15,15));
		img.drawAlignedText(0,0,72,30,(this.page[page] !== undefined ? this.page[page].name : ''),Image.rgb(255,198,0),0,undefined,1,'center','bottom');
		img.drawAlignedText(0,32,72,30,""+page,Image.rgb(255,255,255),18,undefined,1,'center','top');
		return img;
	}
