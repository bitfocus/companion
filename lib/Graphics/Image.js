...
class Image {
	...
	boxLine(x1, y1, x2, y2, color, lineWidth = 1, lineOrientation = 'inside') {
		...
		console.log('boxline', x1, y1, x2-x1, y2-y1, lineWidth)
	 ...
	 * @param {string} text 
	 * @param {number} color CSS color string
	 * @param {number} fontsize Em height
	...
	drawAlignedText(
		...
		if (isNaN(fontheight)) {
			fontsize = 'auto'
		} else if (fontheight < 3 ) { // block out some tiny fontsizes
			fontheight = 3
		} else if (fontheight > 120 ) { // block out some giant fontsizes
			fontheight = 120
		}
		...
		if (fontsize === 'auto') {
			...
			fontheight = checksize.find(size => this.drawAlignedText(x, y, w, h, displayText, color, size, halign, valign, true) === true) ?? 6
		...
		this.context2d.font = `${fontheight}px ${fontfamily}`
