...
class Image {
	...
	boxFilled(x1, y1, x2, y2, color = undefined, strokeColor = undefined, lineWidth = 1, lineOrientation = 'inside') {
	 ...
	 * @returns 
	 */
	drawTextLine(x, y, text, color, fontsize, spacing, double, dummy) {
		...
		if (parseInt(fontsize) == NaN) return 0
	...
	drawAlignedText(
		...
		if (isNaN(fontsize)) {
			fontindex = 'auto'
		} else if (fontsize < 3 ) { // block out some tiny fontsizes
			fontsize = 3
		} else if (fontsize > 120 ) { // block out some giant fontsizes
			fontsize = 120
		}
		...
		if (fontindex === 'auto') {
			...
			const area = w * h / 5000
			if (len < 7 * area) { checksize = [60, 51, 44, 31, 24, 20, 17, 15, 12, 10, 9, 8, 7] }
			else if (len < 30 * area) { checksize = [31, 24, 20,  17, 15, 12, 10, 9, 8, 7] }
			else if (len < 40 * area) { checksize = [24, 20, 17, 15, 12, 10, 9, 8, 7] }
			else if (len < 50 * area) { checksize = [17, 15, 12, 10, 9, 8, 7] }
			else { checksize = [15, 12, 10, 9, 8, 7] }

			fontsize = checksize.find(size => this.drawAlignedText(x, y, w, h, displayText, color, size, spacing, size, halign, valign, true) === true) ?? 6
		...
		// this is necessary to get unicode compatible substring
		...
		const findLastChar = (text) => {
			...
			if (diff >= 0) {
				return { ascent: measure.actualBoundingBoxAscent, descent: measure.actualBoundingBoxDescent, maxCodepoints: length }
			}
			...
			if (Math.abs(diff) > nWidth) { // we seem to be off by more than one char
			...
			for (let i = 0; i <= length; i +=1) {
				if (diff > 0 && w - this.context2d.measureText(substring(text, 0, chars+1)).width < 0) { // we are smaller and next char is too big
					...
					return { ascent: measure.actualBoundingBoxAscent, descent: measure.actualBoundingBoxDescent, maxCodepoints: chars }
				} else if (diff < 0 && w - this.context2d.measureText(substring(text, 0, chars-1)).width > 0) { // we are bigger and one less char fits
					...
					return { ascent: measure.actualBoundingBoxAscent, descent: measure.actualBoundingBoxDescent, maxCodepoints: chars-1 }
				} else if (diff == 0) { // perfect match
					//console.log('line algo says perfect match with '+chars+' chars', substring(text, 0, chars));
					return { ascent: measure.actualBoundingBoxAscent, descent: measure.actualBoundingBoxDescent, maxCodepoints: chars }
				} else { // our assumed nWidth was too big, let's approach now char by char
					if (diff > 0) {
		...
		while (lastDrawnByte < textArr.length) {
			...
			if (maxCodepoints >= (textArr.length - lastDrawnByte)) {
				...
				if (line[breakPos] === ' ') { // get rid of a breaking space at end
					lines.push({
	...
	buffer() {
		...
		const buffer = new Buffer.from(this.context2d.getImageData(0, 0, this.width, this.height).data)
	...
	static emptyAndTime() {
		return { updated: Date.now(), buffer: Buffer.alloc(this.width * this.height * 3) }
	}
}
