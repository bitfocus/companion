...
class Image {
	...
	pixel(x, y, color) {
		...
		const imageData = this.context2d.createImageData(1, 1)
		imageData.data[0] = col.r
		imageData.data[1] = col.g
		imageData.data[2] = col.b
		imageData.data[3] = 255 // TODO: fix transparency
	...
	drawTextLine(x, y, text, color, fontindex, spacing, double, dummy) {
		...
		if (fontindex == 0 || fontindex == 7) fontindex = 8
		this.context2d.font = `${fontindex}px`
		...
		if (!dummy) {
			...
			if (fontindex < 12) { 
  			...
  			this.context2d.fill(textPath.offset(x, y + lineheight))
				...
				this.context2d.fillText(text, x, y + lineheight)
			}
		}
	...
	drawAlignedText(
		...
		lineheight = Math.floor(fontindex * 1.1)
		...
		const measure = (text) => {
			...
			//console.log(`\nmeasure: ${text} lenght: ${text.length} font: ${fontindex}\n`, metrics)
			return {
		...
		while (lastDrawnByte < textArr.length) {
			...
			const { maxBytes, ascent, descent } = measure(byteToString(textArr.slice(lastDrawnByte)))

			//console.log(`check text "${byteToString(textArr.slice(lastDrawnByte))}" lenght=${textArr.length - lastDrawnByte} max=${maxBytes}`)
			...
			if (maxBytes >= (textArr.length - lastDrawnByte)) {
				...
				const line = textArr.slice(lastDrawnByte, lastDrawnByte + maxBytes)
				...
				//console.log('breakpos', breakPos, 'of', line.length)
		...
		let totalHeight = lines[0].ascent + lines[lines.length - 1].descent + (lines.length > 0 ? (lines.length - 1 ) * lineheight : 0 )
		if (dummy) {
		...
		switch (valign) {
			case 'top':
				yAnchor = lines[0].ascent
				break
			case 'center':
				yAnchor = (h - totalHeight) / 2 + lines[0].ascent
				break
			case 'bottom':
				yAnchor = h - totalHeight + lines[0].ascent
				break
		}
		yAnchor += y
	...
	/**
	 * Draws a single char from one of Companions internal fonts
	 * DEPRECATED
	 * @param {*} x 
	 * @param {*} y 
	 * @param {*} char 
	...
	drawFilledPath(pathPoints, color) {
		...
		let col = rgbRev(color)
		this.context2d.fillStyle = `rgba(${col.r}, ${col.g}, ${col.b}, 1)`
		this.context2d.fill(path, "evenodd");
		this.lastUpdate = Date.now()
	}
	...
	buffer() {
		...
		return this.context2d.getImageData(0, 0, this.width, this.height).data.filter((_elem, index) => { return (index + 1) % 4 })
	}
