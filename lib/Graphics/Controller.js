...
class GraphicsController extends CoreBase {
	...
	constructor(registry) {
		...
		this.system.on('graphics_get', (cb) => {
	...
	drawBankImage(c, page, bank) {
		...
		if (c.style == 'pageup') {
			...
			} else if (this.pages[page] === undefined || this.pages[page].name === 'PAGE' || this.pages[page].name === '') {
				img.drawAlignedText(0, 0, 72, 30, 'PAGE', colorButtonYellow, 0, undefined, 1, 'center', 'bottom')
				img.drawAlignedText(0, 32, 72, 30, '' + page, colorWhite, 18, undefined, 1, 'center', 'top')
			} else {
				let pagename = this.pages[page].name
	...
	drawBank(page, bank) {
		...
		if (
			...
			let c = cloneDeep(this.config[page][bank])
