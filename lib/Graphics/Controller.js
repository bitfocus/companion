...
function getIdForLocation(location) {
	return `${location.pageNumber}_${location.column}_${location.row}`
}
...
class GraphicsController extends CoreBase {
	...
	#drawAndCacheBank(location) {
		const imageId = location ? getIdForLocation(location) : undefined
		...
		if (location) {
			setImmediate(() => {
				const values = {
					[`b_text_${location.pageNumber}_${location.column}_${location.row}`]: variableValue,
				}
	...
	getBank(location) {
		const renderId = getIdForLocation(location)
