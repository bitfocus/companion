...
class GraphicsController extends CoreBase {
	...
	drawTopbar(img, c, page, bank) {
		...
		if (page !== undefined || bank !== undefined) {
			let status = this.bank.action.getBankStatus(page, bank)
	...
	drawBank(page, bank) {
		...
		if (
			...
			let style = this.bank.feedback.getStyle(page, bank)
			...
			if (c.style == 'step') {
				c['step_cycle'] = parseInt(this.bank.action.getBankActiveStep(page, bank)) + 1
			}
