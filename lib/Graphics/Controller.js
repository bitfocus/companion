...
class GraphicsController extends CoreBase {
			...
			async (_id, location, skipInvalidation) => {
				try {
					...
					if (location && locationIsInBounds && buttonStyle && buttonStyle.style) {
						...
						if (!render) {
							const { buffer, dataUrl, draw_style } = await this.#pool.exec('drawBankImage', [
								this.#drawOptions,
								buttonStyle,
								location,
								pagename,
							])
							render = GraphicsRenderer.wrapDrawButtonImage(buffer, dataUrl, draw_style, buttonStyle)
						}
					} else {
	...
	async drawPreview(buttonStyle) {
		...
		const { buffer, dataUrl, draw_style } = await this.#pool.exec('drawBankImage', [this.#drawOptions, drawStyle])
