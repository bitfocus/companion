...
import { splitCoordinate, xyToOldBankIndex } from '../Shared/ControlId.js'
...
class GraphicsController extends CoreBase {
	...
	invalidateControl(controlId) {
		...
		if (location) {
			this.invalidateBank(location.pageNumber, location.coordinate)
		}
	}

	invalidateBank(page, coordinate) {
		const render = this.#drawAndCacheBank(page, coordinate)

		this.emit('bank_invalidated', page, coordinate, render)
	}
	...
	regenerateAll(invalidate) {
		for (let pageNumber = 1; pageNumber <= 99; pageNumber++) {
			const populatedCoordinates = this.page.getAllPopulatedCoordinates(pageNumber)
			for (const coordinate of populatedCoordinates) {
				const render = this.#drawAndCacheBank(pageNumber, coordinate)

				if (invalidate) {
					this.emit('bank_invalidated', pageNumber, coordinate, render)
				}
			}
		}
	}

	#drawAndCacheBank(page, coordinate) {
		page = Number(page)

		const imageId = page !== undefined && coordinate !== undefined ? `${page}_${coordinate}` : undefined
		...
		setImmediate(() => {
			const values = {
				[`b_text_${page}_${coordinate}`]: variableValue,
			}

			const xy = splitCoordinate(coordinate)
			const bank = xyToOldBankIndex(...xy)
			if (bank) values[`b_text_${page}_${bank}`] = variableValue
		...
		if (buttonStyle && buttonStyle.style) {
			...
			const render = GraphicsRenderer.drawBankImage(this.draw_options, buttonStyle, page, coordinate, pagename)
			this.renders[imageId] = render

			return render
		} else {
			const render = GraphicsRenderer.drawBlank(this.draw_options, page, coordinate)
	...
	getBank(page, coordinate) {
		let render = this.renders[page + '_' + coordinate]
		if (render) return render

		render = this.#drawAndCacheBank(page, coordinate)
		if (render) {
			this.emit('bank_invalidated', page, coordinate, render)
			return render
		}
		...
		this.logger.silly(
			'!!!! ERROR: UNEXPECTED ERROR while fetching image for unbuffered bank: ' + page + '-' + coordinate
		)
