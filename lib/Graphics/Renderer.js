...
export default class GraphicsRenderer {
	...
	static drawBlank(options, page, bank) {
		...
		if (!options.remove_topbar) {
			img.drawTextLine(2, 3, `${page}.${bank}`, img.rgb(50, 50, 50), 8)
	...
	static drawBankImage(options, bankStyle, page, bank, pagename) {
		...
		if (bankStyle.style == 'pageup') {
			...
			if (options.page_plusminus) {
				...
				img.line(36, 20, 46, 30, colorWhite, 2)
				img.line(36, 20, 26, 30, colorWhite, 2)
			}
			...
			if (options.page_plusminus) {
				...
				img.line(36, 50, 46, 40, colorWhite, 2)
				img.line(36, 50, 26, 40, colorWhite, 2)
			}
			...
			if (!show_topbar) {
				img.drawAlignedText(2, 1, 68, 70, bankStyle.text, bankStyle.color, bankStyle.size, 2, 1, halign, valign)
			} else {
				img.drawAlignedText(2, 15, 68, 57, bankStyle.text, bankStyle.color, bankStyle.size, 2, 1, halign, valign)
			}
	...
	static #drawTopbar(img, show_topbar, bankStyle, page, bank) {
		...
		if (page !== undefined || bank !== undefined) {
			...
			if (bankStyle.action_running) {
				//img.drawTextLine(55, 3, '►', img.rgb(0, 255, 0), 8) // not as nice
				img.drawFilledPath([[55,3], [61,7], [55,11]], img.rgb(0, 255, 0))
			}
		}
