...
export default class GraphicsRenderer {
	...
	/**
	 * Draw the image for a bank or retrieve it
	 * from the cache if it has been used recently
	 ...
	 * @returns Image render object
	 */
	static drawBankImage(options, bankStyle, location, pagename) {
		const globalShowTopBar = !options.remove_topbar && bankStyle.show_topbar === 'default'
		const keyLocation = globalShowTopBar || bankStyle.show_topbar === true ? location : undefined
		const key = JSON.stringify({ options, bankStyle, keyLocation })
		...
		const result = this.#drawUncachedBankImage(options, bankStyle, location, pagename)
	 ...
	 * @returns Image render object
	 */
	static #drawUncachedBankImage(options, bankStyle, location, pagename) {
