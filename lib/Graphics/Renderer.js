...
export default class GraphicsRenderer {
	...
	static drawBlank(options, location) {
		...
		return new ImageResult(img.buffer(), img.toDataURLSync(), undefined)
	}

	/**
	 * Draw the image for a button
	 * @param {import('./Controller.js').GraphicsOptions} options
	 * @param {import('../Data/Model/StyleModel.js').DrawStyleModel} drawStyle The style to draw
	...
	static async drawButtonImage(options, drawStyle, location, pagename) {
		const { buffer, dataUrl, draw_style } = await GraphicsRenderer.drawButtonImageUnwrapped(
			options,
			drawStyle,
			location,
			pagename
		...
		return GraphicsRenderer.wrapDrawButtonImage(buffer, dataUrl, draw_style, drawStyle)
	}
	 ...
	 * @param {import('../Data/Model/StyleModel.js').DrawStyleModel} drawStyle
	 * @returns
	 */
	static wrapDrawButtonImage(buffer, dataUrl, draw_style, drawStyle) {
		const draw_style2 = draw_style === 'button' ? (drawStyle.style === 'button' ? drawStyle : undefined) : draw_style

		return new ImageResult(buffer, dataUrl, draw_style2)
	}
	 ...
	 * @param {string | undefined} pagename
	 * @access public
	 * @returns {Promise<{ buffer: Buffer, dataUrl: string, draw_style: import('../Data/Model/StyleModel.js').DrawStyleModel['style'] | undefined}>} Image render object
	 */
	 ...
	 * @param {import('./Controller.js').GraphicsOptions} options
	 * @param {import('../Data/Model/StyleModel.js').DrawStyleButtonModel} drawStyle The style to draw
	 * @param {import('../Resources/Util.js').ControlLocation | undefined} location
	 * @access private
	 */
