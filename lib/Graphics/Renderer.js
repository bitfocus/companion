...
export default class GraphicsRenderer {
	 ...
	 */
	static drawBlank(options, pageNumber, coordinate) {
		...
		if (!options.remove_topbar) {
			img.drawTextLine(2, 3, `${pageNumber}.${coordinate}`, 'rgb(50, 50, 50)', 8)
	 ...
	 * @param {number | undefined} coordinate
	 * @param {string | undefined} pagename
	 * @access public
	 * @returns Image render object
	 */
	static drawBankImage(options, bankStyle, pageNumber, coordinate, pagename) {
		...
		if (bankStyle.style == 'pageup') {
			...
			if (pageNumber === undefined) {
				...
				img.drawTextLineAligned(36, 32, '' + pageNumber, colorWhite, 18, 'center', 'top')
			} else {
			...
			if (bankStyle.png64 !== undefined && bankStyle.png64 !== null) {
				try {
					...
					GraphicsRenderer.#drawTopbar(img, show_topbar, bankStyle, pageNumber, coordinate)
					return {
						buffer: img.buffer(),
						updated: Date.now(),
						style: draw_style,
	 ...
	 * @access private
	 */
	static #drawTopbar(img, show_topbar, bankStyle, pageNumber, coordinate) {
		if (!show_topbar) {
			if (bankStyle.pushed) {
			...
			if (typeof bankStyle.step_cycle === 'number' && pageNumber !== undefined) {
			...
			if (pageNumber === undefined) {
				...
				img.drawTextLine(4, 2, `${pageNumber}.${coordinate}${step}`, colorBlack, 9)
			} else {
				img.drawTextLine(4, 2, `${pageNumber}.${coordinate}${step}`, colorButtonYellow, 9)
			}
		}
		...
		if (bankStyle.cloud && show_topbar) {
			img.drawPixelBuffer(rightMax - 17, 3, 15, 8, internalIcons.cloud, 'base64')
			rightMax -= 17
		}

		// next error or warning icon
		if (pageNumber !== undefined || coordinate !== undefined) {
