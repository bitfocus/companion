...
export default class GraphicsRenderer {
	...
	static async drawButtonImage(options, drawStyle, location, pagename) {
		const { buffer, draw_style } = await GraphicsRenderer.drawButtonImageUnwrapped(
			options,
			drawStyle,
			location,
			pagename
		...
		return GraphicsRenderer.wrapDrawButtonImage(buffer, draw_style, drawStyle)
	}
	 ...
	 * @returns
	 */
	static wrapDrawButtonImage(buffer, draw_style, drawStyle) {
