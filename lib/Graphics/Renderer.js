...
export default class GraphicsRenderer {
	...
	static drawBlank(options, location) {
		...
		if (!options.remove_topbar) {
			img.drawTextLine(2, 3, `${location.pageNumber}/${location.column}/${location.row}`, 'rgb(50, 50, 50)', 8)
	...
	static #drawTopbar(img, show_topbar, bankStyle, location) {
		if (!show_topbar) {
			...
			if (location === undefined) {
				...
				img.drawTextLine(4, 2, `${location.pageNumber}/${location.column}/${location.row}${step}`, colorBlack, 9)
			} else {
				img.drawTextLine(4, 2, `${location.pageNumber}/${location.column}/${location.row}${step}`, colorButtonYellow, 9)
			}
		}
