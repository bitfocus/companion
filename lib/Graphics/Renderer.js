	...
	static drawBankImage(options, bankStyle, page, bank, pagename) {
		...
		if (bankStyle.style == 'pageup') {
			...
			if (options.page_plusminus) {
				...
				img.drawPath([[46,30], [36,20], [26,30]], colorWhite, 2) // Arrow up path
			}
			...
			if (options.page_plusminus) {
				...
				img.drawPath([[46,40], [36,50], [26,40]], colorWhite, 2) // Arrow down path
			}
			...
			!show_topbar ? img.box(0, 0, 72, 72, parseColor(bankStyle.bgcolor)) : img.box(0, 14, 72, 72, parseColor(bankStyle.bgcolor))
	...
	static #drawTopbar(img, show_topbar, bankStyle, page, bank) {
		if (!show_topbar) {
			...
			if (page === undefined) {
				...
				img.box(0, 0, 71, 14, colorButtonYellow)
		...
		if (page !== undefined || bank !== undefined) {
			...
			if (statusColour) {
				img.box(62, 2, 70, 10, statusColour)
			}
