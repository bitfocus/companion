import { formatCoordinate } from '../Shared/ControlId.js'
...
class GraphicsPreview extends CoreBase {
	 ...
	 * @access public
	 */
	updateButton(page, coordinate, render) {
		...
		if (this.io.countRoomMembers(previewRoom) > 0) {
			this.io.emitToRoom(previewRoom, `preview:page-bank`, page, coordinate, render.buffer)
		}

		const previewRoom2 = PreviewBankRoom(page, coordinate)
		if (this.io.countRoomMembers(previewRoom2) > 0) {
			this.io.emitToRoom(previewRoom2, `preview:page-bank`, page, coordinate, render.buffer)
		}
	}
}
