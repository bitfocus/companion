...
class GraphicsPreview extends CoreBase {
	...
	handlePreviewPageSubscribe(client, page) {
		...
		for (let i = 0; i < global.MAX_BUTTONS; ++i) {
			result[i + 1] = this.graphics.getBank(page, i + 1)?.buffer
		}
	...
	updateBank(page, bank, render) {
		...
		this.io.emitToRoom(WebButtonsRoom, 'web-buttons:bank-data', page, bank, render.buffer)

		// Send to bank-grid preview clients
		this.io.emitToRoom(PagePreviewRoom(page), 'page-preview:bank-data', page, bank, render.buffer)
	}
