...
class GraphicsPreview extends CoreBase {
	...
	clientConnect(client) {
		client.onPromise('preview:page:subscribe', (page) => {
			...
			for (let i = 1; i <= global.MAX_BUTTONS; ++i) {
				const controlId = CreateBankControlId(page, i)
				renders[controlId] = this.graphics.getBank(page, i).buffer
			}
		...
		client.onPromise('preview:bank:subscribe', (controlId) => {
			const parsed = ParseControlId(controlId)
			if (parsed?.type !== 'bank') throw new Error('Invalid controlId')

			client.join(PreviewBankRoom(parsed.page, parsed.bank))

			return this.graphics.getBank(parsed.page, parsed.bank).buffer
		})
		client.onPromise('preview:bank:unsubscribe', (controlId) => {
			const parsed = ParseControlId(controlId)
			if (parsed?.type !== 'bank') throw new Error('Invalid controlId')

			client.leave(PreviewBankRoom(parsed.page, parsed.bank))
		})
	}
	...
	updateBank(page, bank, render) {
		...
		if (this.io.countRoomMembers(previewRoom) > 0) {
			this.io.emitToRoom(previewRoom, `preview:page-bank`, page, controlId, render.buffer)
		}

		const previewRoom2 = PreviewBankRoom(page, bank)
		if (this.io.countRoomMembers(previewRoom2) > 0) {
			this.io.emitToRoom(previewRoom2, `preview:page-bank`, page, controlId, render.buffer)
		}
	}
}
