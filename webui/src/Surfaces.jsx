...
export const SurfacesPage = memo(function SurfacesPage() {
	...
	const refreshUSB = useCallback(() => {
		...
		socketEmit(context.socket, 'devices_reenumerate', [], 30000).then(([errorMsg]) => {
								...
								<td>
									{
										...
										? <CButton color="success" onClick={() => configureDevice(dev)}><FontAwesomeIcon icon={faCog} /> Settings</CButton>
										: ''
									}
								</td>
							</tr>
...
const SurfaceEditModal = forwardRef(function SurfaceEditModal (_props, ref) {
	...
	useEffect(() => {
		...
		if (deviceInfo?.id) {
			socketEmit(context.socket, 'device_config_get', [deviceInfo.id]).then(([err, config]) => {
					...
					? <CForm>
						{
							deviceInfo.config?.includes('brightness')
								? <CFormGroup>
									...
									<CInput name="brightness" type="range" min={0} max={100} step={1} value={deviceConfig.brightness} onChange={(e) => updateConfig('brightness', parseInt(e.currentTarget.value))} />
								</CFormGroup>
								: ''
						}
		
						{
							deviceInfo.config?.includes('orientation')
								? <CFormGroup>
									...
									<CSelect name="orientation" value={deviceConfig.rotation} onChange={(e) => updateConfig('rotation', parseInt(e.currentTarget.value))}>
		...
		
						{
							deviceInfo.config?.includes('page')
								? <CFormGroup>
									...
									<CInput name="page" type="range" min={1} max={99} step={1} value={deviceConfig.page} onChange={(e) => updateConfig('page', parseInt(e.currentTarget.value))} />
		...
		
						{
							deviceInfo.config?.includes('keysPerRow')
								? <CFormGroup>
									...
									<CInput name="keysPerRow" type="range" min={1} max={99} step={1} value={deviceConfig.keysPerRow} onChange={(e) => updateConfig('keysPerRow', parseInt(e.currentTarget.value))} />
		...
		
						{
							deviceInfo.config?.includes('keysPerColumn')
								? <CFormGroup>
									...
									<CInput name="keysPerColumn" type="range" min={1} max={99} step={1} value={deviceConfig.keysPerColumn} onChange={(e) => updateConfig('keysPerColumn', parseInt(e.currentTarget.value))} />
