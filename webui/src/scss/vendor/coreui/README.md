...
reachable from what `App.scss` forwards (`root`, `reboot`, `type`, `containers`, `grid`,
`utilities/api`) plus their transitive dependencies were copied. The goal is to eventually replace
everything here with first-party CSS under `webui/src/` and delete this directory.
