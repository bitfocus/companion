...
export const InstanceVariables = function InstanceVariables({ resetToken }) {
  ...
  if (instanceId) {
    ...
    return (
      <VariablesList
        selectedInstanceLabel={instance?.label}
...
function VariablesInstanceList({ setInstance, instancesLabelMap }) {
  ...
  const options = Object.entries(context.variableDefinitions || []).map(
    ([label, defs]) => {
      ...
      const module = instance
        ? context.modules[instance.instance_type]
        : undefined;
    ...
    <div>
      ...
      {options.length === 0 ? (
        <CAlert color="primary">
          You have no connections that support variables at the moment. More
          modules will support variables in the future.
        </CAlert>
      ) : (
        options
      )}
