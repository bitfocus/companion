...
/** The grid itself, where every cell exists */
const EVERY_CELL = () => true

/** Viewing as a surface, where only the cells it names do */
...
describe('planGridDrop', () => {
	it('moves a single button to where it was dropped', () => {
		const plan = planGridDrop(at(1, 1), at(2, 3), [at(1, 1)], GRID_SIZE, occupiedAt(at(1, 1)), EVERY_CELL)
	...
	it('refuses a drop onto a cell the view does not have', () => {
		// Viewing as a surface, where the cell dropped on is one of the holes between its controls
		const plan = planGridDrop(
			...
			GRID_SIZE,
			occupiedAt(at(1, 1)),
			onlyCells(at(1, 1), at(2, 4))
		)

		expect(plan?.fitsOnGrid).toBe(false)
	})

	it('allows a drop onto a cell the view does have', () => {
		const plan = planGridDrop(
			at(1, 1),
			at(2, 3),
			[at(1, 1)],
			GRID_SIZE,
			occupiedAt(at(1, 1)),
			onlyCells(at(1, 1), at(2, 3))
		)

		expect(plan?.fitsOnGrid).toBe(true)
	})

	it('does nothing when dropped back where it started', () => {
		expect(planGridDrop(at(1, 1), at(1, 1), [at(1, 1)], GRID_SIZE, occupiedAt(at(1, 1)), EVERY_CELL)).toBeNull()
	})

	it('swaps when one button lands on another', () => {
		const plan = planGridDrop(at(1, 1), at(2, 3), [at(1, 1)], GRID_SIZE, occupiedAt(at(1, 1), at(2, 3)), EVERY_CELL)
	...
	it('keeps a region in shape, with the dragged cell under the cursor', () => {
		...
		const plan = planGridDrop(at(2, 2), at(3, 5), sources, GRID_SIZE, occupiedAt(...sources), EVERY_CELL)
	...
	it('reports a drop that would hang off the grid, rather than vanishing', () => {
		...
		const plan = planGridDrop(at(1, 6), at(1, 7), sources, GRID_SIZE, occupiedAt(...sources), EVERY_CELL)
	...
	it('will not drop only the buttons that fit', () => {
		...
		expect(planGridDrop(at(0, 0), at(1, 0), sources, GRID_SIZE, occupiedAt(...sources), EVERY_CELL)?.fitsOnGrid).toBe(
			false
		)
	})

	it('fits when the whole region lands on the grid', () => {
		expect(planGridDrop(at(1, 1), at(2, 3), [at(1, 1)], GRID_SIZE, occupiedAt(at(1, 1)), EVERY_CELL)?.fitsOnGrid).toBe(
			true
		)
	})
	...
	it('reports the cells it would land on', () => {
		...
		const plan = planGridDrop(
			...
			sources,
			GRID_SIZE,
			occupiedAt(...sources, at(2, 0), at(2, 1)),
			EVERY_CELL
		)
	...
	it('does not count a cell the region is vacating as overwritten', () => {
		...
		const sources = [at(0, 0), at(0, 1)]
		const plan = planGridDrop(at(0, 0), at(0, 1), sources, GRID_SIZE, occupiedAt(at(0, 0), at(0, 1)), EVERY_CELL)
	...
	it('swaps a whole region too - the displaced buttons go where the dragged ones came from', () => {
		const sources = [at(0, 0), at(0, 1)]
		const plan = planGridDrop(at(0, 0), at(2, 0), sources, GRID_SIZE, occupiedAt(...sources, at(2, 0)), EVERY_CELL)
	...
	it('moves rather than swaps when a region is nudged onto part of itself', () => {
		...
		const sources = [at(0, 0), at(0, 1)]
		const plan = planGridDrop(at(0, 0), at(0, 1), sources, GRID_SIZE, occupiedAt(...sources, at(0, 2)), EVERY_CELL)
	...
	it('still moves onto empty cells, where there is nothing to trade with', () => {
		const sources = [at(0, 0), at(0, 1)]
		const plan = planGridDrop(at(0, 0), at(2, 0), sources, GRID_SIZE, occupiedAt(...sources), EVERY_CELL)
	...
	it('is nothing at all when the selection was empty cells only', () => {
		expect(
			planGridDrop(at(0, 0), at(2, 0), [at(0, 0), at(0, 1)], GRID_SIZE, occupiedAt(at(2, 0)), EVERY_CELL)
		).toBeNull()
	})

	it('drops onto another page', () => {
		const plan = planGridDrop(at(1, 1, 1), at(1, 1, 4), [at(1, 1, 1)], GRID_SIZE, occupiedAt(at(1, 1, 1)), EVERY_CELL)
...
describe('previewPlacements', () => {
	...
	it('covers every cell of a region', () => {
		...
		const plan = planGridDrop(at(1, 1), at(3, 1), sources, GRID_SIZE, occupiedAt(...sources), EVERY_CELL)!
