...
export function EditButton({ page, bank, onKeyUp }) {
  ...
  const errors = Object.values(tableLoadStatus).filter(
    (s) => typeof s === "string"
  );
  ...
  const dataReady =
    !loadError &&
    !!config &&
    Object.values(tableLoadStatus).filter((s) => s !== true).length === 0;
    ...
    <KeyReceiver onKeyUp={onKeyUp} tabIndex={0} className="edit-button-panel">
      ...
      {config ? (
        <div style={{ display: dataReady ? "" : "none" }}>
          <div>
            ...
            <CDropdown className="mt-2" style={{ display: "inline-block" }}>
              ...
              <CDropdownMenu>
                ...
                <CDropdownItem onClick={() => setButtonType("pageup")}>
                  Page up
                </CDropdownItem>
                <CDropdownItem onClick={() => setButtonType("pagenum")}>
                  Page number
                </CDropdownItem>
                <CDropdownItem onClick={() => setButtonType("pagedown")}>
                  Page down
                </CDropdownItem>
              </CDropdownMenu>
            </CDropdown>
            ...
            <CButton
              ...
              onMouseDown={() =>
                context.socket.emit("hot_press", page, bank, true)
              }
              onMouseUp={() =>
                context.socket.emit("hot_press", page, bank, false)
              }
            >
              Test actions
            </CButton>
          ...
          <p>
            <b>Hint:</b> Control buttons with OSC or HTTP: /press/bank/{page}/
            {bank} to press this button remotely. OSC port 12321!
          </p>
        </div>
      ) : (
        ""
