...
export const InstanceEditPanel = memo(function InstanceEditModal({
  instanceId,
  doConfigureInstance,
  showHelp,
}) {
  ...
  const doSave = useCallback(() => {
    ...
    if (isInvalid.length > 0) {
      setError(
        `Some config fields are not valid: ${isInvalid
          .map(([k]) => k)
          .join(", ")}`
      );
      return;
    ...
    socketEmit(context.socket, "instance_config_put", [
      instanceId,
      instanceConfig,
    ])
      .then(([err, ok]) => {
      ...
      <h5>
        ...
        {moduleInfo?.help ? (
          <div
            ...
            onClick={() => showHelp(instanceConfig?.instance_type)}
          >
...
function ConfigField({ setValue, setValid, ...props }) {
  ...
  switch (definition.type) {
    ...
    case "textinput":
      return (
        <TextInputField {...props} setValue={setValue2} setValid={setValid2} />
      );
    case "number":
      return (
        <NumberInputField
