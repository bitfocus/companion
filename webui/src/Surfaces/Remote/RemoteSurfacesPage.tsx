...
export const RemoteSurfacesPage = observer(function RemoteSurfacesPage(): React.JSX.Element {
	...
	return (
		<Grid.Row className="split-panels">
			<Grid.Col xl={6} className={`primary-panel ${showPrimaryPanel ? 'block' : 'hidden xl:block'}`}>
				<MyErrorBoundary>
					<RemoteSurfacesList selectedRemoteConnectionId={selectedRemoteConnectionId} />
				</MyErrorBoundary>
			</Grid.Col>

			<Grid.Col xl={6} className={`secondary-panel ${showSecondaryPanel ? 'block' : 'hidden xl:block'}`}>
