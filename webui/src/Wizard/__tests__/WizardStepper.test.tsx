...
describe('WizardStepper', () => {
	it('renders one entry per item with sequential numbers', () => {
		render(<WizardStepper items={ITEMS} currentIndex={1} onJump={() => {}} />)
	...
	it('marks the active step with aria-current and the active class', () => {
		const { container } = render(<WizardStepper items={ITEMS} currentIndex={2} onJump={() => {}} />)

		expect(screen.getByRole('button', { name: 'Services' })).toHaveAttribute('aria-current', 'step')
		expect(container.querySelector('.wizard-stepper-step-active')).toHaveTextContent('Services')
	})

	it('marks earlier steps as complete and later steps as upcoming', () => {
		const { container } = render(<WizardStepper items={ITEMS} currentIndex={3} onJump={() => {}} />)

		const complete = container.querySelectorAll('.wizard-stepper-step-complete')
		expect(complete).toHaveLength(2) // Surfaces + Services
		const upcoming = container.querySelectorAll('.wizard-stepper-step-upcoming')
	...
	it('calls onJump with the item index when a step is clicked', async () => {
		...
		const onJump = vi.fn()
		render(<WizardStepper items={ITEMS} currentIndex={1} onJump={onJump} />)
