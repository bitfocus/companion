...
export const LogPanel = memo(function LogPanel() {
  ...
  const doToggleWarn = useCallback(() => doToggleConfig("warn"), [
    doToggleConfig,
  ]);
  const doToggleInfo = useCallback(() => doToggleConfig("info"), [
    doToggleConfig,
  ]);
  const doToggleDebug = useCallback(() => doToggleConfig("debug"), [
    doToggleConfig,
  ]);
        ...
        <CCol lg={12} className="log-buttons">
          <CButtonGroup>
            <CButton
              color="warning"
              size="sm"
              onClick={doToggleWarn}
              style={{ opacity: config.warn ? 1 : 0.2 }}
            >
              Warning
            </CButton>
            <CButton
              color="info"
              size="sm"
              onClick={doToggleInfo}
              style={{ opacity: config.info ? 1 : 0.2 }}
            >
              Info
            </CButton>
            <CButton
            ...
            if (h.level === "error" || config[h.level]) {
              ...
              return (
                <div key={h.id} className={`log-line log-type-${h.level}`}>
                  {time_format} <strong>{h.source}</strong>:{" "}
