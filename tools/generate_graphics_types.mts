...
generatedFile += `import type { JsonValue } from 'type-fest'\n`
generatedFile += `import type { ExpressionOrValue } from '../Model/Options.js'\n`
generatedFile += `import type { ButtonGraphicsElementUsage, ButtonGraphicsDecorationType, ButtonGraphicsShowStatusIcons, HorizontalAlignment, VerticalAlignment, TextStyle, CompositeElementOptionKey, ColorValue } from './StyleModel.js'\n`
generatedFile += '\n'
...
for (const field of canvasElementSchema.flatMap((s) => s.fields)) {
	if (field.id === 'decoration') {
		...
		generatedFile += `\tshowStatusIcons: ButtonGraphicsShowStatusIcons\n`
	} else {
		const tsType = convertFieldType(field, false)
		generatedFile += `\t${field.id}: ${tsType}\n`
	}
