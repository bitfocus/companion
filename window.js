...
function skeleton_info_draw() {
	...
	document.getElementById('model').innerHTML =
		skeleton_info.appName +
		' v' +
		skeleton_info.appVersion +
		' (' +
		skeleton_info.appBuild.replace(/-*master-*/, '').replace(/^-/, '') +
		')'
...
function get_interfaces_list() {
	network.get_interfaces_list(function (err, list) {
		document.getElementById('ifs').innerHTML =
			"<option value='' selected>Change network interface</option><option value='127.0.0.1'>localhost / 127.0.0.1</option>"
		...
		for (var n in list) {
			...
			if (obj.ip_address !== null) {
				document.getElementById('ifs').innerHTML +=
					"<option value='" + obj.ip_address + "'>" + obj.name + ': ' + obj.ip_address + ' (' + obj.type + ')</option>'
			}
		}
