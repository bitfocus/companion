...
| ![Button latch on](images/button-latch-on-with-topbar.png?raw=true 'Button latch on')    | The button was pressed. Usually this bar is shown only briefly, but it can stay on if you press and don't release the button. This can happen via an Action too, if you use "Button Trigger Press" without "Button Trigger Release" or instead of using "Button Trigger Press and Release" for example. |
| ![Button delay](images/button-delay.png?raw=true 'Button delay')                         | Some actions are still queued to run for this button, typically due to a Wait action (see [Actions](#header-actions) above).                                                                                                                                                                            |
