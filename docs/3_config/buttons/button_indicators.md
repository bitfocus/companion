...
| ![Button latch on](images/button-latch-on-with-topbar.png?raw=true "Button latch on")    | The button was pressed (if shown briefly) or button is held down.                              |
| ![Button delay](images/button-delay.png?raw=true "Button delay")                         | There are delayed actions queued to run for this button (using the **internal: Wait** action). |
