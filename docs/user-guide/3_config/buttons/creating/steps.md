...
Sometimes you don't want it to progress between steps automatically like this. You can disable this with the **Step Progression** option in the button configuration further up. Instead you either choose to make the current step number be based upon an expression, or to only be changed by the internal actions.
With these, you can do complex scenarios like shift layers where holding one button will change other buttons to step 2, and switch back to 1 upon release.
