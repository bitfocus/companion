...
To keep the common edits quick, each button also has a **Pinned** view: the properties you change most often, gathered from across its elements into one panel, so relabelling a button doesn't mean selecting an element and scrolling to find its text. Every element starts with a sensible set pinned, and you can pin or unpin any property with the pin that appears beside its label.
