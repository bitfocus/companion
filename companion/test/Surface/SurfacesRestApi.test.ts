...
import { createSurfacesRestApiRouter } from '../../lib/Surface/SurfacesRestApi.js'
...
const SURFACES_PATH = `${REST_API_BASE_PATH}/surfaces/v1`
...
function createTestRegistry(surfaceController: SurfaceController, pageStore: IPageStore): Registry {
	return {
		instance: {
			createRestApiRouter: () => express.Router(),
		},
		surfaces: {
			createRestApiRouter: (logger: Logger) => createSurfacesRestApiRouter(logger, surfaceController, pageStore),
		},
...
function createService(): TestService {
	...
	const restApiRouter = createRestApiRouter(
		createTestRegistry(surfaceController, pageStore),
...
describe('Surfaces REST API', () => {
	describe('GET /surfaces', () => {
		test('returns a flat list of the surfaces of all groups', async () => {
			...
			expect(res.body.data).toEqual([
				...
				expect.objectContaining({ id: 'surface-2', brightness: null }),
				expect.objectContaining({ id: 'surface-3', isConnected: false, size: null }),
			])
		})
	...
	describe('PATCH /surfaces/:surfaceId', () => {
		test('sets the brightness and returns the updated surface', async () => {
			const service = createService()
			service.surfaceController.getDevicesList.mockReturnValue(createDevicesList())
			service.surfaceController.setDeviceBrightness.mockReturnValue(undefined)
		...
		test.each([{ brightness: 101 }, { brightness: -1 }, { brightness: 50.5 }, { brightness: '50' }, {}])(
