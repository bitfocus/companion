...
describe('trigger controls', () => {
	...
	afterEach(async () => {
		// createTestApp() may have rejected before assigning, so guard so teardown can't mask that error
		await app?.close()
	})
	...
	test('a disabled trigger does not run', async () => {
		...
		app.registry.variables.custom.setValue('watch', 'go')
		// Both changes set result to 'triggered', so prove the disabled trigger ignored the first change
		// before enabling - otherwise a stray first run would go unnoticed
		await settle()
		expect(app.getCustomVariableValue('result')).toBe('initial')
