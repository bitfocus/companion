...
describe('planGridTransfer', () => {
	...
	describe('validation', () => {
		it('rejects moving the same button twice, since it can only be in one place', () => {
			expect(() =>
				planGridTransfer(
					'move',
					[
		...
		it('allows copying the same button twice, since each destination gets a new one', () => {
			const plan = planGridTransfer(
				'copy',
				[
					{ fromLocation: at(0, 0), toLocation: at(1, 1) },
			...
			const placements = byLocation(plan.placements)
			expect(placements['1/1/1']).toEqual({ location: at(1, 1), kind: 'clone', sourceControlId: 'control:1/0/0' })
			expect(placements['1/2/2']).toEqual({ location: at(2, 2), kind: 'clone', sourceControlId: 'control:1/0/0' })
		})
