...
/** A minimal conversion result; each set defaults to empty. */
...
describe('LayeredButtonDrawer', () => {
	...
	beforeEach(() => {
		...
		deps = {
			...
			// The drawer subscribes to the graphics 'button_drawn' event, so this must be an emitter.
			graphics: Object.assign(new EventEmitter(), {
	...
	describe('getDrawStyle', () => {
		...
		it('handles a control with no location (null location string)', async () => {
	...
	describe('referenced button drawn (button_drawn event)', () => {
		...
		it('suppresses the redraw for an established cycle (target still references us back)', async () => {
			...
			deps.graphics.emit('button_drawn', OTHER_LOCATION, {
				referencedLocations: new Set([formatLocation(LOCATION)]),
			})
