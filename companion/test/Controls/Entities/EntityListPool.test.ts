...
describe('EntityListPool', () => {
	...
	test('construction', () => {
		...
		const executeExpressionInControl = vi.fn()
		const pool = new ControlEntityListPoolButton(deps, sendRuntimeProps, executeExpressionInControl, true)
		...
		expect(deps.reportChange).toHaveBeenCalledTimes(0)
	})
	...
	test('add step from empty', () => {
		...
		expect(sendRuntimeProps).toHaveBeenCalledTimes(0)
		expect(executeExpressionInControl).toHaveBeenCalledTimes(0)
		expect(deps.reportChange).toHaveBeenCalledTimes(1)
		expect(deps.reportChange).toHaveBeenCalledWith({
			redraw: true,
