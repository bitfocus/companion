...
export class VariablesAndExpressionParser {
	...
	parseEntityOption(
		...
		try {
			...
			if (!rawValue) {
				return { ok: true, value: undefined, variableIds: new Set() }
			}
			...
			if ((rawValue.isExpression && options.allowExpression) || options.forceExpression) {
				...
				return { ok: true, value: parseResult.text, variableIds: parseResult.variableIds }
			} else {
				// 'expression-or-variables' with isExpression=false - just use the value as-is
				return { ok: true, value: rawValue.value, variableIds: new Set() }
			}
		} catch (e) {
			// Backstop: parseEntityOption must never throw, so callers can always rely on the result shape
			return { ok: false, error: stringifyError(e, true) || 'Unknown error', variableIds: new Set() }
		}
	}
}
