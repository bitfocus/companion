...
export function planGridTransfer(
	...
	// A button can be copied to as many places as you like, because each destination gets a new one.
	// It can only *be* in one place, though, so moving or swapping the same one twice would have it
	// claim two cells at once - and the grid would be left with two cells pointing at one control.
	if (operation !== 'copy') {
		const seenSources = new Set<string>()
		for (const { fromLocation } of realPairs) {
			const key = formatLocation(fromLocation)
			if (seenSources.has(key)) {
				throw new GridTransferError(`Cannot ${operation} ${key} twice`)
			}
			seenSources.add(key)
		}
	}

	// If doing a swap, empty cells need to be swapped too. For other modes, empty cells should be left alone.
	const carryingPairs =
	...
	// Resolve the plan in terms of what ends up at each location
