...
export class PreviewGraphics {
				...
				.subscription(async function* ({ signal, input }) {
					...
					try {
						...
						self.#buttonReferencePreviews.set(id, {
							...
							referencedVariableIds: Array.from(locationValue.referencedVariableIds),
						})
	...
	#triggerRecheck(previewSession: PreviewSession): void {
		...
		previewSession.referencedVariableIds = Array.from(locationValue.referencedVariableIds)
