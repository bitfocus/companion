...
import { ControlEntityListPoolCustomVariables } from '../Controls/Entities/EntityListPoolCustomVariables.js'
...
export interface VariablesCustomVariableEvents {
	definition_changed: [id: string, info: CustomVariableDefinition | null]
}

export class CustomVariablesController extends EventEmitter<VariablesCustomVariableEvents> {
	...
	#entities!: ControlEntityListPoolCustomVariables
	 ...
	 */
	init(instanceDefinitions: InstanceDefinitions, internalModule: InternalController, moduleHost: ModuleHost): void {
		...
		this.#entities = new ControlEntityListPoolCustomVariables({
			...
			localVariablesChanged: (changedVariables: Set<string>) => {
				// TODO
			},
		})

		// this.#entities.loadStorage()
	...
	// 	try {
	// 		this.emit('definition_changed', name, info)
	// 	} catch (e) {
	...
	verifyConnectionIds(knownConnectionIds: Set<string>): void {
		this.#entities.verifyConnectionIds(knownConnectionIds)
	}
}
