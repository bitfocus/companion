...
export class ServiceHttpApi extends CoreBase {
	...
	#setupLegacyHttpRoutes() {
		...
		this.#legacyApiRouter.get('/set/custom-variable/:name', (req, res) => {
			...
			const result = this.registry.customVariables.setValue(req.params.name, String(req.query.value))
			if (result) {
	...
	#customVariableSetValue = (req, res) => {
		...
		const result = this.registry.customVariables.setValue(variableName, variableValue)
		if (result) {
	...
	#customVariableGetValue = (req, res) => {
		...
		const result = this.registry.customVariables.getValue(variableName)
		if (result === undefined) {
			res.status(404).send('Not found')
		} else {
