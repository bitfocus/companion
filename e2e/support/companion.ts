...
export async function launchCompanion(): Promise<CompanionServer> {
	...
	let ready: { port: number; configDir: string }
	try {
		ready = await new Promise<{ port: number; configDir: string }>((resolve, reject) => {
		...
		// Startup failed (timeout/spawn error), so no CompanionServer is returned and nobody can call
		// stop() - make sure we don't leak the child and keep the Playwright worker alive
